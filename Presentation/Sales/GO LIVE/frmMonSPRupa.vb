...
Public Class frmMonSPRupa
    ...
    Sub reload_table()
        ...
        cmd = New SqlClient.SqlCommand(" select  sum(Qty) Qty, sum(jml_rp_trans-jml_ppn) DPP, sum(jml_ppn) PPN, sum(jml_rp_trans) Total, " & kolom & " from sales.dbo.VSOD_OMZET as s where  tipe_trans not in  ('" & ServiceSpringUmum & "', '" & ServiceSponUmum & "') and  kd_customer <> 'INTERNAL'" & sql & " group by " & kolom, sqlconn)
