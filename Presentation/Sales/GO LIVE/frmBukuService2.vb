...
Public Class frmBukuService2
    ...
    Private Sub reload_table(ByVal kode As String)
        ...
        If Not Me.Parent Is Nothing Then
            ...
            lblFound.Refresh()
            If umum.kddep = KD_DIV_SPRING Then
                DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select Nama_Customer,Almt_pnrm,No_Telp,No_sp,Status,Deskripsi,Qty,Keterangan,Tgl_kirim,realisasi_tgl_kirim,isClosed,Owner,Tipe_trans from Sales.dbo.vBuku where  '1'='" & kode & "' and tipe_trans in ('" & ServiceSpringUmum & "', '" & ServiceSpringInternal & "') " & sql & sqlstat & " order by no_sp, len(no_seq), no_seq", sqlconn))
            Else
                DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select Nama_Customer,Almt_pnrm,No_Telp,No_sp,Status,Deskripsi,Qty,Keterangan,Tgl_kirim,realisasi_tgl_kirim,isClosed,Owner,Tipe_trans from Sales.dbo.vBuku where  '1'='" & kode & "' and tipe_trans in ('" & ServiceSponUmum & "', '" & ServiceSponInternal & "') " & sql & sqlstat & " order by no_sp, len(no_seq), no_seq", sqlconn))
            End If
            DA.Fill(dsete, "SOB")
