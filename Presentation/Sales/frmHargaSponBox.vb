...
Public Class frmHargaSponBox
    ...
    Private Sub frmHargaSponBox_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        dadbe = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select Kode_Barang, Nama_Barang  " & _
                    ...
                    "where kd_jenis in (select id_data from SIF.dbo.SIF_Gen_Reff_D where ref_role ='SOD' and id_ref_file='BUSAPRICE' and rec_stat='Y') and rec_stat='Y'", sqlconn))
        dadbe.Fill(dsete, "VBARANG")
        dadbe = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select *, 0 state  from sif.dbo.SIF_Harga_SponBox a where kd_stok in (select kode_barang from sif.dbo.sif_barang where kd_jenis IN (select id_data from SIF.dbo.SIF_Gen_Reff_D where ref_role ='SOD' and id_ref_file='BUSAPRICE' and rec_stat='Y')) ", sqlconn))
        dadbe.Fill(dsete, "BOX")

        repBarang.DataSource = dsete.Tables("VBARANG")
        txtBarang.Properties.DataSource = dsete.Tables("VBARANG")
    ...
    Private Sub GV_InitNewRow(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Grid.InitNewRowEventArgs) Handles GV.InitNewRow
        ...
        If txtBarang.Text <> "" Then GV.SetRowCellValue(e.RowHandle, "KD_STOK", txtBarang.EditValue)
    End Sub
    ...
    Private Sub txtBarang_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtBarang.EditValueChanged
        If txtBarang.Text <> "" Then
            Dim ftr As String = "KD_STOK = '" & txtBarang.EditValue & "'"
            GV.Columns("KD_STOK").FilterInfo = New DevExpress.XtraGrid.Columns.ColumnFilterInfo(DevExpress.XtraGrid.Columns.ColumnFilterType.Value, ftr)
            KD_STOK.Group()
        Else
            GV.ActiveFilter.Clear()
            KD_STOK.UnGroup()
