...
Public Class frmRcnReturSrv
    ...
    Private Sub reload(Optional ByVal vtrans As String = "")
        Dim dr As SqlDataReader
        If vtrans = "" Then

            dr = New SqlClient.SqlCommand("select * from SIF.dbo.SIF_Gen_Reff_D where Id_Ref_Data='KGTPPIC' and Id_Data='" & txtJnsTrans.EditValue & "'", sqlconn).ExecuteReader
            dr.Read()
...
"order by uni.no_sp, uni.no_seq", sqlconn).ExecuteReader
    ...
    Private Sub frmRcnKrm_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        setStatus()
        State = "ADD"
        If Not ds1.Tables("JnsTrans") Is Nothing Then ds1.Tables("JnsTrans").Clear()
        da1 = New SqlDataAdapter(New SqlCommand("select * from SIF.dbo.SIF_Gen_Reff_D where Id_Ref_Data='KGTPPIC' and (Id_Data='03' or Id_Data='04')", sqlconn))
        da1.Fill(ds1, "JnsTrans")
        txtJnsTrans.Properties.DataSource = ds1.Tables("JnsTrans")
        'reload()
    ...
    Public Sub add_click()
        txtTanggal.DateTime = getTanggal()
    ...
    Public Sub edit_click()
        ...
        da3 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select P.tanggal Tanggal, P.no_trans [No Transaksi], S.Desc_Data [Transaksi], S.Val_kode1 [Kode] from PROD.dbo.PROD_rcn_krm_m P " & _
                                                                    "left join SIF.dbo.SIF_Gen_Reff_D S on P.tipe_trans=S.Val_kode1 " & _
                                                                    "where S.Id_Ref_Data='KGTPPIC' and (S.Id_Data='03' or S.Id_Data='04') " & _
        ...
        If Not frmCari.row Is Nothing Then
            ...
            txtJnsTrans.Properties.ReadOnly = True

            txtNoTrans.Text = frmCari.row("No Transaksi")
            txtTanggal.DateTime = frmCari.row("Tanggal")
            txtJnsTrans.Text = frmCari.row("Transaksi")
           

            reload(frmCari.row("Kode"))
            btnSave = True
            btnadd = False
            btnEdit = False
            btnDelete = True
    ...
    Public Sub cancel_click()
        txtJnsTrans.Properties.ReadOnly = False
        btnSave = False
        btnCancel = False
        btnDelete = False
        btnEdit = True
    ...
    Private Sub txtJnsTrans_EditValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtJnsTrans.EditValueChanged
        reload()
    End Sub
End Class
