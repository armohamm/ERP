...
Public Class frmRcnRph_Koreksi
    ...
    Private Sub reload()
        ...
        da1 = New SqlDataAdapter(New SqlCommand("Select m.no_rph from PROD.dbo.PROD_rcn_prod_m m", sqlconn))
        da1.Fill(ds1, "noTrans")


        txtNoTrans.Properties.DisplayMember = "no_rph"
        txtNoTrans.Properties.ValueMember = "no_rph"
    ...
    Public Sub edit_click()
        ...
        da3 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select P.tanggal Tanggal, P.no_rph, p.no_rph_koreksi from PROD.dbo.PROD_rcn_prod_koreksi_m P " & _
                                                                    "group by P.no_rph_koreksi, P.no_rph, P.tanggal order by P.tanggal desc, P.no_rph desc", sqlconn))
        ...
        If Not frmCari.row Is Nothing Then
            ...
            txt_notrans_koreksi.Text = frmCari.row("no_rph_koreksi")
            txtNoTrans.EditValue = frmCari.row("no_rph")
    ...
    Public Sub save_click()
        ...
        If txtNoTrans.EditValue = "" Then
            ...
            Exit Sub
        End If
        Dim tgl As Date = txtTanggal.EditValue
        If State = "ADD" Then
            ...
            notrans = getNoTrans("RPH_KOR", tgl, BTRANS)

            Dim row As DataRow = ds1.Tables("Rcn_Krm_M").Rows(0)
            txt_notrans_koreksi.Text = notrans
            row("tanggal") = tgl
            row("no_rph_koreksi") = txt_notrans_koreksi.Text
            ...
            For i = ds1.Tables("Rcn_Krm_D").Rows.Count - 1 To 0 Step -1
                ...
                If Not TypeOf rows("jumlah_koreksi") Is DBNull Then
                    If rows("jumlah_koreksi") > 0 Then
                        rows("no_rph_koreksi") = txt_notrans_koreksi.Text
            ...
            Try
                ...
                showMessages("Data Koreksi rencana produksi Berhasil Disimpan")
                btnSave = False
                setStatus()
        ...
        ElseIf State = "EDIT" Then
            ...
            qstr = "delete from PROD.dbo.PROD_rcn_prod_koreksi_d where no_rph_koreksi='" & txt_notrans_koreksi.Text & "'"
            dr = New SqlClient.SqlCommand(qstr, sqlconn, BTRANS).ExecuteReader
            dr.Close()
            qstr = "delete from PROD.dbo.PROD_rcn_prod_koreksi_m where no_rph_koreksi='" & txt_notrans_koreksi.Text & "'"
            dr = New SqlClient.SqlCommand(qstr, sqlconn, BTRANS).ExecuteReader
            dr.Close()
            ...
            BTRANS.Commit()
            txtNoTrans.Text = notrans
            showMessages("Data Koreksi rencana produksi Berhasil Diedit.")
            btnSave = False
            setStatus()
    ...
    Sub isi_detail()
        ...
        ElseIf State = "EDIT" Then
            ...
            da1 = New SqlDataAdapter(New SqlCommand("Select m.* From PROD.dbo.PROD_rcn_prod_koreksi_m m where 1= 0", sqlconn))
            da1.Fill(ds1, "Rcn_Krm_M")
            dr = New SqlClient.SqlCommand("Select m.* From PROD.dbo.PROD_rcn_prod_koreksi_m m where m.no_rph_koreksi ='" & txt_notrans_koreksi.Text & "'", sqlconn).ExecuteReader
            While dr.Read

                Dim rwm As DataRow = ds1.Tables("Rcn_Krm_M").NewRow
                rwm("no_rph") = dr.Item("no_rph")
                rwm("no_rph_koreksi") = dr.Item("no_rph_koreksi")
                rwm("tipe_trans") = dr.Item("tipe_trans")
                rwm("kd_jadwal") = dr.Item("kd_jadwal")
                rwm("kd_jenis") = dr.Item("kd_jenis")
                rwm("rec_stat") = dr.Item("rec_stat")
            ...
            If Not ds1.Tables("Rcn_Krm_D") Is Nothing Then ds1.Tables("Rcn_Krm_D").Clear()
            da2 = New SqlDataAdapter(New SqlCommand("Select d.*,'' Nama_Customer,'' Nama_Barang,'' Tgl_Kirim, '' Kd_Satuan,0.00 jumlah_koreksi2,0.00 jumlah_koreksi_maks From PROD.dbo.PROD_rcn_prod_koreksi_d d where 1=0", sqlconn))
            ...
            dr2 = New SqlClient.SqlCommand("Select dt.*,v.Nama_Customer,v.Nama_Barang,v.Tgl_Kirim ,v.tgl_kirim_SO,v.Kd_Satuan, " & _
                                                    ...
                                                    "From PROD.dbo.PROD_rcn_prod_koreksi_d dt " & _
                                                    "left join PROD.dbo.PRODV_MON_SO v " & _
                                                    "on dt.no_sp = v.No_sp and dt.no_sp_dtl = v.no_seq_d and dt.no_seq = v.No_sp_box " & _
                                                    "left join (Select d.no_rph,d.no_sp,d.no_sp_dtl,d.no_seq, " & _
                                                    "SUM(d.jumlah_koreksi) jumlah_koreksi2 " & _
                                                    "From PROD.dbo.PROD_rcn_prod_koreksi_d d " & _
                                                    "where d.no_rph_koreksi <> '" & txt_notrans_koreksi.Text & "'" & _
                                                    "Group BY " & _
                                                    "d.no_rph, d.no_sp,d.no_sp_dtl,d.no_seq) a  " & _
                                                    "on a.no_rph = dt.no_rph and a.no_sp =dt.no_sp " & _
                                                    "and a.no_sp_dtl = dt.no_sp_dtl and a.no_seq = dt.no_seq " & _
                                                    "where dt.no_rph_koreksi ='" & txt_notrans_koreksi.Text & "'", sqlconn).ExecuteReader

            While dr2.Read
                Dim rw As DataRow = ds1.Tables("Rcn_Krm_D").NewRow
                rw("kd_cabang") = dr2.Item("kd_cabang")
                rw("kd_departemen") = dr2.Item("kd_departemen")
                rw("tanggal") = dr2.Item("tanggal")
                rw("no_rph") = dr2.Item("no_rph")
                rw("no_rph_koreksi") = dr2.Item("no_rph_koreksi")
                rw("kd_jenis") = dr2.Item("kd_jenis")
                ...
                rw("bom_service") = dr2.Item("bom_service")
                rw("barcode") = dr2.Item("barcode")
    ...
    Private Sub txtNoTrans_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtNoTrans.EditValueChanged
        ...
        If State = "ADD" Then
            ...
            da1 = New SqlDataAdapter(New SqlCommand("Select m.* From PROD.dbo.PROD_rcn_prod_koreksi_m m where 1= 0", sqlconn))
            da1.Fill(ds1, "Rcn_Krm_M")
            dr = New SqlClient.SqlCommand("Select m.* From PROD.dbo.PROD_rcn_prod_m m where m.no_rph ='" & txtNoTrans.EditValue & "'", sqlconn).ExecuteReader
            While dr.Read
            ...
            dr2 = New SqlClient.SqlCommand("Select dt.*,v.Nama_Customer,v.Nama_Barang,v.Tgl_Kirim ,v.tgl_kirim_SO,v.Kd_Satuan, " & _
                                           "ISNULL(a.jumlah_koreksi2,0) jumlah_koreksi2 , " & _
                                           "(dt.jumlah - ISNULL(a.jumlah_koreksi2,0)) as jumlah_koreksi_maks " & _
                                           "From PROD.dbo.PROD_rcn_prod_d dt " & _
                                           "left join PROD.dbo.PRODV_MON_SO v " & _
                                           "on dt.no_sp = v.No_sp and dt.no_sp_dtl = v.no_seq_d and dt.no_seq = v.No_sp_box " & _
                                           "left join (Select d.no_rph,d.no_sp,d.no_sp_dtl,d.no_seq, " & _
                                           "SUM(d.jumlah_koreksi) jumlah_koreksi2 " & _
                                           ...
                                           "on a.no_rph = dt.no_rph and a.no_sp =dt.no_sp " & _
                                           "and a.no_sp_dtl = dt.no_sp_dtl and a.no_seq = dt.no_seq " & _
                                           "where dt.no_rph ='" & txtNoTrans.EditValue & "'", sqlconn).ExecuteReader
