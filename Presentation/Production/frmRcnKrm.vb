...
Public Class frmRcnKrm
    ...
    Private Sub reload()
        ...
        If State = "ADD" Then
            ...
            dr = New SqlClient.SqlCommand("select v.*, ISNULL(i.nilai,0) nilai, ISNULL(i.nilai_m3,0) nilai_m3 " & _
        ...
        ElseIf State = "EDIT" Then
            ...
            '    "select v.*, '' no_trans,0 jumlah, 0 jml_indeks, 0 jml_m3  " & _
            '    "from PROD.dbo.PRODV_MON_SO v  " & _
            '    "left join PROD.dbo.PROD_rcn_krm_d d on v.no_sp = d.no_sp and v.no_seq_d=d.no_sp_dtl and v.No_sp_box = d.no_seq " & _
            '    "where Len(v.no_sp) > 10 And v.qty_sisa_krm > 0 and d.no_sp is null " & _
            '    "UNION  " & _
            '    "select v.*, d.no_trans,d.jumlah, d.jml_indeks, ISNULL(d.jml_m3,0)  " & _
            ...
            dr = New SqlClient.SqlCommand("select uni.*,ISNULL(i.nilai,0) nilai from (  " & _
                                          ...
                                          "or Len(v.no_sp) > 10 And v.qty_sisa_krm > 0  and d.no_trans is Null) Subquery1 " & _
                                          "LEFT OUTER JOIN " & _
    ...
    Public Sub save_click()
        ...
        ElseIf State = "EDIT" Then  '------------Simpan EDIT
            ...
            '    End If
            'Next
            'If Not ds1.Tables("KIRIM1") Is Nothing Then ds1.Tables("KIRIM1").Clear()
            'da1 = New SqlDataAdapter(New SqlCommand("select t.*, '' nama_jenis, '' nama_barang,'' qty_order,'' sisa, '' tgl_kirim, '' prioritas, 0.00 nilai, 0.00 nilai_m3, '' kd_stok, '' nama_customer, '' nama_wilayah, '' nama_area from PROD.dbo.PROD_rcn_krm_d t where 1=0", sqlconn))
            'da1.Fill(ds1, "KIRIM1")
