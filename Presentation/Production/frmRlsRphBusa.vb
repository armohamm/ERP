...
Public Class frmRlsRphBusa
    ...
    Public Sub cetak_click()
        ...
        ' callReport(App_Path() & "\report\crzRlsRph3.rpt", "", "NoRPH=" & TextEdit1.Text)
        frmCetakRPH.Jenis_RPH = "RPHBUSA"
    ...
    Public Sub tampil()
        ...
        "d.jns_produksi [Jenis Produksi], m.no_spk [No Spk] from PROD.dbo.PRODV_RCN_PROD d " & _
        "left join (Select DISTINCT m.no_rph, m.no_spk from PROD.dbo.PROD_spk_m m) m  " & _
        "on m.no_rph = d.no_rph " & _
        "where kd_departemen='" & kddep & "' and jns_produksi = 'PRODUKSI' and sts_centi <> 'Y' " & _
        "group by d.no_rph, d.kd_jadwal, d.tanggal, d.jns_produksi, m.no_spk order by tanggal desc, d.no_rph desc "
        ...
        If Not frmCari.row Is Nothing Then
            ...
            If Not ds2.Tables("rph2") Is Nothing Then ds2.Tables("rph2").Clear()
            Dim sqlcmd As New SqlCommand
            sqlcmd.CommandText = "select Nama_Barang, Nama_Jenis,Jenis_Group,Nama_Jenis_Group,kd_jenis,no_rph, " & _
                                 "Nama_Merk, Nama_Tipe, Merk_Kain,kd_tipe,kd_sub_tipe , Nama_Sub_Tipe,kd_kain, " & _
                                 "jumlah, u.Tinggi, u.Lebar, u.Panjang, " & _
                                 "(Select SUBSTRING(( Select ' ' + so.Keterangan  + '/' + CONVERT(varchar(10),FLOOR(d.jumlah))+'*' " & _
                                 "from PROD.dbo.PRODV_RCN_PROD S1  " & _
                                 "inner join PROD.dbo.PROD_rcn_prod_d d " & _
                                 "on d.no_rph = s1.no_rph and d.kd_barang = s1.Kode_Barang  " & _
                                 ...
                                 "where(s1.Kode_Barang = s2.Kode_Barang) " & _
                                 "and S1.no_rph = '" & frmCari.row("NO TRANS") & "' and S1.kd_jenis=s2.kd_jenis and S1.kd_merk = s2.kd_merk and S1.kd_tipe = s2.kd_tipe and S1.kd_kain=s2.kd_kain  For XML PATH ('')),2,500)) as ket " & _
                                 "from PROD.dbo.PRODV_RCN_PROD s2 " & _
                                 "left join SIF.dbo.SIF_Ukuran u " & _
                                 "on s2.kd_ukuran = u.Kode_Ukuran " & _
                                 "where kd_departemen = '" & kddep & "' and no_rph = '" & frmCari.row("NO TRANS") & "' and kd_jns_persd ='1'"
