...
Public Class frmRlsSpkTunda
    ...
    Private Sub reload(Optional ByVal notrans As String = "")
        ...
        If Not ds1.Tables("spkorang") Is Nothing Then ds1.Tables("spkorang").Clear()
        ...
        da1 = New SqlDataAdapter(New SqlCommand("select distinct(no_spk_man_rls) as no_spk_man_rls from PROD.dbo.PROD_rls_spk_man_m order by no_spk_man_rls", sqlconn))
        da1.Fill(ds1, "spkorang")
        txtOrang.Properties.DataSource = ds1.Tables("spkorang")
        txtOrang.Properties.DisplayMember = "no_spk_man_rls"
        txtOrang.Properties.ValueMember = "no_spk_man_rls"

        da1 = New SqlDataAdapter(New SqlCommand("select distinct(no_spk), no_rph from PROD.dbo.PROD_spk_m where sts_centi='N' or sts_centi is NULL order by no_spk", sqlconn))
        da1.Fill(ds1, "nospk")
    ...
    Private Sub txtOrang_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtOrang.EditValueChanged
        If State = "ADD" Then
            If Not txtOrang.EditValue Is Nothing Then
                If Not ds1.Tables("Orang") Is Nothing Then ds1.Tables("Orang").Clear()
                da1 = New SqlDataAdapter(New SqlCommand("select kd_kegiatan,kd_kegiatan_spk from PROD.dbo.PROD_rls_spk_man_m where no_spk_man_rls='" & txtOrang.EditValue & "'", sqlconn))
                da1.Fill(ds1, "Orang")

                Dim dr As DataRow = ds1.Tables("Orang").Rows(0)
                txtkgtn.EditValue = dr("kd_kegiatan")
                txtKegiatan.EditValue = dr("kd_kegiatan_spk")
    ...
    Public Sub add_click()
        ...
        txtNoSpkMan.Properties.ReadOnly = False
    ...
    Public Sub edit_click()
        Dim qstr As String = "select t.tanggal [Tanggal], t.no_spk_tunda [No SPK Man], t.no_spk [No SPK],t.no_rph [No RPH], " & _
            "g.Desc_Data [KEGIATAN],g1.Desc_Data [KEGIATAN SPK],p.Nama_Pegawai [PEGAWAI], " & _
            "g2.Desc_Data [GROUP KERJA],u.kd_mesin [MESIN] " & _
            "from PROD.dbo.PROD_spk_tunda_m t " & _
            "left join PROD.dbo.PROD_rls_spk_man_m m " & _
            "on m.no_spk_man_rls=t.no_spk_man_rls " & _
            ...
            "where (t.rec_stat='N' or t.rec_stat is NULL) and g1.Id_Ref_File='SPK' " & _
        ...
        If Not frmCari.row Is Nothing Then
            ...
            reload()
            If Not ds1.Tables("nospk") Is Nothing Then ds1.Tables("nospk").Clear()
            da1 = New SqlDataAdapter(New SqlCommand("select distinct(no_spk), no_rph from PROD.dbo.PROD_spk_tunda_m order by no_spk", sqlconn))
            da1.Fill(ds1, "nospk")
            txtNoSpk.Properties.DataSource = ds1.Tables("nospk")

            qstr = "select P.* from PROD.dbo.PROD_spk_tunda_m P where P.no_spk_tunda='" & frmCari.row("No SPK Man") & "'"
            If Not ds1.Tables("spkmMaster") Is Nothing Then ds1.Tables("spkmMaster").Clear()
            ...
            txtNoSpkMan.Text = rowm("no_spk_tunda")
            txtTgl.DateTime = rowm("tanggal")
            txtOrang.EditValue = rowm("no_spk_man_rls")
            ...
            If Not ds1.Tables("spkmDetail") Is Nothing Then ds1.Tables("spkmDetail").Clear()
            da3 = New SqlDataAdapter(New SqlCommand("select d.*, '' nama_barang,'' Nama_Tipe,'' Merk_Kain,'' Nama_Ukuran, 0 sisa from PROD.dbo.PROD_spk_tunda_d  d where no_spk='" & txtNoSpk.EditValue & "' and kd_kegiatan_spk='" & txtKegiatan.EditValue & "'  order by kd_barang", sqlconn))
    ...
    Public Sub save_click()
        ...
        If State = "ADD" Then
            ...
            End If
            If Not ds1.Tables("spkmMaster") Is Nothing Then ds1.Tables("spkmMaster").Clear()
            da2 = New SqlDataAdapter(New SqlCommand("select * from PROD.dbo.PROD_spk_tunda_m  d where 1=0", sqlconn))
            da2.Fill(ds1, "spkmMaster")
            ...
            For i = ds1.Tables("spkmDetail").Rows.Count - 1 To 0 Step -1
                ...
                If Not TypeOf rows("jumlah_kerja") Is DBNull Then
                    If rows("jumlah_kerja") > 0 Then
                        rows("no_spk_tunda") = notrans
                        rows("no_spk_man_rls") = txtOrang.EditValue
    ...
    Private Sub loadDetail()
        If Not ds1.Tables("spkmDetail") Is Nothing Then ds1.Tables("spkmDetail").Clear()
        da3 = New SqlDataAdapter(New SqlCommand("select d.*, '' nama_barang,'' Nama_Tipe,'' Merk_Kain,'' Nama_Ukuran, 0 sisa from PROD.dbo.PROD_spk_tunda_d  d where 1=0", sqlconn))
        da3.Fill(ds1, "spkmDetail")
        Dim dr As SqlDataReader = New SqlClient.SqlCommand("select P.*,S.kd_jenis,S.Nama_Barang, T.Nama_Tipe, K.Merk_Kain, U.Nama_Ukuran " & _
                                                           ...
                                                           "left join SIF.dbo.SIF_Ukuran U on S.kd_ukuran=U.Kode_Ukuran " & _
                                                           "left join SIF.dbo.SIF_Kain K on S.kd_kain=K.Kode_Kain " & _
                                                           "where P.no_spk='" & txtNoSpk.EditValue & "' and P.kd_kegiatan_spk='" & txtKegiatan.EditValue & "'  " & _
