...
Public Class frmzPmhSpkInsentifBar
    Dim obj As Object
    Dim prn As frmMain
    ...
    Private Sub isiDetil()
        ...
        da1 = New SqlDataAdapter(New SqlCommand("select distinct d.kd_barang,d.no_urut_brg,b.nama_barang,t.nama_tipe,m.merk_kain,u.nama_ukuran,d.jumlah_produksi as jumlah_realisasi,(d.jumlah_produksi- isnull(c.jumlah_kerja,0)) as sisa_rencana_prod,0.00 as jumlah_kerja, d.jumlah_sisa as jumlah_sisa,0.00 as jumlah_rework,0.00 as jumlah_delay,isnull(c.jumlah_kerja,1) as indeks " & _
                        "from PROD.dbo.PROD_spk_d d left join PROD.dbo.PROD_rcn_prod_m x on x.no_rph=d.no_rph " & _
                         "left join(Select SUM(jumlah_kerja)as jumlah_kerja,no_spk_man,kd_kegiatan, kd_kegiatan_spk, " & _
                                                   "kd_barang from prod.dbo.prod_rls_spk_man_d  " & _
                                                   "Group by no_spk_man,kd_kegiatan,kd_kegiatan_spk,kd_barang ) c " & _
                                                  "on (d.no_spk = c.no_spk_man) " & _
                        ...
                        "left join sif.dbo.sif_kain m on b.kd_kain=m.kode_kain " & _
                        "left join sif.dbo.SIF_ukuran u on b.kd_ukuran=u.kode_ukuran " & _
                        "where x.tanggal='" & Format(txtTgl.EditValue, "yyyyMMdd") & "' and d.kd_kegiatan_spk='" & txtKegiatan.EditValue & "'", sqlconn))
        da1.Fill(ds1, "detil")
        ...
        da1 = New SqlDataAdapter(New SqlCommand("select sum(d.jumlah_kerja)as jumlah_kerja,d.no_spk_man,d.kd_kegiatan " & _
                "from PROD.dbo.PROD_rls_spk_man_d d left join PROD.dbo.PROD_spk_d z on d.no_spk_man = z.no_spk and d.kd_barang=z.kd_barang and d.kd_kegiatan=z.kd_kegiatan and d.kd_kegiatan_spk=z.kd_kegiatan_spk and d.no_urut=z.no_urut_brg  " & _
                "left join PROD.dbo.PROD_rcn_prod_m x on x.no_rph=z.no_rph " & _
                "left join SIF.dbo.Sif_barang b on d.kd_barang=b.kode_barang " & _
                "left join sif.dbo.sif_tipe t on b.kd_tipe=t.kode_tipe " & _
                "left join sif.dbo.sif_kain m on b.kd_kain=m.kode_kain " & _
                "left join sif.dbo.SIF_ukuran u on b.kd_ukuran=u.kode_ukuran " & _
                "where x.tanggal='" & Format(txtTgl.EditValue, "yyyyMMdd") & "' and z.kd_kegiatan_spk='" & txtKegiatan.EditValue & "' group by d.kd_kegiatan,d.no_spk_man", sqlconn))
    ...
    Private Sub txtNoSpk_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtKegiatan.EditValueChanged
        If sender.EditValue <> Nothing Then
            If State = "ADD" Then
                ...
                da1 = New SqlDataAdapter(New SqlCommand("Select subquery1.Id_Data, subquery1.Desc_Data from " & _
                                                        ...
                                                        "left join PROD.dbo.PROD_rcn_prod_m x on x.no_rph=spk.no_rph " & _
                                                        ...
                                                        "where x.tanggal='" & Format(txtTgl.EditValue, "yyyyMMdd") & "' and reff.Id_Ref_Data='KGTSPK' " & _
                                                        "group by  spk.kd_kegiatan_spk,spk.kd_kegiatan, reff.Desc_Data, a.jumlah_kerja) subquery1 " & _
            ...
            ElseIf State = "EDIT" Then
                ...
                da1 = New SqlDataAdapter(New SqlCommand("select distinct(spk.kd_kegiatan) Id_Data, reff.Desc_Data from SIF.dbo.SIF_Gen_Reff_D reff  " & _
                                                        "left join PROD.dbo.PROD_spk_m spk on reff.Id_Data=spk.kd_kegiatan left join PROD.dbo.PROD_rcn_prod_m x on x.no_rph=spk.no_rph  " & _
                                                        "where x.tanggal='" & Format(txtTgl.EditValue, "yyyyMMdd") & "' and reff.Id_Ref_Data='KGTSPK'", sqlconn))
                ...
                'da1 = New SqlDataAdapter(New SqlCommand("select * from SIF.dbo.SIF_Gen_Reff_D reff  " & _
                '        "left join PROD.dbo.PROD_spk_m spk on reff.Id_Data=spk.kd_kegiatan_spk left join PROD.dbo.PROD_rcn_prod_m x on x.no_rph=spk.no_rph " & _
                '        "where x.tanggal='" & Format(txtTgl.EditValue, "yyyyMMdd") & "'  and reff.id_ref_file='SPK' ", sqlconn))
    ...
    Public Sub edit_click()
        ...
        If Not frmCari.row Is Nothing Then
            ...
            If Not ds1.Tables("spkmDetil") Is Nothing Then ds1.Tables("spkmDetil").Clear()
            da3 = New SqlDataAdapter(New SqlCommand("select *, '' nama_barang,'' nama_tipe,'' merk_kain,'' nama_ukuran,0 as indeks,0 as sisa_rencana_prod  from PROD.dbo.PROD_rls_spk_man_d  where  no_spk_man_rls='" & frmCari.row("No_SPK_Rls") & "' order by kd_barang", sqlconn))
            ...
            For Each rw As DataRow In ds1.Tables("spkmDetil").Rows
                ...
                "Left join " & _
                "(select s.kd_kegiatan,s.no_spk,s.kd_barang,s.no_urut_brg,s.jumlah_produksi as jumlah_produksi,(s.jumlah_produksi - isnull(c.jumlah_kerja,0)) as sisa_rencana_prod " & _
                    "from PROD.dbo.PROD_spk_d s left join PROD.dbo.PROD_rcn_prod_m x on x.no_rph=s.no_rph " & _
                    "left join(Select SUM(jumlah_kerja)as jumlah_kerja,no_spk_man,kd_kegiatan, " & _
                    "kd_barang from prod.dbo.prod_rls_spk_man_d " & _
                    "Group by no_spk_man,kd_kegiatan,kd_barang) c " & _
                     "on (s.no_spk = c.no_spk_man) " & _
                     ...
                     "left join sif.dbo.sif_kain m on b.kd_kain=m.kode_kain " & _
                     "left join sif.dbo.SIF_ukuran u on b.kd_ukuran=u.kode_ukuran " & _
                     "where x.tanggal='" & Format(txtTgl.EditValue, "yyyyMMdd") & "' and  no_spk_man_rls='" & frmCari.row.Item("No_SPK_Rls") & "'", sqlconn).ExecuteReader
    ...
    Public Sub save_click()
        ...
        If State = "ADD" Then
            ...
            dread = New SqlClient.SqlCommand("select distinct(no_rph) from PROD.dbo.PROD_spk_m left join PROD.dbo.PROD_rcn_prod_m x on x.no_rph=m.no_rph where tanggal='" & Format(txtTgl.EditValue, "yyyyMMdd") & "'", sqlconn).ExecuteReader
            dread.Read()
    ...
    Private Sub loadKapasitas()
        ...
        da1 = New SqlDataAdapter(New SqlCommand("select * " & _
                                                ...
                                                "where " & _
                                                "kd_kegiatan = '" & txtKegiatan.EditValue & "'", sqlconn))
    ...
    Private Sub loadDetail()
        ...
        Dim dr As SqlDataReader = New SqlClient.SqlCommand("select P.*,S.kd_jenis,S.Nama_Barang, T.Nama_Tipe, K.Merk_Kain, U.Nama_Ukuran " & _
                                                           "from PROD.dbo.PROD_spk_man_d P left join PROD.dbo.PROD_rcn_prod_m x on x.no_rph=P.no_rph " & _
                                                           ...
                                                           "where x.tanggal='" & Format(txtTgl.EditValue, "yyyyMMdd") & "' and  P.kd_kegiatan_spk='" & txtKegiatan.EditValue & "' " & _
    ...
    Private Sub gv_CellValueChanged(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles gv.CellValueChanged
        ...
        If State = "EDIT" Then
            If e.Column Is jumlah_kerja Then
                ...
                da1 = New SqlDataAdapter(New SqlCommand("select * from PROD.dbo.PROD_rls_spk_man_d where  no_spk_man_rls= '" & txtNoSpkRls.Text & "'  ", sqlconn)) '<- query buat ditampilkan di LOV
                da1.Fill(ds4, "cek1")
            ...
            If e.Column Is jumlah_kerja Then
                ...
                da4 = New SqlDataAdapter(New SqlCommand("select a.no_rph no_rph, a.no_spk, a.kd_kegiatan, a.kd_kegiatan_spk, " & _
                                                " a.kd_barang kd_barang, a.jumlah_produksi  as jumlah_produksi,isnull(b.jumlah_rework,0) as jumlah_rework, " & _
                                                 ...
                                                 "from prod.dbo.prod_spk_d a left join PROD.dbo.PROD_rcn_prod_m x on x.no_rph=a.no_rph " & _
                                                 "left join(Select SUM(jumlah_kerja)as jumlah_kerja,no_spk_man,kd_kegiatan,jumlah_rework, " & _
                                                 "kd_barang from prod.dbo.prod_rls_spk_man_d " & _
                                                 "Group by no_spk_man,kd_kegiatan,kd_barang,jumlah_rework) b " & _
                                                 "on (a.no_spk = b.no_spk_man) " & _
                                                 ...
                                                 "and (a.kd_barang = b.kd_barang) " & _
                                                  "INNER JOIN SIF.dbo.SIF_Barang br " & _
                                                  "on br.Kode_Barang = a.kd_barang " & _
                                                 "where x.tanggal='" & Format(txtTgl.EditValue, "yyyyMMdd") & "' " & _
                                                            "and a.kd_kegiatan_spk= '" & txtKegiatan.EditValue & "' ", sqlconn))
    ...
    Private Sub txtTgl_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtTgl.EditValueChanged
        If Not ds1.Tables("jnsKeg") Is Nothing Then ds1.Tables("jnsKeg").Clear()
        da1 = New SqlDataAdapter(New SqlCommand("select distinct b.kd_kegiatan_spk Id_Data,a.Desc_Data Desc_Data " & _
                                                "from PROD.dbo.PROD_SPK_D b " & _
                                                "inner join  SIF.dbo.SIF_Gen_Reff_D  a  on b.kd_kegiatan_spk=a.Id_data " & _
                                                "inner join  PROD.dbo.PROD_rcn_prod_m x on x.no_rph=b.no_rph " & _
                                                "where a.id_ref_file='SPK' and x.tanggal='" & Format(txtTgl.EditValue, "yyyyMMdd") & "'", sqlconn))
        da1.Fill(ds1, "jnsKeg")
        txtKegiatan.Properties.DataSource = ds1.Tables("jnsKeg")
    ...
    Private Sub txbBarcode_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txbBarcode.KeyPress
        ...
        If e.KeyChar = Chr(13) Then
            ...
            Dim dtrow() As DataRow = ds1.Tables("detil").Select("kd_barang='" & txbBarcode.Text & "' ")
            'MsgBox("ketemu")
            ...
            If dtrow.Count <> 0 Then
                ...
                For i As Int16 = 0 To dtrow.Count - 1
                    ...
                    If dtrow(i)("jumlah_kerja") <= dtrow(i)("jumlah_realisasi") Then  ' jika sisa masih ada maka ble terus transaksi barcode

                        dtrow(i)("jumlah_kerja") = dtrow(i)("jumlah_kerja") + 1
                        lblnmbarang.Text = dtrow(i)("nama_barang")
                       
                        ok = True
                        BindingContext(gc.DataSource).Position = getPosisi(dtrow(i))
                    ...
                    Else
                        gv.SetColumnError(jumlah_kerja, "Jumlah Dikerjakan tidak boleh lebih besar dari Jumlah Produksi", DevExpress.XtraEditors.DXErrorProvider.ErrorType.Critical)
                        showMessages("Data kelebihan:" & dtrow(i)("jumlah_kerja") - dtrow(i)("jumlah_realisasi"))
            ...
            Else
                lblnmbarang.Text = "Item tidak ada dalam Daftar"
    ...
    Function getPosisi(ByVal dr As DataRow) As Integer
        Dim i As Int16 = 0
        For Each rw As DataRow In ds1.Tables("detil").Rows
            If dr Is rw Then
                Return i
            Else
                i += 1
    ...
    Private Sub txbBarcode_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txbBarcode.TextChanged
