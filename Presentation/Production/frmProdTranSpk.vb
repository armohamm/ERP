...
Public Class frmProdTranSpk
    ...
    Public Sub edit_click()
        ...
        matilookup()
        dsedit = New DataSet
        'daedit = New SqlDataAdapter(New SqlCommand("select distinct a.no_rph as [NO RPH],a.no_spk as [NO SPK],a.kd_kegiatan_spk as [KODE SPK], " & _
        ...
        '                                            "and a.kd_departemen='" & kddep & "' and (m.sts_centi = 'N' or m.sts_centi is NULL )", sqlconn)) '<- query buat ditampilkan di LOV
        daedit = New SqlDataAdapter(New SqlCommand("select distinct a.no_rph as [NO_RPH],m.tanggal as [TANGGAL],a.no_spk as [NO_SPK],a.kd_kegiatan as [KODE_KEGIATAN],c.desc_data as [NAMA_KEGIATAN],a.kd_kegiatan_spk as [KODE_SPK], " & _
                                                   "b.desc_data as [NAMA_SPK] from PROD.dbo.PROD_spk_d a  " & _
                                                   "left join PROD.dbo.PROD_spk_m m on " & _
                                                   "m.no_spk = a.no_spk And m.kd_kegiatan = a.kd_kegiatan And m.kd_kegiatan_spk = a.kd_kegiatan_spk " & _
                                                   "and m.no_rph = a.no_rph " & _
                                                   "LEFT Join SIF.dbo.SIF_Gen_Reff_D b on a.kd_kegiatan_spk=b.id_data " & _
                                                   "LEFT Join SIF.dbo.SIF_Gen_Reff_D c on a.kd_kegiatan=c.Id_Data " & _
                                                   "where b.ref_role='PROD' and b.id_Ref_File='SPK' and c.Ref_Role='PROD' and c.Id_Ref_File='KGTSPK' " & _
                                                   "and a.kd_departemen='" & kddep & "' and (m.sts_centi = 'N' or m.sts_centi is NULL )", sqlconn)) '<- query buat ditampilkan di LOV
        ...
        If Not frmCari.row Is Nothing Then
            kode = frmCari.row("NO_RPH").ToString
            kode1 = frmCari.row("NO_SPK").ToString
            kode2 = frmCari.row("KODE_KEGIATAN").ToString
            kode3 = frmCari.row("KODE_SPK").ToString
            ...
            da1 = New SqlDataAdapter(New SqlCommand("select desc_data from PROD.dbo.PROD_rcn_prod_m a,PROD.dbo.PROD_spk_m b,SIF.dbo.SIF_Gen_Reff_D c where b.no_spk='" & kode1 & "' and a.no_rph=b.no_rph and a.kd_jadwal=c.id_data and c.ref_role='PROD' and c.id_Ref_File='JDWKRJ'", sqlconn))
            da1.Fill(ds1, "isispk")
            ...
            da2 = New SqlDataAdapter(New SqlCommand("select distinct kd_kegiatan,desc_data from PROD.dbo.PROD_spk_m a,SIF.dbo.SIF_Gen_Reff_D b where a.no_spk='" & kode1 & "' and a.no_rph='" & kode & "'and b.ref_role='PROD' and b.id_Ref_File='KGTSPK'and a.kd_kegiatan=b.id_data  ", sqlconn))
            ...
            da3 = New SqlDataAdapter(New SqlCommand("select distinct kd_kegiatan_spk,desc_data from PROD.dbo.PROD_spk_m a,SIF.dbo.SIF_Gen_Reff_D b where no_spk='" & kode1 & "' and a.kd_kegiatan='" & kode2 & "' and a.no_rph='" & kode & "' and b.ref_role='PROD' and b.id_Ref_File='SPK'and a.kd_kegiatan_spk=b.id_data ", sqlconn))
    ...
    Public Sub cetak_click()
        ...
        callReport(App_Path() & "\report\Reportspkq1.rpt", "", "&norph=" & lkrph.EditValue & " &nospk=" & lkspk.EditValue & " &kdkegiatan=" & lkkegiatan.EditValue & " &kapasitas=" & kapasits.Text & " &kdkegiatanspk=" & lkkegtn.EditValue & " &vnama_report=" & TEMPLATE, False)
    ...
    Private Sub lkkegtn_EditValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles lkkegtn.EditValueChanged
        ...
        da3 = New SqlDataAdapter(New SqlCommand("select jumlah,kapasitas from PROD.dbo.PROD_spk_m a,SIF.dbo.SIF_Gen_Reff_D b where no_spk='" & lkspk.EditValue & "' and a.kd_kegiatan='" & lkkegiatan.EditValue & "' and a.no_rph='" & lkrph.EditValue & "' and b.ref_role='PROD' and b.id_Ref_File='SPK' and a.kd_kegiatan_spk=b.id_data and a.kd_kegiatan_spk='" & lkkegtn.EditValue & "'", sqlconn))
        ...
        "and a.kd_kegiatan_spk= '" & lkkegtn.EditValue & "' and a.no_rph='" & lkrph.EditValue & "' Order by a.kd_barang ", sqlconn))
        da4.Fill(ds4, "loada")
        ...
        If ds4.Tables("loada").Rows.Count = 0 Then
            ...
            da4 = New SqlDataAdapter(New SqlCommand("select distinct a.kd_barang,m.Nama_Merk,t.Nama_Tipe, u.Nama_Ukuran, k.Merk_Kain,floor(a.jumlah_produksi) as jumlah_produksi,a.jumlah_kerja as jumlah_kerja,a.no_spk,a.kd_kegiatan_spk,a.kd_kegiatan,a.no_rph,br.kd_ukuran,br.kd_tipe,  floor(1 * (ISNULL(a.jumlah_produksi,0))) as indeks " & _
            ", (Select SUBSTRING(( Select ' ' + so.Keterangan  + '/' + CONVERT(varchar(10),FLOOR(d.jumlah))+'*' " & _
            "from PROD.dbo.PROD_rcn_prod_b S1  " & _
            "inner join PROD.dbo.PROD_rcn_prod_d d  " & _
            "on d.no_rph = s1.no_rph and d.kd_barang = s1.kd_barang  " & _
            "inner join sales.dbo.SALES_SO_D so " & _
            "on so.Kd_Stok = d.kd_barang and so.No_sp = d.no_sp and so.no_seq = d.no_sp_dtl  " & _
            "where(s1.kd_barang = a.kd_barang) " & _
            "and S1.no_rph = '" & lkrph.EditValue & "' " & _
            "For XML PATH ('')),2,500)) as ket " & _
            "from PROD.dbo.PROD_spk_d a " & _
            "LEFT JOIN SIF.dbo.SIF_Barang br " & _
            "on a.kd_barang = br.Kode_Barang " & _
            "LEFT JOIN SIF.dbo.SIF_Tipe t " & _
            "ON t.Kode_Tipe = br.kd_tipe " & _
            "LEFT JOIN SIF.dbo.SIF_Ukuran u " & _
            "ON u.Kode_Ukuran = br.kd_ukuran " & _
            "LEFT JOIN SIF.dbo.SIF_Kain k " & _
            "ON k.Kode_Kain = br.kd_kain " & _
            "LEFT JOIN SIF.dbo.SIF_Merk m " & _
            "ON m.Kode_Merk = br.kd_merk " & _
            "where a.no_spk='" & lkspk.EditValue & "' and a.kd_kegiatan='" & lkkegiatan.EditValue & "' " & _
            "and a.kd_kegiatan_spk= '" & lkkegtn.EditValue & "' and a.no_rph='" & lkrph.EditValue & "' Order by a.kd_barang,br.kd_tipe", sqlconn))
            da4.Fill(ds4, "load")
            gControl.DataSource = ds4.Tables("load")
        Else
            x = 1
            ...
            'da4 = New SqlDataAdapter(New SqlCommand("select distinct  a.*,br.kd_ukuran,m.Nama_Merk, t.Nama_Tipe, k.Merk_Kain, u.Nama_Ukuran, isnull((b.nilai_konv * a.jumlah_produksi),1) indeks from PROD.dbo.PROD_spk_d a " & _
            da4 = New SqlDataAdapter(New SqlCommand("select distinct a.kd_barang,m.Nama_Merk,t.Nama_Tipe, u.Nama_Ukuran, k.Merk_Kain,floor(a.jumlah_produksi) as jumlah_produksi,a.jumlah_kerja as jumlah_kerja,a.no_spk,a.kd_kegiatan_spk,a.kd_kegiatan,a.no_rph,br.kd_ukuran,br.kd_tipe, floor((isnull(b.nilai_konv,1)) * (ISNULL(a.jumlah_produksi,0))) as indeks " & _
            ", (Select SUBSTRING(( Select ' ' + so.Keterangan  + '/' + CONVERT(varchar(10),FLOOR(d.jumlah))+'*' " & _
            "from PROD.dbo.PROD_rcn_prod_b S1  " & _
            "inner join PROD.dbo.PROD_rcn_prod_d d  " & _
            "on d.no_rph = s1.no_rph and d.kd_barang = s1.kd_barang  " & _
