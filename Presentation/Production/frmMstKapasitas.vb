...
Public Class frmMstKapasitas
    ...
    End Sub
    Public Sub view()
        daKapasitas = New SqlDataAdapter(New SqlCommand("select * from SIF.dbo.SIF_kapasitas", sqlconn))
        dsKapasitas.Reset()
    ...
    Private Sub frmMstKapasitas_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        daPPIC = New SqlDataAdapter(New SqlCommand("select id_data, desc_data from SIF_Gen_Reff_D where ref_role = 'PROD' and id_ref_file = 'KGTPPIC'", sqlconn))
        daPPIC.Fill(dsPPIC, "jnsKeg")
        daKapasitas = New SqlDataAdapter(New SqlCommand("select * from SIF.dbo.SIF_kapasitas", sqlconn)) 'where jns_kegiatan ='" & txtkegiatanppic.EditValue & "'", sqlconn))
        daKapasitas.Fill(dsKapasitas, "Kapasitas")
        daJenis = New SqlDataAdapter(New SqlCommand("select id_data kd_jns_brg, desc_data nama_jenis FROM SIF.dbo.SIF_Gen_Reff_D where ref_role = 'SIF' and id_ref_file = 'GRPJNS' ", sqlconn))
        daJenis.Fill(dsJenis, "jnsBrg")

        gControl.DataSource = dsKapasitas.Tables("Kapasitas")

        cbPPIC.DataSource = dsPPIC.Tables("jnsKeg")
        cbPPIC.ValueMember = "id_data"
        cbPPIC.DisplayMember = "desc_data"

        cbJenis.DataSource = dsJenis.Tables("jnsBrg")
        cbJenis.ValueMember = "kd_jns_brg"
    ...
    Private Sub gView_ValidateRow(ByVal sender As System.Object, ByVal e As DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs) Handles gView.ValidateRow
        ...
        If e.Row.item("kegiatan_ppic").ToString = "" Then
            isValid = False
            gView.SetColumnError(kegiatan_ppic, "Kolom 'Kegiatan PPIC' harus diisi.", DevExpress.XtraEditors.DXErrorProvider.ErrorType.Critical)
        End If
        If e.Row.item("kd_jenis").ToString = "" Then
            isValid = False
            gView.SetColumnError(kd_jenis, "Kolom 'Jenis Barang' harus diisi.", DevExpress.XtraEditors.DXErrorProvider.ErrorType.Critical)
        End If
    ...
    Public Sub cetak_click()
        ...
        print_gc1(gControl, "                                                                       MASTER KAPASITAS PRODUKSI    " & vbCrLf & _
                 "     " & vbCrLf)
