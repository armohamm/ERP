...
Public Class frmSetParamSPK
    ...
    Public Sub add_click()
        ...
        Dim myBuilder As New SqlClient.SqlCommandBuilder
        dSO = New DataSet
        'DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select *, 1 as state from sif.dbo.SIF_Gen_Reff order by Ref_Role", sqlconn)) '<- query buat ditampilkan di LOV, pakailah alias colom biar enak dibaca
        'DA.Fill(dSO, "M")
        'GC.DataSource = dSO.Tables("M")
        DAD = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select d.*, 1 as state from sif.dbo.SIF_Gen_Reff_D d " & _
                                                                    "where d.Ref_Role = 'PROD' and d.Id_Ref_File = 'KGTSPK'", sqlconn)) '<- query buat ditampilkan di LOV, pakailah alias colom biar enak dibaca
        DAD.Fill(dSO, "D")
        ...
        If Not dSO1.Tables("Departemen") Is Nothing Then dSO1.Tables("Departemen").Clear()
        DA = New SqlDataAdapter(New SqlCommand("select Kd_Departemen, Nama_Departemen from SIF.dbo.SIF_Departemen ", sqlconn))
        DA.Fill(dSO1, "Departemen")
        Rep_Departemen.DataSource = dSO1.Tables("Departemen")
    ...
    Public Sub cancel_click()
        'dSO.Tables("M").Clear()
        dSO.Tables("D").Clear()
        'DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select *, 1 as state from sif.dbo.SIF_Gen_Reff", sqlconn)) '<- query buat ditampilkan di LOV, pakailah alias colom biar enak dibaca
        'DA.Fill(dSO, "M")
        DAD = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select d.*, 1 as state from sif.dbo.SIF_Gen_Reff_D d " & _
                                                                    "where d.Ref_Role = 'PROD' and d.Id_Ref_File = 'KGTSPK'", sqlconn)) '<- query buat ditampilkan di LOV, pakailah alias colom biar enak dibaca
        DAD.Fill(dSO, "D")
    ...
    Public Sub edit_click()
        ...
        'dSO = New DataSet
        'DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select * from sif.dbo.SIF_Gen_Reff", sqlconn)) '<- query buat ditampilkan di LOV, pakailah alias colom biar enak dibaca
        'DA.Fill(dSO, "M")
        'default_add(prn)
        btnDelete = True
        setStatus()
    ...
    Private Sub kosong_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        GVD.ViewCaption = "Parameter Area Kegiatan SPK"
    ...
    Private Sub GVD_InitNewRow(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Grid.InitNewRowEventArgs) Handles GVD.InitNewRow
        ...
        row("Id_Ref_File") = "KGTSPK"
        row("Id_Ref_Data") = "KGTSPK"
        row("Ref_Role") = "PROD"
