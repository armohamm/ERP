...
Public Class frmMonDPB
    ...
    Private Sub frmMonDPB_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub reload()
        ...
        da1 = New SqlDataAdapter(New SqlCommand("select  t.tanggal [Tanggal], t.no_trans [No Transaksi] " & _
                                                ...
                                                "inner join PROD.dbo.PROD_dpb_m d " & _
    ...
    Sub LoadDataBarang()
        ...
        ElseIf State = "EDIT" Then
            ...
            da2.Fill(ds1, "Barang")
            da2 = New SqlDataAdapter(New SqlCommand("select distinct t.no_sp, t.no_trans, t.no_sp_dtl, t.jumlah,sd.Qty_kirim,sj.no_sj," & _
                                                    ...
                                                    "LEFT JOIN SALES.dbo.SALES_SJ sj " & _
                                                    "on sj.no_dpb = t.no_trans " & _
                                                    "LEFT JOIN SALES.dbo.SALES_SJ_D sd " & _
                                                    "on sd.no_sj = sj.no_sj and t.no_sp = sd.No_sp and t.no_sp_dtl = sd.no_seq " & _
    ...
    Private Sub txtNoTrans_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtNoTrans.EditValueChanged
        ...
        dr = New SqlClient.SqlCommand("select * from PROD.dbo.PROD_rcn_krm_m where no_trans='" & txtNoTrans.EditValue & "' ", sqlconn).ExecuteReader
        dr.Read()
        TextEdit1.Text = txtNoTrans.EditValue
        txtKendaraan.EditValue = dr.Item("kd_kendaraan")
        txtSopir.EditValue = dr.Item("kd_sopir")
        txtKernet.EditValue = dr.Item("kd_kenek")
        dr.Close()
    ...
    Public Sub cetak_click()
        ...
        Try
            ...
            Else
                callReport(App_Path() & "\report\PROD_DPB(Rekap).rpt", "", "&no_kirim=" & txtNoTrans.EditValue)
            End If
