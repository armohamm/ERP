...
Public Class aFormLunasPiutangUsahaNonDO
    ...
    Public Sub edit_click()
        ...
        If Not frmCari.row Is Nothing Then
            ...
            lookBank.EditValue = dsEdit.Tables("NotaLunasCari").Rows(0).Item("kd_bank")
            btnSave = True
            btnCancel = True
            btnadd = False
            btnEdit = False
    ...
    Private Sub v_nota()
        ...
        daNota = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select * from FIN.dbo.FIN_Nota where kd_cust='" & lookcust.EditValue & "' and kd_valuta='" & LEValuta.EditValue & "' and tipe_trans = 'JPJ-KPT-02' and jml_akhir>0 and not (len(no_posting)=0 or no_posting is null or no_posting='')", sqlconn))
    ...
    Private Sub V_Bank()
        If Not dsNotaLunas.Tables("View_Bank") Is Nothing Then dsNotaLunas.Tables("View_Bank").Clear()
        daRek = New SqlDataAdapter(New SqlCommand("select kd_bank, nama_bank from SIF.dbo.SIF_Bank", sqlconn))
        daRek.Fill(dsNotaLunas, "View_Bank")
        lookBank.Properties.DataSource = dsNotaLunas.Tables("View_Bank")
    ...
    Private Sub V_giro()
        ...
        If lookcust.EditValue = vbNullString Then
            ...
            daGiro = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select nomor, keterangan,tgl_jth_tempo, jml_trans from FIN.dbo.FIN_GIRO where jns_giro='" & lookbayar.EditValue & "' and kd_valuta='" & LEValuta.EditValue & "' and jns_trans='JUAL' and tipe_trans='JRR-KPT-10' and status='CLEARING' and nomor not in (select no_giro from FIN.dbo.FIN_NOTA_LUNAS) AND NOT (no_jur IS NULL)", sqlconn))
        Else
            Lookgiro.Enabled = True
            daGiro = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select nomor, keterangan,tgl_jth_tempo, jml_trans from FIN.dbo.FIN_GIRO where kartu='" & lookcust.EditValue.ToString & "' and jns_giro='" & lookbayar.EditValue.ToString & "' and kd_valuta='" & LEValuta.EditValue.ToString & "' and jns_trans='JUAL' and tipe_trans='JRR-KPT-10' and status='CLEARING' and nomor not in (select distinct isnull(no_giro,'kosong') from FIN.dbo.FIN_NOTA_LUNAS) AND NOT (no_jur IS NULL)", sqlconn))
        End If
    ...
    Public Sub save_click(Optional ByRef isDelete As Boolean = False)
        ...
        If Not isDelete Then
            If STATE = "ADD" Then
                ...
                row.Item("kd_bank") = lookBank.EditValue
    ...
    Private Sub aFormLunasPiutangUsahaNonDO_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        connect()
        prn = getParent(Me)
        View_Valuta()
        V_Customer()
    ...
    Private Sub GV_ValidateRow(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs) Handles GV.ValidateRow
        Try
            ...
            If view.GetRowCellValue(e.RowHandle, column3) > view.GetRowCellValue(e.RowHandle, column1) Then
                ...
                view.SetColumnError(column3, "Penambahan jumlah lunas terlalu banyak,tidak boleh melebihi tagihan.")
                'End If
                'If view.FocusedColumn.FieldName = "jml_piut" Or view.FocusedColumn.FieldName = "jml_biaya1" Or view.FocusedColumn.FieldName = "jml_pembulatan" Or view.FocusedColumn.FieldName = "keterangan" Then
