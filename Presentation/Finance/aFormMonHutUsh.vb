...
Public Class aFormMonHutUsh
    ...
    Private Sub loadLookUp()
        ...
        daLookKar = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT Nama_Supplier, Kode_Supplier FROM SIF.dbo.SIF_Supplier WHERE rec_stat = 'Y' UNION ALL SELECT '[Semua Data]', '000000'", sqlconn))
        daLookKar.Fill(dsLookUp, "LookKar")
        lueKaryawan.Properties.DataSource = dsLookUp.Tables("LookKar")
        lueKaryawan.Properties.DisplayMember = "Nama_Supplier"
        lueKaryawan.Properties.ValueMember = "Kode_Supplier"

        daLookKar = New SqlDataAdapter(New SqlCommand("select Kode_Valuta, Nama_Valuta from SIF.dbo.SIF_Valuta", sqlconn))
        daLookKar.Fill(dsLookUp, "Valuta")
    ...
    Private Sub loadPiutang()
        ...
        If chkAll.Checked Then
            If chkPosting.Checked Then
                If lueKaryawan.EditValue = "000000" Or lueKaryawan.EditValue = vbNullString Then
                    daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_supplier) as 'nomer', kd_supplier, SUM(jml_tagihan) jml_tagihan, SUM(jml_bayar) jml_bayar, SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_PEMBELIAN WHERE kd_valuta = '" & lckValuta.EditValue & "' AND tipe_trans IN ('JPP-KUT-01','JPP-KUT-02') AND tgl_posting BETWEEN CONVERT(DATE,'" & deTglAwal.EditValue & "',103) AND CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY kd_supplier", sqlconn))
                Else
                    daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_supplier) as 'nomer', kd_supplier, SUM(jml_tagihan) jml_tagihan, SUM(jml_bayar) jml_bayar, SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_PEMBELIAN WHERE kd_valuta = '" & lckValuta.EditValue & "' AND tipe_trans IN ('JPP-KUT-01','JPP-KUT-02') AND kd_supplier = '" & lueKaryawan.EditValue & "' AND tgl_posting BETWEEN CONVERT(DATE,'" & deTglAwal.EditValue & "',103) AND CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY kd_supplier", sqlconn))
                End If
            Else
                If lueKaryawan.EditValue = "000000" Or lueKaryawan.EditValue = vbNullString Then
                    daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY k.kd_supplier) as 'nomer', k.kd_supplier, SUM(k.jml_tagihan) jml_tagihan, SUM(k.jml_bayar) jml_bayar, SUM(k.sisa) sisa FROM (SELECT DISTINCT a.no_inv, a.kd_supplier, SUM(a.jml_tagihan) jml_tagihan, (SELECT ISNULL(SUM(x.jml_bayar),0) jmlbayar FROM FIN.dbo.FIN_BELI_LUNAS_D x, FIN.dbo.FIN_PEMBELIAN y, FIN.dbo.FIN_BELI_LUNAS z WHERE x.prev_no_inv = y.no_inv AND x.no_trans = z.no_trans AND z.kd_kartu = a.kd_supplier AND x.prev_no_inv = a.no_inv AND (z.no_posting IS NULL OR z.no_posting = '')) jml_bayar, SUM(a.jml_tagihan) - (SELECT ISNULL(SUM(x.jml_bayar),0) jmlbayar FROM FIN.dbo.FIN_BELI_LUNAS_D x, FIN.dbo.FIN_PEMBELIAN y, FIN.dbo.FIN_BELI_LUNAS z WHERE x.prev_no_inv = y.no_inv AND x.no_trans = z.no_trans AND z.kd_kartu = a.kd_supplier AND x.prev_no_inv = a.no_inv AND (z.no_posting IS NULL OR z.no_posting = '')) 'sisa' FROM FIN.dbo.FIN_PEMBELIAN a WHERE (a.no_posting IS NULL OR a.no_posting = '') AND a.kd_valuta = '" & lckValuta.EditValue & "' AND (a.tipe_trans LIKE 'JPP-KUT-01' OR a.tipe_trans LIKE 'JPP-KUT-02') AND a.tgl_inv BETWEEN CONVERT(DATE,'" & deTglAwal.EditValue & "',103) AND CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) GROUP BY a.no_inv, a.kd_supplier) k GROUP BY k.kd_supplier", sqlconn))
                Else
                    daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY k.kd_supplier) as 'nomer', k.kd_supplier, SUM(k.jml_tagihan) jml_tagihan, SUM(k.jml_bayar) jml_bayar, SUM(k.sisa) sisa FROM (SELECT DISTINCT a.no_inv, a.kd_supplier, SUM(a.jml_tagihan) jml_tagihan, (SELECT ISNULL(SUM(x.jml_bayar),0) jmlbayar FROM FIN.dbo.FIN_BELI_LUNAS_D x, FIN.dbo.FIN_PEMBELIAN y, FIN.dbo.FIN_BELI_LUNAS z WHERE x.prev_no_inv = y.no_inv AND x.no_trans = z.no_trans AND z.kd_kartu = a.kd_supplier AND x.prev_no_inv = a.no_inv AND (z.no_posting IS NULL OR z.no_posting = '')) jml_bayar, SUM(a.jml_tagihan) - (SELECT ISNULL(SUM(x.jml_bayar),0) jmlbayar FROM FIN.dbo.FIN_BELI_LUNAS_D x, FIN.dbo.FIN_PEMBELIAN y, FIN.dbo.FIN_BELI_LUNAS z WHERE x.prev_no_inv = y.no_inv AND x.no_trans = z.no_trans AND z.kd_kartu = a.kd_supplier AND x.prev_no_inv = a.no_inv AND (z.no_posting IS NULL OR z.no_posting = '')) 'sisa' FROM FIN.dbo.FIN_PEMBELIAN a WHERE (a.no_posting IS NULL OR a.no_posting = '') AND a.kd_valuta = '" & lckValuta.EditValue & "' AND (a.tipe_trans LIKE 'JPP-KUT-01' OR a.tipe_trans LIKE 'JPP-KUT-02') AND a.tgl_inv BETWEEN CONVERT(DATE,'" & deTglAwal.EditValue & "',103) AND CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND a.kd_supplier = '" & lueKaryawan.EditValue & "' GROUP BY a.no_inv, a.kd_supplier) k GROUP BY k.kd_supplier", sqlconn))
                End If
            End If
        Else
            If chkPosting.Checked Then
                If lueKaryawan.EditValue = "000000" Or lueKaryawan.EditValue = vbNullString Then
                    daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_supplier) as 'nomer', kd_supplier, SUM(jml_tagihan) jml_tagihan, SUM(jml_bayar) jml_bayar, SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_PEMBELIAN WHERE kd_valuta = '" & lckValuta.EditValue & "' AND tipe_trans IN ('JPP-KUT-01','JPP-KUT-02') AND jml_akhir > 0 AND tgl_posting BETWEEN CONVERT(DATE,'" & deTglAwal.EditValue & "',103) AND CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY kd_supplier", sqlconn))
                Else
                    daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_supplier) as 'nomer', kd_supplier, SUM(jml_tagihan) jml_tagihan, SUM(jml_bayar) jml_bayar, SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_PEMBELIAN WHERE kd_valuta = '" & lckValuta.EditValue & "' AND tipe_trans IN ('JPP-KUT-01','JPP-KUT-02') AND jml_akhir > 0 AND kd_supplier = '" & lueKaryawan.EditValue & "' AND tgl_posting BETWEEN CONVERT(DATE,'" & deTglAwal.EditValue & "',103) AND CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY kd_supplier", sqlconn))
                End If
            Else
                If lueKaryawan.EditValue = "000000" Or lueKaryawan.EditValue = vbNullString Then
                    daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY k.kd_supplier) as 'nomer', k.kd_supplier, SUM(k.jml_tagihan) jml_tagihan, SUM(k.jml_bayar) jml_bayar, SUM(k.sisa) sisa FROM (SELECT DISTINCT a.no_inv, a.kd_supplier, SUM(a.jml_tagihan) jml_tagihan, (SELECT ISNULL(SUM(x.jml_bayar),0) jmlbayar FROM FIN.dbo.FIN_BELI_LUNAS_D x, FIN.dbo.FIN_PEMBELIAN y, FIN.dbo.FIN_BELI_LUNAS z WHERE x.prev_no_inv = y.no_inv AND x.no_trans = z.no_trans AND z.kd_kartu = a.kd_supplier AND x.prev_no_inv = a.no_inv AND (z.no_posting IS NULL OR z.no_posting = '')) jml_bayar, SUM(a.jml_tagihan) - (SELECT ISNULL(SUM(x.jml_bayar),0) jmlbayar FROM FIN.dbo.FIN_BELI_LUNAS_D x, FIN.dbo.FIN_PEMBELIAN y, FIN.dbo.FIN_BELI_LUNAS z WHERE x.prev_no_inv = y.no_inv AND x.no_trans = z.no_trans AND z.kd_kartu = a.kd_supplier AND x.prev_no_inv = a.no_inv AND (z.no_posting IS NULL OR z.no_posting = '')) 'sisa' FROM FIN.dbo.FIN_PEMBELIAN a WHERE (a.no_posting IS NULL OR a.no_posting = '') AND a.kd_valuta = '" & lckValuta.EditValue & "' AND (a.tipe_trans LIKE 'JPP-KUT-01' OR a.tipe_trans LIKE 'JPP-KUT-02') AND a.jml_akhir > 0 AND a.tgl_inv BETWEEN CONVERT(DATE,'" & deTglAwal.EditValue & "',103) AND CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) GROUP BY a.no_inv, a.kd_supplier) k GROUP BY k.kd_supplier", sqlconn))
                Else
                    daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY k.kd_supplier) as 'nomer', k.kd_supplier, SUM(k.jml_tagihan) jml_tagihan, SUM(k.jml_bayar) jml_bayar, SUM(k.sisa) sisa FROM (SELECT DISTINCT a.no_inv, a.kd_supplier, SUM(a.jml_tagihan) jml_tagihan, (SELECT ISNULL(SUM(x.jml_bayar),0) jmlbayar FROM FIN.dbo.FIN_BELI_LUNAS_D x, FIN.dbo.FIN_PEMBELIAN y, FIN.dbo.FIN_BELI_LUNAS z WHERE x.prev_no_inv = y.no_inv AND x.no_trans = z.no_trans AND z.kd_kartu = a.kd_supplier AND x.prev_no_inv = a.no_inv AND (z.no_posting IS NULL OR z.no_posting = '')) jml_bayar, SUM(a.jml_tagihan) - (SELECT ISNULL(SUM(x.jml_bayar),0) jmlbayar FROM FIN.dbo.FIN_BELI_LUNAS_D x, FIN.dbo.FIN_PEMBELIAN y, FIN.dbo.FIN_BELI_LUNAS z WHERE x.prev_no_inv = y.no_inv AND x.no_trans = z.no_trans AND z.kd_kartu = a.kd_supplier AND x.prev_no_inv = a.no_inv AND (z.no_posting IS NULL OR z.no_posting = '')) 'sisa' FROM FIN.dbo.FIN_PEMBELIAN a WHERE (a.no_posting IS NULL OR a.no_posting = '') AND a.kd_valuta = '" & lckValuta.EditValue & "' AND (a.tipe_trans LIKE 'JPP-KUT-01' OR a.tipe_trans LIKE 'JPP-KUT-02') AND a.jml_akhir > 0 AND a.tgl_inv BETWEEN CONVERT(DATE,'" & deTglAwal.EditValue & "',103) AND CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND a.kd_supplier = '" & lueKaryawan.EditValue & "' GROUP BY a.no_inv, a.kd_supplier) k GROUP BY k.kd_supplier", sqlconn))
                End If
            End If
        End If
    ...
    Private Sub loadDetil(Optional ByVal kodeKar As String = "")
        ...
        If chkAll.Checked Then
            If chkPosting.Checked Then
                daDetilCari = New SqlDataAdapter(New SqlCommand("SELECT no_inv, tgl_inv, no_jurnal, tgl_posting, keterangan, ISNULL(jml_tagihan,0) jml_tagihan, ISNULL(jml_bayar,0) jml_bayar, ISNULL(jml_akhir,0) jml_akhir, kd_supplier FROM FIN.dbo.FIN_PEMBELIAN WHERE (tipe_trans LIKE 'JPP-KUT-01' or tipe_trans LIKE 'JPP-KUT-02') AND kd_supplier='" & kodeKar & "' AND tgl_posting BETWEEN CONVERT(DATE,'" & deTglAwal.EditValue & "',103) AND CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (no_posting IS NULL OR no_posting = '')", sqlconn))
            Else
                daDetilCari = New SqlDataAdapter(New SqlCommand("SELECT a.no_inv, a.tgl_inv, a.no_jurnal, a.tgl_posting, a.keterangan, a.jml_tagihan, (SELECT ISNULL(SUM(x.jml_bayar),0) FROM FIN.dbo.FIN_BELI_LUNAS_D x, FIN.dbo.FIN_PEMBELIAN y, FIN.dbo.FIN_BELI_LUNAS z WHERE x.prev_no_inv = y.no_inv AND x.no_trans = z.no_trans AND z.kd_kartu = a.kd_supplier AND x.prev_no_inv = a.no_inv) jml_bayar, a.jml_tagihan - (SELECT ISNULL(SUM(x.jml_bayar),0) FROM FIN.dbo.FIN_BELI_LUNAS_D x, FIN.dbo.FIN_PEMBELIAN y, FIN.dbo.FIN_BELI_LUNAS z WHERE x.prev_no_inv = y.no_inv AND x.no_trans = z.no_trans AND z.kd_kartu = a.kd_supplier AND x.prev_no_inv = a.no_inv AND (z.no_posting IS NULL OR z.no_posting = '')) jml_total FROM FIN.dbo.FIN_PEMBELIAN a WHERE (a.tipe_trans LIKE 'JPP-KUT-01' OR a.tipe_trans LIKE 'JPP-KUT-02') AND a.kd_supplier='" & kodeKar & "' AND a.tgl_inv BETWEEN CONVERT(DATE,'" & deTglAwal.EditValue & "',103) AND CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (a.no_posting IS NULL OR a.no_posting = '') AND a.kd_valuta = '" & lckValuta.EditValue & "'", sqlconn))
            End If
        Else
            If chkPosting.Checked Then
                daDetilCari = New SqlDataAdapter(New SqlCommand("SELECT no_inv, tgl_inv, no_jurnal, tgl_posting, keterangan, jml_tagihan, jml_bayar, jml_akhir, kd_supplier FROM FIN.dbo.FIN_PEMBELIAN WHERE (tipe_trans LIKE 'JPP-KUT-01' or tipe_trans LIKE 'JPP-KUT-02') AND jml_akhir > 0 AND kd_supplier='" & kodeKar & "' AND tgl_posting BETWEEN CONVERT(DATE,'" & deTglAwal.EditValue & "',103) AND CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (no_posting IS NULL OR no_posting = '')", sqlconn))
            Else
                daDetilCari = New SqlDataAdapter(New SqlCommand("SELECT a.no_inv, a.tgl_inv, a.no_jurnal, a.tgl_posting, a.keterangan, a.jml_tagihan, (SELECT ISNULL(SUM(x.jml_bayar),0) FROM FIN.dbo.FIN_BELI_LUNAS_D x, FIN.dbo.FIN_PEMBELIAN y, FIN.dbo.FIN_BELI_LUNAS z WHERE x.prev_no_inv = y.no_inv AND x.no_trans = z.no_trans AND z.kd_kartu = a.kd_supplier AND x.prev_no_inv = a.no_inv) jml_bayar, a.jml_tagihan - (SELECT ISNULL(SUM(x.jml_bayar),0) FROM FIN.dbo.FIN_BELI_LUNAS_D x, FIN.dbo.FIN_PEMBELIAN y, FIN.dbo.FIN_BELI_LUNAS z WHERE x.prev_no_inv = y.no_inv AND x.no_trans = z.no_trans AND z.kd_kartu = a.kd_supplier AND x.prev_no_inv = a.no_inv AND (z.no_posting IS NULL OR z.no_posting = '')) jml_total FROM FIN.dbo.FIN_PEMBELIAN a WHERE (a.tipe_trans LIKE 'JPP-KUT-01' OR a.tipe_trans LIKE 'JPP-KUT-02') AND a.jml_akhir > 0 AND a.kd_supplier='" & kodeKar & "' AND a.tgl_inv BETWEEN CONVERT(DATE,'" & deTglAwal.EditValue & "',103) AND CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (a.no_posting IS NULL OR a.no_posting = '') AND a.kd_valuta = '" & lckValuta.EditValue & "'", sqlconn))
            End If
        End If
    ...
    Private Sub loadGiro(Optional ByVal kdTrans As String = "")
        ...
        If Not kdTrans = "" Then
            daGiroCari = New SqlDataAdapter(New SqlCommand("SELECT nomor, jml_trans, status FROM FIN.dbo.FIN_GIRO WHERE idrec IN (SELECT kd_giro FROM FIN.dbo.FIN_BELI_LUNAS WHERE no_trans = '" & kdTrans & "' AND NOT (no_posting IS NULL OR no_posting = ''))", sqlconn))
        Else
            dsGiro.Tables.Clear()
        End If
    ...
    Private Sub loadAll()
        ...
        If Not gvPiutang.RowCount = 0 Then
            kKar = viewPiutang.GetFocusedRowCellValue("kd_supplier").ToString
            loadDetil(kKar)
    ...
    Private Sub gvLunas_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles gvLunas.Click
        Try
            ...
            If Not jml <= 0 Then
                kodetrans = viewLunas.GetFocusedRowCellValue("no_trans_d").ToString
                loadGiro(kodetrans)
                vgiro = viewGiro.GetFocusedRowCellValue("jml_bayar_l").ToString
                If vgiro <= 0 Then
                    If Not dsGiro.Tables("Giro") Is Nothing Then dsGiro.Tables("Giro").Clear()
                End If
            Else
