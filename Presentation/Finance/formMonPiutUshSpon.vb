...
Public Class formMonPiutUshSpon
    ...
    Private Sub loadPiutang()
        ...
        If txtNoTrans.Text = "" Then
            If chkAll.Checked Then
                If lueKaryawan.EditValue = "00000" Or lueKaryawan.EditValue = vbNullString Then
                    If deTglAkhir.EditValue = vbNullString Then
                        daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_cust) as 'nomer', jns_inv, kd_cust, SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_NOTA WHERE (tipe_trans LIKE 'JPJ-KPT-02%' or  tipe_trans LIKE 'JPJ-KPT-04%') AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY jns_inv, kd_cust", sqlconn))
                    Else
                        daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_cust) as 'nomer', jns_inv, kd_cust, SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_NOTA WHERE (tipe_trans LIKE 'JPJ-KPT-02%' or  tipe_trans LIKE 'JPJ-KPT-04%') AND tgl_posting <= CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY jns_inv, kd_cust", sqlconn))
                    End If
                Else
                    If deTglAkhir.EditValue = vbNullString Then
                        daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_cust) as 'nomer', jns_inv, kd_cust, SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_NOTA WHERE (tipe_trans LIKE 'JPJ-KPT-02%' or  tipe_trans LIKE 'JPJ-KPT-04%') AND kd_cust LIKE '%" & lueKaryawan.EditValue & "%' AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY jns_inv, kd_cust", sqlconn))
                    Else
            ...
            Else
                If lueKaryawan.EditValue = "00000" Or lueKaryawan.EditValue = vbNullString Then
                    If deTglAkhir.EditValue = vbNullString Then
                        daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_cust) as 'nomer', jns_inv, kd_cust, SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_NOTA WHERE (tipe_trans LIKE 'JPJ-KPT-02%' or  tipe_trans LIKE 'JPJ-KPT-04%') AND jml_akhir > 0 AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY kd_buku_besar, kd_cust", sqlconn))
                    Else
                        daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_cust) as 'nomer', jns_inv, kd_cust, SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_NOTA WHERE (tipe_trans LIKE 'JPJ-KPT-02%' or  tipe_trans LIKE 'JPJ-KPT-04%') AND jml_akhir > 0 AND tgl_posting <= CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY jns_inv, kd_cust", sqlconn))
                    End If
                Else
                    If deTglAkhir.EditValue = vbNullString Then
    ...
    Private Sub loadDetil(Optional ByVal kodeBB As String = "", Optional ByVal kodeKar As String = "")
        ...
        If txtNoTrans.Text = "" Then
            If chkAll.Checked Then
                If deTglAkhir.EditValue = vbNullString Then
                    daDetilCari = New SqlDataAdapter(New SqlCommand("SELECT no_inv, tgl_inv, no_jurnal, tgl_posting, keterangan, jml_tagihan, jml_bayar, jml_akhir, kd_cust FROM FIN.dbo.FIN_NOTA WHERE (tipe_trans LIKE 'JPJ-KPT-02%' or  tipe_trans LIKE 'JPJ-KPT-04%') AND kd_cust='" & kodeKar & "' AND jns_inv ='" & kodeBB & "' AND NOT (no_posting IS NULL OR no_posting = '')", sqlconn))
                Else
            ...
            End If
        Else
            daDetilCari = New SqlDataAdapter(New SqlCommand("SELECT no_inv, tgl_inv, no_jurnal, tgl_posting, keterangan, jml_tagihan, jml_bayar, jml_akhir, kd_cust FROM FIN.dbo.FIN_NOTA WHERE (tipe_trans LIKE 'JPJ-KPT-02%' or  tipe_trans LIKE 'JPJ-KPT-04%') AND no_inv='" & txtNoTrans.Text & "'", sqlconn))
        End If
