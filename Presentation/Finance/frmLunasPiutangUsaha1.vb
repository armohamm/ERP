...
Public Class frmLunasPiutangUsaha1
    ...
    Public Sub edit_click()
        ...
        dsedit = New DataSet
        da3 = New SqlDataAdapter(New SqlCommand("select * from FIN.dbo.FIN_NOTA_LUNAS where tipe_trans='JKM-KPT-01'", sqlconn)) '<- query buat ditampilkan di LOV
        ...
        If Not frmCari.row Is Nothing Then
            ...
            da3 = New SqlDataAdapter(New SqlCommand("select * from FIN.dbo.FIN_NOTA_LUNAS where no_trans  = '" & kode & "'", sqlconn)) '<- query buat ditampilkan di LOV
            da3.Fill(dsedit, "POcari")
            da4 = New SqlDataAdapter(New SqlCommand("select * from FIN.dbo.FIN_NOTA_LUNAS_D where no_trans = '" & kode & "'", sqlconn))
            ...
            Lookgiro.EditValue = frmCari.row("no_giro").ToString
    ...
    Private Sub v_nota()
        'EDITED BY RENDY
        If Not ds2.Tables("vnota") Is Nothing Then ds2.Tables("vnota").Clear()
        'ds2.Clear()
    ...
    Private Sub V_Supplier()
        ...
        da5 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select Kd_Customer, Nama_Customer, Alamat1 from sif.dbo.SIF_Customer", sqlconn))
    ...
    Private Sub V_pembayaran()
        If Not ds7.Tables("View_bayar") Is Nothing Then ds7.Tables("View_bayar").Clear()
        da7 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select Id_Data,Desc_Data from sif.dbo.SIF_Gen_Reff_D where Id_Ref_data='JNSBYR'", sqlconn))
    ...
    Private Sub V_giro()
        If Not ds1.Tables("View_giro") Is Nothing Then ds1.Tables("View_giro").Clear()
        If lookcust.EditValue = vbNullString Then
            Lookgiro.Enabled = False
            da1 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select nomor, keterangan,tgl_jth_tempo, jml_trans from FIN.dbo.FIN_GIRO where jns_giro='" & lookbayar.EditValue & "' and kd_valuta='" & LEValuta.EditValue & "' and jns_trans='JUAL' and tipe_trans='JRR-KPT-10' and status='CLEARING'", sqlconn))
        Else
            Lookgiro.Enabled = True
            da1 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select nomor, keterangan,tgl_jth_tempo, jml_trans from FIN.dbo.FIN_GIRO where kartu='" & lookcust.EditValue.ToString & "' and jns_giro='" & lookbayar.EditValue.ToString & "' and kd_valuta='" & LEValuta.EditValue.ToString & "' and jns_trans='JUAL' and tipe_trans='JRR-KPT-10' and status='CLEARING'", sqlconn))
        End If
    ...
    Public Sub save_click(Optional ByRef isDelete As Boolean = False)
        ...
        If Not isDelete Then
            ...
            If STATE = "ADD" Then
                da3 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select * from FIN.dbo.FIN_NOTA_LUNAS where no_trans = '" & kode & "' ", sqlconn))
                da3.Fill(ds3, "PO")
    ...
    Private Sub frmLunasPiutangUsaha_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub GV_CellValueChanged(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles GV.CellValueChanged
        ...
        If STATE = "ADD" Then
            da2 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select * from FIN.dbo.FIN_NOTA where no_inv='" & GV.GetRow(e.RowHandle).item("prev_no_inv") & "'", sqlconn))
            ...
            'EDITED BY RENDY
            Try
                If e.Column Is jml_ppn Then
                    If Not GV.GetRow(e.RowHandle).item("jml_ppn").Equals(vbNull) Then
                        view.SetRowCellValue(e.RowHandle, view.Columns("jml_bayar"), (GV.GetRow(e.RowHandle).item("jml_piut") + GV.GetRow(e.RowHandle).item("jml_ppn") + GV.GetRow(e.RowHandle).item("pendp_lain")) - GV.GetRow(e.RowHandle).item("biaya_lain"))
                    End If
                End If
    ...
    Private Sub GV_RowUpdated(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.RowObjectEventArgs) Handles GV.RowUpdated
        ...
        '    GV.GetRow(e.RowHandle).item("pendp_lain") = 0
        '    GV.GetRow(e.RowHandle).item("biaya_lain") = 0
        '    GV.GetRow(e.RowHandle).item("jml_bayar") = rw("jml_val_trans") + rw("jml_val_ppn")
        '    GV.GetRow(e.RowHandle).item("kd_buku_besar") = rw("kd_buku_besar")
        'ElseIf Not e.Column Is prev_no_inv Then
        'For Each rows As DataRow In dsedit.Tables("PODcari").Rows
        '    hitung_total1()
