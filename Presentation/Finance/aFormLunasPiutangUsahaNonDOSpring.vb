...
Public Class aFormLunasPiutangUsahaNonDOSpring
    ...
    Public Sub controlOff(ByVal stat As Boolean)
        ...
        GVDetil.OptionsBehavior.ReadOnly = stat
    End Sub
    ...
    Public Sub edit_click()
        ...
        If Not frmCari.row Is Nothing Then
            ...
            'daNotaLDPEdit = New SqlDataAdapter(New SqlCommand("select * from FIN.dbo.FIN_NOTA_LUNAS_D_D where no_trans = '" & kodeCari & "'", sqlconn))
            'daNotaLDPEdit.Fill(dsEdit, "NotaLunasDPCari")

            'Dim colLunasD(1) As DataColumn
            'Dim colLunasDP(1) As DataColumn

            'colLunasD(0) = dsNotaLunas.Tables("NotaLunasDCari").Columns("no_trans")
            'colLunasD(1) = dsNotaLunas.Tables("NotaLunasDCari").Columns("prev_no_inv")
            'colLunasD(2) = dsNotaLunas.Tables("NotaLunasDCari").Columns("no_seq")

            'colLunasDP(0) = dsNotaLunas.Tables("NotaLunasDPCari").Columns("no_trans")
            'colLunasDP(1) = dsNotaLunas.Tables("NotaLunasDPCari").Columns("prev_no_inv")
            'colLunasDP(2) = dsNotaLunas.Tables("NotaLunasDPCari").Columns("no_seq")

            'rl = dsNotaLunas.Relations.Add("DetailDetail", colLunasD, colLunasDP, False)
    ...
    Private Sub v_nota()
        ...
        'ds2.Clear()
        If STATE = "ADD" Then
            daNota = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select a.*, b.Desc_Data, (select distinct SUM(jml_tagihan) from fin.dbo.FIN_NOTA where jml_akhir > 0 and MONTH(tgl_inv) = MONTH(a.tgl_inv) and kd_cust = a.kd_cust and kd_valuta = a.kd_valuta and tipe_trans in ('JPJ-KPT-01','JPJ-KPT-05') and not (len(no_posting)=0 or no_posting is null or no_posting='')) as jmltagihan from FIN.dbo.FIN_Nota a, sif.dbo.sif_gen_reff_d b where a.jns_inv = b.Id_Data and a.kd_cust = '" & lookcust.EditValue & "' and a.kd_valuta = '" & LEValuta.EditValue & "' and a.tipe_trans in ('JPJ-KPT-01','JPJ-KPT-05') and a.jml_akhir > 0 and not (len(a.no_posting)=0 or a.no_posting is null or a.no_posting='') and b.Id_Ref_FIle = 'JNSINV'", sqlconn))
        Else
            daNota = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select a.*, b.Desc_Data, (select distinct SUM(jml_tagihan) from fin.dbo.FIN_NOTA where jml_akhir > 0 and MONTH(tgl_inv) = MONTH(a.tgl_inv) and kd_cust = a.kd_cust and kd_valuta = a.kd_valuta and tipe_trans in ('JPJ-KPT-01','JPJ-KPT-05') and not (len(no_posting)=0 or no_posting is null or no_posting='')) as jmltagihan from FIN.dbo.FIN_Nota a, sif.dbo.sif_gen_reff_d b where a.jns_inv = b.Id_Data and a.kd_cust = '" & lookcust.EditValue & "' and a.kd_valuta = '" & LEValuta.EditValue & "' and a.tipe_trans in ('JPJ-KPT-01','JPJ-KPT-05') and not (len(a.no_posting)=0 or a.no_posting is null or a.no_posting='') and b.Id_Ref_FIle = 'JNSINV'", sqlconn))
        End If
    ...
    Private Sub V_giro()
        ...
        If lookcust.EditValue = vbNullString Then
            ...
            daGiro = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select nomor, keterangan,tgl_jth_tempo, jml_trans from FIN.dbo.FIN_GIRO where jns_giro='" & lookbayar.EditValue & "' and kd_valuta='" & LEValuta.EditValue & "' and jns_trans='JUAL' and tipe_trans='JRR-KPT-10' and status='CLEARING' and nomor not in (select no_giro from FIN.dbo.FIN_NOTA_LUNAS) AND NOT (no_jur IS NULL OR no_jur = '')", sqlconn))
        Else
            Lookgiro.Enabled = True
            daGiro = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select nomor, keterangan,tgl_jth_tempo, jml_trans from FIN.dbo.FIN_GIRO where divisi = '2' and kartu='" & lookcust.EditValue.ToString & "' and jns_giro='" & lookbayar.EditValue.ToString & "' and kd_valuta='" & LEValuta.EditValue.ToString & "' and jns_trans='JUAL' and tipe_trans='JRR-KPT-10' and status='CLEARING' and nomor not in (select distinct isnull(no_giro,'kosong') from FIN.dbo.FIN_NOTA_LUNAS where tipe_trans in ('JPJ-KPT-01', 'JPJ-KPT-05')) AND NOT (no_jur IS NULL OR no_jur = '')", sqlconn))
        End If
    ...
    Private Sub bersih()
        ...
        'If Not dsEdit.Tables("NotaLunasDPCari") Is Nothing Then dsEdit.Tables("NotaLunasDPCari").Clear()
    ...
    Public Sub save_click(Optional ByRef isDelete As Boolean = False)
        ...
        If Not txtJmlGiro.Text = "" Or txtJmlGiro.Text = "0,00" Then
            ...
            End If
        End If
        If Not GV.Columns("jml_bayar").SummaryText.Equals(txtPembayaran.Text) Then
        ...
        If Not isDelete Then
            ...
            If GV.RowCount <= 0 Then
                MsgBox("Data pelunasan belum diisi. Mohon diisi terlebih dahulu.", MsgBoxStyle.Exclamation + MsgBoxStyle.OkOnly, "PERINGATAN!")
                Exit Sub
            End If
            'For i As Integer = 0 To GV.RowCount - 1
            '    If GV.GetRowCellValue(i, "prev_no_inv").ToString = vbNullString Or GV.GetRowCellValue(i, "prev_no_inv").ToString = "" Then
            '        MsgBox("Ada nota yang kosong pada detil pembayaran. Mohon dihapus terlebih dahulu", MsgBoxStyle.Exclamation + MsgBoxStyle.OkOnly, "PERINGATAN!")
            '        Exit Sub
            '    End If
            'Next
        ...
        If Not isDelete Then
            ...
            ElseIf STATE = "EDIT" Then
                ...
                row.Item("kd_kartu") = lookcust.EditValue
                row.Item("keterangan") = txtKeterangan.Text
                row.Item("kd_bank") = IIf(lookBank.Text = vbNullString, 0, lookBank.EditValue)
                row.Item("kd_valuta") = LEValuta.EditValue
                row.Item("kurs_valuta") = txtkurs.EditValue
                row.Item("jml_rp_trans") = GV.Columns("jml_bayar").SummaryText * txtkurs.Text
                row.Item("jml_val_trans") = GV.Columns("jml_bayar").SummaryText
            ...
            For Each rowx As DataRow In dsNotaLunas.Tables("NotaLunasDPot").Rows
                ...
                ElseIf Not rowx.RowState = DataRowState.Deleted Then
                    If lookbayar.EditValue = "01" Or lookbayar.EditValue = "02" Then
                        rowx.Item("tipe_trans") = "JKM-KPT-01"
                    Else
                        rowx.Item("tipe_trans") = "JKM-KPT-06"
        ...
        Try
            ...
            If isDelete Then
                With New SqlCommand("DELETE FROM FIN.dbo.FIN_JURNAL WHERE no_jur = '" & txtNomor.Text & "'", sqlconn, BTRANS)
                    .ExecuteNonQuery()
                End With
                With New SqlCommand("DELETE FROM FIN.dbo.FIN_JURNAL_D WHERE no_jur = '" & txtNomor.Text & "'", sqlconn, BTRANS)
                    .ExecuteNonQuery()
                End With
            End If

            If Not isDelete Then
                If STATE = "ADD" Then txtNomor.EditValue = getNoTrans("JKM", getTanggal(BTRANS), BTRANS)
    ...
    Public Sub reload_table(ByVal kode As String)
        ...
        colLunasDP(2) = dsNotaLunas.Tables("NotaLunasDPot").Columns("no_seq")
    ...
    Private Sub aFormLunasPiutangUsahaNonDOSpon_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.GotFocus
        btnCetak = True
        setStatus()
    End Sub

    Private Sub aFormLunasPiutangUsahaNonDOSpring_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        connect()
        prn = getParent(Me)
        View_Valuta()
        V_Customer()
    ...
    Private Sub Lookgiro_EditValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles Lookgiro.EditValueChanged
        ...
        'End While
        'drPost.Close()
        If Not dsLookUp.Tables("View_giro").Rows.Count <= 0 Then
            Dim row As DataRow = dsLookUp.Tables("View_giro").Select("nomor='" & Lookgiro.EditValue & "'")(0)
    ...
    Private Sub GV_RowUpdated(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.RowObjectEventArgs) Handles GV.RowUpdated
        ...
        x = CType(GC.FocusedView, DevExpress.XtraGrid.Views.Base.BaseView)
        For i = 0 To GC.FocusedView.RowCount - 1
            jumlah += GC.FocusedView.GetRow(i).Item("jml_piut")
        Next
        lblSelisih.Text = "Selisih : " & CStr(formatNumber(CStr(txtPembayaran.EditValue - jumlah)))
    End Sub
    ...
    Private Sub GVDetil_CellValueChanged(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles GVDetil.CellValueChanged
        'Dim viewD As ColumnView = sender
        'Dim viewM As ColumnView = GV.Columns.View
        'viewM.SetRowCellValue(rhandel, "jml_biaya1", viewD.Columns("jumlah").SummaryText
    ...
    Private Sub GVDetil_InitNewRow(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Grid.InitNewRowEventArgs) Handles GVDetil.InitNewRow
        Dim View As ColumnView = sender
        GC.FocusedView.GetRow(e.RowHandle).Item("no_seq_d") = GC.FocusedView.RowCount + 1
    ...
    Private Sub GVDetil_InvalidValueException(ByVal sender As Object, ByVal e As DevExpress.XtraEditors.Controls.InvalidValueExceptionEventArgs) Handles GVDetil.InvalidValueException
