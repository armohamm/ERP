...
Public Class formMonPiutUshSponTotal
        ...
        If txtNoTrans.Text = "" Then
            If chkAll.Checked Then
                If lueKaryawan.EditValue = "00000" Or lueKaryawan.EditValue = vbNullString Then
                    ...
                    'Else
                    daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_cust) as 'nomer', kd_cust, SUM(jml_tagihan) as 'sisa' FROM FIN.dbo.FIN_NOTA WHERE (tipe_trans LIKE 'JPJ-KPT-02%' or  tipe_trans LIKE 'JPJ-KPT-04%') AND tgl_posting BETWEEN CONVERT(DATE,'" & deTglAwal.EditValue & "',103) AND CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY kd_cust", sqlconn))
                    'End If
                Else
                    'If deTglAwal.EditValue = vbNullString Then
            ...
            Else
                If lueKaryawan.EditValue = "00000" Or lueKaryawan.EditValue = vbNullString Then
                    ...
                    'Else
                    daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_cust) as 'nomer', kd_cust, SUM(jml_tagihan) as 'sisa' FROM FIN.dbo.FIN_NOTA WHERE (tipe_trans LIKE 'JPJ-KPT-02%' or  tipe_trans LIKE 'JPJ-KPT-04%') AND jml_akhir > 0 AND tgl_posting BETWEEN CONVERT(DATE,'" & deTglAwal.EditValue & "',103) AND CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY kd_cust", sqlconn))
                    'End If
                Else
                    'If deTglAwal.EditValue = vbNullString Then
