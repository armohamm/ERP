'tgl 23/09/2001 JKK-KPT-02
Imports System.Data.SqlClient
Imports DevExpress.XtraGrid.Views.Grid
Imports DevExpress.XtraGrid.Views.Base
'sss
Public Class formMonPiutLain
    ...
    Private Sub loadLookUp()
        ...
        daLookKar = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT b.kd_cust, a.nama FROM FIN.dbo.v_kartu a, FIN.dbo.FIN_NOTA b WHERE a.kode=b.kd_cust AND b.tipe_trans='JKK-KPT-02' AND jml_akhir > 0 UNION SELECT '00000', '[Semua Karyawan]'", sqlconn))
    ...
    Private Sub loadDetil(Optional ByVal kodeBB As String = "", Optional ByVal kodeKar As String = "")
        ...
        If txtNoTrans.Text = "" Then
            If deTglAkhir.EditValue = vbNullString Then
                daDetilCari = New SqlDataAdapter(New SqlCommand("SELECT no_inv, tgl_inv, no_jurnal, tgl_posting, keterangan, jml_tagihan, jml_bayar, jml_akhir, kd_cust FROM FIN.dbo.FIN_NOTA WHERE tipe_trans = 'JKK-KPT-02' AND jml_akhir > 0 AND kd_cust='" & kodeKar & "' AND kd_buku_besar ='" & kodeBB & "' AND NOT (no_posting IS NULL OR no_posting = '') ", sqlconn))
            Else
                daDetilCari = New SqlDataAdapter(New SqlCommand("SELECT no_inv, tgl_inv, no_jurnal, tgl_posting, keterangan, jml_tagihan, jml_bayar, jml_akhir, kd_cust FROM FIN.dbo.FIN_NOTA WHERE tipe_trans = 'JKK-KPT-02' AND jml_akhir > 0 AND kd_cust='" & kodeKar & "' AND kd_buku_besar ='" & kodeBB & "' AND tgl_posting <= CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (no_posting IS NULL OR no_posting = '') ", sqlconn))
            End If
        Else
            If deTglAkhir.EditValue = vbNullString Then
                daDetilCari = New SqlDataAdapter(New SqlCommand("SELECT no_inv, tgl_inv, no_jurnal, tgl_posting, keterangan, jml_tagihan, jml_bayar, jml_akhir, kd_cust FROM FIN.dbo.FIN_NOTA WHERE tipe_trans = 'JKK-KPT-02' AND jml_akhir > 0 AND no_inv='" & txtNoTrans.Text & "' AND NOT (no_posting IS NULL OR no_posting = '') ", sqlconn))
            Else
                daDetilCari = New SqlDataAdapter(New SqlCommand("SELECT no_inv, tgl_inv, no_jurnal, tgl_posting, keterangan, jml_tagihan, jml_bayar, jml_akhir, kd_cust FROM FIN.dbo.FIN_NOTA WHERE tipe_trans = 'JKK-KPT-02' AND jml_akhir > 0 AND no_inv='" & txtNoTrans.Text & "' AND tgl_posting <= CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (no_posting IS NULL OR no_posting = '') ", sqlconn))
            End If
        End If
    ...
    Private Sub formMonPiutLain_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        connect()
        prn = getParent(Me)
        Me.Enabled = True
        txtNoTrans.Text = ""
    ...
    Public Sub cetak_click()
        If Not kodeKary = "" Then
            If Not kodeInvo = "" Then
                callReport(App_Path() & "\report\repKartuRinciLainComm.rpt", "", "periode=" & Format(deTglAkhir.EditValue, "yyyyMM") & "&val=IDR&kartu=" & kodeKary & "&inv=" & kodeInvo, False)
            Else
                callReport(App_Path() & "\report\repKartuRinciLainComm.rpt", "", "periode=" & Format(deTglAkhir.EditValue, "yyyyMM") & "&val=IDR&kartu=" & kodeKary & "&inv=", False)
            End If
        Else
            MsgBox("Tidak ada data yang dipilih untuk dicetak. " & vbCrLf & "Pilih salah satu data dari tabel Rekapitulasi Data Piutang Lain-Lain." & vbCrLf & "(Jika ingin mencetak nota yang spesifik, pilih pada tabel Data Rincian Transaksi)", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "PEMBERITAHUAN!")
        End If
        kodeKary = ""
        kodeInvo = ""
    End Sub
