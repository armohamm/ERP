...
Public Class formMonPiutUsh
    ...
    Private Sub loadLookUp()
        ...
        daLookKar = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT b.kd_cust, a.nama FROM FIN.dbo.v_kartu a, FIN.dbo.FIN_NOTA b WHERE a.kode=b.kd_cust AND b.tipe_trans LIKE 'JPJ%' AND jml_akhir > 0 UNION SELECT '00000', '[Semua Data]'", sqlconn))
    ...
    Private Sub loadPiutang()
        ...
        If txtNoTrans.Text = "" Then
            If chkAll.Checked Then
                If lueKaryawan.EditValue = "00000" Or lueKaryawan.EditValue = vbNullString Then
                    If deTglAkhir.EditValue = vbNullString Then
                        daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_cust) as 'nomer', jns_inv, kd_cust, SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_NOTA WHERE tipe_trans LIKE 'JPJ%' AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY jns_inv, kd_cust", sqlconn))
                    Else
                        daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_cust) as 'nomer', jns_inv, kd_cust, SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_NOTA WHERE tipe_trans LIKE 'JPJ%' AND tgl_posting <= CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY jns_inv, kd_cust", sqlconn))
                    End If
                Else
                    If deTglAkhir.EditValue = vbNullString Then
                        daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_cust) as 'nomer', jns_inv, kd_cust, SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_NOTA WHERE tipe_trans LIKE 'JPJ%' AND kd_cust LIKE '%" & lueKaryawan.EditValue & "%' AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY jns_inv, kd_cust", sqlconn))
                    Else
                        daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_cust) as 'nomer', jns_inv, kd_cust, SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_NOTA WHERE tipe_trans LIKE 'JPJ%' AND kd_cust LIKE '%" & lueKaryawan.EditValue & "%' AND tgl_posting <= CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY jns_inv, kd_cust", sqlconn))
                    End If
                End If
            Else
                If lueKaryawan.EditValue = "00000" Or lueKaryawan.EditValue = vbNullString Then
                    If deTglAkhir.EditValue = vbNullString Then
                        daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_cust) as 'nomer', jns_inv, kd_cust, SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_NOTA WHERE tipe_trans LIKE 'JPJ%' AND jml_akhir > 0 AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY kd_buku_besar, kd_cust", sqlconn))
                    Else
                        daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_cust) as 'nomer', jns_inv, kd_cust, SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_NOTA WHERE tipe_trans LIKE 'JPJ%' AND jml_akhir > 0 AND tgl_posting <= CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY jns_inv, kd_cust", sqlconn))
                    End If
                Else
                    If deTglAkhir.EditValue = vbNullString Then
            ...
            End If
        Else
            daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_cust) as 'nomer', jns_inv, kd_cust, SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_NOTA WHERE no_inv LIKE '%" & txtNoTrans.EditValue & "%' GROUP BY jns_inv, kd_cust", sqlconn))
        End If
    ...
    Private Sub loadDetil(Optional ByVal kodeBB As String = "", Optional ByVal kodeKar As String = "")
        ...
        If txtNoTrans.Text = "" Then
            If chkAll.Checked Then
                If deTglAkhir.EditValue = vbNullString Then
                    daDetilCari = New SqlDataAdapter(New SqlCommand("SELECT no_inv, tgl_inv, no_jurnal, tgl_posting, keterangan, jml_tagihan, jml_bayar, jml_akhir, kd_cust FROM FIN.dbo.FIN_NOTA WHERE tipe_trans LIKE 'JPJ%' AND kd_cust='" & kodeKar & "' AND jns_inv ='" & kodeBB & "' AND NOT (no_posting IS NULL OR no_posting = '')", sqlconn))
                Else
                    daDetilCari = New SqlDataAdapter(New SqlCommand("SELECT no_inv, tgl_inv, no_jurnal, tgl_posting, keterangan, jml_tagihan, jml_bayar, jml_akhir, kd_cust FROM FIN.dbo.FIN_NOTA WHERE tipe_trans LIKE 'JPJ%' AND kd_cust='" & kodeKar & "' AND jns_inv ='" & kodeBB & "' AND tgl_posting <= CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (no_posting IS NULL OR no_posting = '')", sqlconn))
                End If
            Else
                If deTglAkhir.EditValue = vbNullString Then
                    daDetilCari = New SqlDataAdapter(New SqlCommand("SELECT no_inv, tgl_inv, no_jurnal, tgl_posting, keterangan, jml_tagihan, jml_bayar, jml_akhir, kd_cust FROM FIN.dbo.FIN_NOTA WHERE tipe_trans LIKE 'JPJ%' AND kd_cust='" & kodeKar & "' AND jns_inv ='" & kodeBB & "' AND jml_akhir > 0 AND NOT (no_posting IS NULL OR no_posting = '')", sqlconn))
                Else
                    daDetilCari = New SqlDataAdapter(New SqlCommand("SELECT no_inv, tgl_inv, no_jurnal, tgl_posting, keterangan, jml_tagihan, jml_bayar, jml_akhir, kd_cust FROM FIN.dbo.FIN_NOTA WHERE tipe_trans LIKE 'JPJ%' AND kd_cust='" & kodeKar & "' AND jns_inv ='" & kodeBB & "' AND jml_akhir > 0 AND tgl_posting <= CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (no_posting IS NULL OR no_posting = '')", sqlconn))
                End If
            End If
        Else
            daDetilCari = New SqlDataAdapter(New SqlCommand("SELECT no_inv, tgl_inv, no_jurnal, tgl_posting, keterangan, jml_tagihan, jml_bayar, jml_akhir, kd_cust FROM FIN.dbo.FIN_NOTA WHERE tipe_trans LIKE 'JPJ%' AND no_inv='" & txtNoTrans.Text & "'", sqlconn))
        End If
    ...
    Private Sub PrintableComponentLinkPiutang_CreateReportHeaderArea(ByVal sender As System.Object, ByVal e As DevExpress.XtraPrinting.CreateAreaEventArgs) Handles PrintableComponentLinkPiutang.CreateReportHeaderArea
        Dim reportHeader As String = "REKAP DATA PIUTANG USAHA"
    ...
    Private Sub PrintableComponentLinkDetil_CreateReportHeaderArea(ByVal sender As System.Object, ByVal e As DevExpress.XtraPrinting.CreateAreaEventArgs) Handles PrintableComponentLinkDetil.CreateReportHeaderArea
        Dim reportHeader As String = "REKAP DATA DETIL PIUTANG USAHA"
        Dim reportHeader2 As String = ":"
        Dim reportHeader3 As String = "PT. Dynasti Indomegah"
        Dim reportHeader4 As String = "Tanggal Cetak"
    ...
    Private Sub PrintableComponentLinkLunas_CreateReportHeaderArea(ByVal sender As System.Object, ByVal e As DevExpress.XtraPrinting.CreateAreaEventArgs) Handles PrintableComponentLinkLunas.CreateReportHeaderArea
        Dim reportHeader As String = "REKAP DATA PELUNASAN PIUTANG USAHA"
        Dim reportHeader2 As String = ":"
        Dim reportHeader3 As String = "PT. Dynasti Indomegah"
        Dim reportHeader4 As String = "Tanggal Cetak"
        Dim reportHeader5 As String = getTanggal.ToLongDateString
