...
Public Class aFormLunasHutangUsaha
                ...
                If Not isDelete Then
                    ...
                    For Each rw As DataRow In dsHutang.Tables("HutangD").Rows
                        If rw.RowState = DataRowState.Added Then
                            ...
                            rw("tipe_trans") = IIf(lookBayar.EditValue = "03", "JKK-KUT-04", "JKK-KUT-01")
    ...
    Private Sub loadGiro()
        ...
        If STATE = "ADD" Then
            ...
            daLookUp = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select idrec, nomor, keterangan, tgl_jth_tempo, jml_trans from FIN.dbo.FIN_GIRO where kartu='" & lookSupplier.EditValue & "' and jns_giro='" & lookBayar.EditValue & "' and kd_valuta='" & lookValuta.EditValue & "' and jns_trans='BELI' and tipe_trans='JRR-KUT-01' and status LIKE 'KELUAR%' and (idrec not in (select distinct isnull(kd_giro,'kosong') from FIN.dbo.FIN_BELI_LUNAS) AND nomor not in (select distinct isnull(kd_giro,'kosong') from FIN.dbo.FIN_BELI_LUNAS)) AND NOT (no_jur IS NULL OR no_jur = '')", sqlconn))
        Else
            lookGiro.Enabled = True
            daLookUp = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select idrec, nomor, keterangan, tgl_jth_tempo, jml_trans from FIN.dbo.FIN_GIRO where kartu='" & lookSupplier.EditValue & "' and jns_giro='" & lookBayar.EditValue & "' and kd_valuta='" & lookValuta.EditValue & "' and jns_trans='BELI' and tipe_trans='JRR-KUT-01' and status LIKE 'KELUAR%'", sqlconn))
        End If
    ...
    Private Sub lookGiro_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lookGiro.EditValueChanged
        ...
        daCari = New SqlDataAdapter(New SqlCommand("select jml_trans, kd_valuta, kurs_valuta from FIN.dbo.FIN_GIRO where idrec = '" & lookGiro.EditValue & "'", sqlconn))
