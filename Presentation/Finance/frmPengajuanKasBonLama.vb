...
Public Class frmPengajuanKasBonLama
    ...
    Dim dsValuta, dsSupp, dsKasBon As New DataSet
    Dim daValuta, daSupp, dakasBon, dakasBon2 As SqlDataAdapter
    ...
    Sub loadTemp()
        ...
        dakasBon2 = New SqlDataAdapter(New SqlCommand("select a.* from FIN.dbo.FIN_KAS_BON a where MONTH(a.tgl_trans) in (MONTH(GETDATE()), MONTH(DATEADD(mm, -1, GETDATE())), MONTH(DATEADD(mm, -2, GETDATE())), MONTH(DATEADD(mm, -3, GETDATE())), MONTH(DATEADD(mm, -4, GETDATE()))) AND YEAR(a.tgl_trans) = YEAR(GETDATE()) order by a.tgl_trans desc", sqlconn))
    ...
    Private Sub GV_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles GV.DoubleClick
        ...
        Dim dr As SqlDataReader = New SqlCommand("select jml_bayar from FIN.dbo.FIN_KAS_BON where nomor = '" & GV.GetRow(GV.GetSelectedRows(0)).item("nomor") & "' ", sqlconn).ExecuteReader
        dr.Read()
        If dr.HasRows Then
            jumlah = dr.Item(0)
        End If
    ...
    Public Sub cetak_click()
        ...
        If Not txtNomor.Text = "" Then
            Dim sqlx As New SqlCommand("update fin.dbo.fin_kas_bon set jml_cetak=isnull(jml_cetak,0)+1, tgl_cetak=getdate() where nomor='" & txtNomor.Text & "'", sqlconn)
    ...
    Public Sub delete_click()
        If Not dsKasBon.Tables("kasbon") Is Nothing Then
            For Each rw As DataRow In dsKasBon.Tables("kasbon").Rows
                Dim sqlm As New SqlCommand("delete from fin.dbo.fin_jurnal where no_jur ='" & rw.Item("no_jur") & "'", sqlconn)
                sqlm.ExecuteNonQuery()
                Dim sqld As New SqlCommand("delete from fin.dbo.fin_jurnal_d where no_jur ='" & rw.Item("no_jur") & "'", sqlconn)
    ...
    Private Sub LOVKartu_EditValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles LOVKartu.EditValueChanged
        Dim angka1 As Integer
        Dim angka2 As Integer
        If LOVKartu.EditValue Is Nothing Then Exit Sub
        If LOVKartu.EditValue = "" Then Exit Sub
        If LOVKartu.Text = "" Then Exit Sub
        If Not STATE = "EDIT" Then
            'dr = New SqlCommand("SELECT COUNT(nomor) AS angka FROM FIN.dbo.FIN_KAS_BON WHERE kd_kartu = '" & LOVKartu.EditValue & "' AND jml_akhir > 0 AND MONTH(tgl_trans) = MONTH(GETDATE())", sqlconn).ExecuteReader
            dr = New SqlCommand("SELECT COUNT(nomor) AS angka FROM FIN.dbo.FIN_KAS_BON WHERE kd_kartu = '" & LOVKartu.EditValue & "' AND jml_akhir > 0 ", sqlconn).ExecuteReader
            dr.Read()
            If dr.HasRows Then
                angka1 = dr.Item(0)
            End If
