...
Public Class frmReportIkhtisarBB
    ...
    Private Sub frmReportIkhtisarBB_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        connect()
        prn = getParent(Me)
        Me.Enabled = True
    ...
    Sub CetakLaporan(Optional ByVal tahun As String = "", Optional ByVal bulan As String = "", Optional ByVal Boll As Boolean = False)

        Try
            ErrorProvider1.Clear()
            If lookupBulanAwal.EditValue = vbNullString Then
                ErrorProvider1.SetError(lookupBulanAwal, "Harap isi periode terlebih dahulu.")
            ElseIf lookupBulanAwal.EditValue <> vbNullString Then
                callReport(App_Path() & "\report\CRIkhtisarBBmaster1.rpt", "", "&bulan=" & bulan & "&tahun=" & tahun, Boll)
    ...
    Public Sub cetak_click()
        If Not lookupBulanAwal.EditValue = vbNullString Then
            CetakLaporan(VarTahun, VarBulan, False)
        Else
            MsgBox("Silahkan isi periode terlebih dahulu!", MsgBoxStyle.Exclamation + MsgBoxStyle.OkOnly, "PERINGATAN!")
        End If
    End Sub
