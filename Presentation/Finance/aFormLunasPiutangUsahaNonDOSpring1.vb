...
Public Class aFormLunasPiutangUsahaNonDOSpring1
    ...
    Private Sub v_nota()
        ...
        'ds2.Clear()
        If STATE = "ADD" Then
            daNota = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select a.*, isnull(a.jml_diskon,0) + isnull(a.jml_cashback,0) as jmldiskon, b.Desc_Data, (select distinct SUM(jml_tagihan) from fin.dbo.FIN_NOTA where jml_akhir > 0 and MONTH(tgl_inv) = MONTH(a.tgl_inv) and kd_cust = a.kd_cust and kd_valuta = a.kd_valuta and tipe_trans in ('JPJ-KPT-01','JPJ-KPT-05') and not (len(no_posting)=0 or no_posting is null or no_posting='')) as jmltagihan from FIN.dbo.FIN_Nota a, sif.dbo.sif_gen_reff_d b where a.jns_inv = b.Id_Data and a.kd_cust = '" & lookcust.EditValue & "' and a.kd_valuta = '" & LEValuta.EditValue & "' and a.tipe_trans in ('JPJ-KPT-01','JPJ-KPT-05') and a.jml_akhir > 0 and not (len(a.no_posting)=0 or a.no_posting is null or a.no_posting='') and b.Id_Ref_FIle = 'JNSINV'", sqlconn))
        Else
    ...
    Private Sub V_Rek()
        ...
        daRek = New SqlDataAdapter(New SqlCommand("select kd_buku_besar, nm_buku_besar from SIF.dbo.SIF_buku_besar_adv WHERE kd_buku_besar LIKE '4%' AND grup_header = 'D' UNION SELECT '0000000', 'N/A'", sqlconn))
    ...
    Public Sub save_click(Optional ByRef isDelete As Boolean = False)
        ...
        If Not isDelete Then
            showMessages("Menyimpan", , "LOADING")
            frmMessage.Refresh()
            If STATE = "ADD" Then
        ...
        Try
            ...
            daNotaL.SelectCommand.CommandTimeout = 0
            daNotaL.InsertCommand.CommandTimeout = 0
            daNotaL.DeleteCommand.CommandTimeout = 0
            daNotaLD.SelectCommand.CommandTimeout = 0
            daNotaLD.InsertCommand.CommandTimeout = 0
            daNotaLDP.DeleteCommand.CommandTimeout = 0
            daNotaLDP.SelectCommand.CommandTimeout = 0
            daNotaLDP.InsertCommand.CommandTimeout = 0
            daNotaLDP.DeleteCommand.CommandTimeout = 0
            ...
            If Not isDelete Then
                If lookbayar.EditValue = "04" Then
                    'txtNomor.EditValue = getNoTrans("JKMB", dtTanggal.EditValue, BTRANS)
                    txtNomor.EditValue = getNoTransFin("JKMB", dtTanggal.EditValue, "B" + CStr(lookBank.EditValue), BTRANS)
                ElseIf lookbayar.EditValue = "02" Or lookbayar.EditValue = "01" Then
                    txtNomor.EditValue = getNoTransFin("JKMB", dtTanggal.EditValue, "G" + Lookgiro.Text, BTRANS)
                Else
                    'txtNomor.EditValue = getNoTrans("JKMK", dtTanggal.EditValue, BTRANS)
                    txtNomor.EditValue = getNoTransFin("JKMK", dtTanggal.EditValue, "X", BTRANS)
                End If
    ...
    Private Sub loadDataGrid()
        ...
        If Not (lookcust.EditValue = vbNullString Or STATE = "EDIT") Then
            ...
            DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select a.*, isnull(a.jml_cashback,0) + isnull(a.jml_pot_nota,0) - isnull(a.jml_kirim,0) as jmldiskon, b.Desc_Data, (select distinct SUM(jml_tagihan) from fin.dbo.FIN_NOTA where jml_akhir > 0 and thnbln = a.thnbln and kd_cust = a.kd_cust and kd_valuta = a.kd_valuta and tipe_trans in ('JPJ-KPT-01','JPJ-KPT-05') and not (len(no_posting)=0 or no_posting is null or no_posting='')) as jmltagihan, a.jml_tagihan - (isnull(a.jml_cashback,0) + isnull(a.jml_pot_nota,0) - isnull(a.jml_kirim,0)) - (select isnull(sum(jml_bayar),0) from fin.dbo.FIN_NOTA_LUNAS_D where prev_no_inv = a.no_inv) jmlsisa from FIN.dbo.FIN_Nota a, sif.dbo.sif_gen_reff_d b where a.jns_inv = b.Id_Data and a.kd_cust = '" & lookcust.EditValue & "' and a.kd_valuta = '" & LEValuta.EditValue & "' and a.tipe_trans in ('JPJ-KPT-01','JPJ-KPT-05') and a.jml_akhir > 0 and not (len(a.no_posting)=0 or a.no_posting is null or a.no_posting='') and b.Id_Ref_FIle = 'JNSINV' and a.jml_tagihan - (select isnull(sum(jml_bayar),0) from fin.dbo.FIN_NOTA_LUNAS_D where prev_no_inv = a.no_inv) > 0order by a.no_inv desc", sqlconn))
    ...
    Private Sub GV_CellValueChanged(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles GV.CellValueChanged
        ...
        If e.Column Is cek Then
            ...
            ElseIf view.GetRowCellValue(e.RowHandle, "cek") = "T" Then
                view.SetRowCellValue(e.RowHandle, "jml_piut", view.GetRowCellValue(e.RowHandle, "jmlsisa"))
            End If
        End If
    ...
    Private Sub Lookgiro_EditValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles Lookgiro.EditValueChanged
        'drPost = New SqlCommand("SELECT jml_trans FROM FIN.dbo.FIN_GIRO WHERE idrec = '" & Lookgiro.EditValue & "'", sqlconn).ExecuteReader
        'While drPost.Read()
    ...
    Private Sub GV_RowUpdated(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.RowObjectEventArgs) Handles GV.RowUpdated
        ...
        If Not (txtPembayaran.EditValue = 0) Then
            ...
            'If viewD.GetRowCellValue(e.RowHandle, "cek") = "Y" Then
            '    jumlah += GV.GetRow(i).Item("jml_bayar")
            'End If
            'If txtPembayaran.EditValue < jumlah Then
    ...
    Private Sub btnCentang2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCentang2.Click
        ...
        Next
        txtJmlSub.EditValue = 0
        txtSelisih.EditValue = txtPembayaran.EditValue - 0
    End Sub
End Class