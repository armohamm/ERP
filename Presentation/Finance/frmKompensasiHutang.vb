...
Public Class frmKompensasiHutang
    ...
    Dim daEdit, daPusat, daNota, daKompensasi, daNotaL, daNotaLD, daCust, daVal, daSupp As SqlDataAdapter
    ...
    Private Sub frmKompensasiHutang_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        connect()
        prn = getParent(Me)
        V_Valuta()
        v_kartu()
    ...
    Private Sub v_nota()
        ...
        daNota = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select A.*, B.nama  " & _
        "  from FIN.dbo.FIN_PEMBELIAN A, FIN.dbo.v_kartu B " & _
        "  where A.kd_supplier = b.kode  " & _
        "  AND A.kd_valuta ='IDR' " & _
    ...
    Private Sub v_supplier()
        If Not dsKompensasi.Tables("Supplier") Is Nothing Then dsKompensasi.Tables("Supplier").Clear()
        daSupp = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select kode, nama from FIN.dbo.v_kartu", sqlconn))
        daSupp.Fill(dsKompensasi, "Supplier")
        lookUpKartu.DataSource = dsKompensasi.Tables("Supplier")
        lookUpKartu.ValueMember = "kode"
        lookUpKartu.DisplayMember = "nama"
    End Sub
    ...
    Private Sub reload_table(ByVal kode As String)
        ...
        daNotaL = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select * from FIN.dbo.FIN_BELI_LUNAS where no_trans = '" & kode & "'", sqlconn))
        daNotaL.Fill(dsKompensasi, "NotaLunas")

        daNotaLD = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select * from FIN.dbo.FIN_BELI_LUNAS_D where no_trans= '" & kode & "'", sqlconn))
    ...
    Private Sub gvNotaDetil_CellValueChanged(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles gvNotaDetil.CellValueChanged
        ...
        If e.Column Is prev_no_inv Then
            'param = ""
            'statex = "XXX"
            drNota = New SqlCommand("SELECT * FROM FIN.dbo.FIN_PEMBELIAN WHERE no_inv = '" & view.GetRowCellValue(e.RowHandle, "prev_no_inv") & "'", sqlconn).ExecuteReader
            While drNota.Read
                view.SetRowCellValue(e.RowHandle, "jml_tagihan", drNota.Item("jml_akhir"))
                view.SetRowCellValue(e.RowHandle, "kd_kartu", drNota.Item("kd_supplier"))
    ...
    Public Sub save_click(Optional ByRef isDelete As Boolean = False)
        Try
                    ...
                    If MsgBox("Simpan data?", MsgBoxStyle.YesNo + MsgBoxStyle.Question, "SIMPAN?") = MsgBoxResult.Yes Then
                        ...
                        For Each rows As DataRow In dsKompensasi.Tables("NotaLunasD").Rows
                            If rows.RowState = DataRowState.Added Then
                                ...
                                rows.Item("tipe_trans") = "JRR-KUT-06"
                        ...
                        For Each rw As DataRow In dsKompensasi.Tables("Kompensasi").Rows
                            If rw.RowState = DataRowState.Added Then
                                ...
                                rw.Item("kd_kartu") = "CST00000"
                                rw.Item("tipe_trans") = "JRR-KUT-06"
            ...
            Try
                ...
                call_procedure("JRR-KUT-06", nomor, BTRANS)
    ...
    Public Sub edit_click()
        ...
        daEdit = New SqlDataAdapter(New SqlCommand("select no_trans as 'No. Transaksi', tgl_trans as 'Tanggal Transaksi', jml_rp_trans as 'Jumlah' from FIN.dbo.FIN_BELI_LUNAS where tipe_trans LIKE 'JRR-KUT-06'", sqlconn)) '<- query buat ditampilkan di LOV
    ...
    Private Sub gvKompensasi_InitNewRow(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Grid.InitNewRowEventArgs) Handles gvKompensasi.InitNewRow
        ...
        View.SetRowCellValue(e.RowHandle, View.Columns("kd_buku_pusat"), "0000000")
    End Sub
    ...
    Private Sub gvKompensasi_ValidateRow(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs) Handles gvKompensasi.ValidateRow
        Try
            ...
            If Microsoft.VisualBasic.Left(gvKompensasi.GetFocusedRowCellValue("kd_buku_besar").ToString, 2) = "5" Or Microsoft.VisualBasic.Left(gvKompensasi.GetFocusedRowCellValue("kd_buku_besar").ToString, 2) = "6%" Or Microsoft.VisualBasic.Left(gvKompensasi.GetFocusedRowCellValue("kd_buku_besar").ToString, 2) = "8%" Then
                If gvKompensasi.GetFocusedRowCellValue("kd_buku_pusat").ToString = "0000000" Then
                    e.Valid = False
    ...
    Private Sub gvKompensasi_ValidatingEditor(ByVal sender As Object, ByVal e As DevExpress.XtraEditors.Controls.BaseContainerValidateEditorEventArgs) Handles gvKompensasi.ValidatingEditor
        If gvKompensasi.FocusedColumn.FieldName = "kd_buku_pusat" Then
            ...
            If Not gvKompensasi.GetFocusedRowCellValue("kd_buku_besar").ToString = vbNullString Or gvKompensasi.GetFocusedRowCellValue("kd_buku_besar").ToString = "" Then
                If Microsoft.VisualBasic.Left(gvKompensasi.GetFocusedRowCellValue("kd_buku_besar").ToString, 2) = "5%" Or Microsoft.VisualBasic.Left(gvKompensasi.GetFocusedRowCellValue("kd_buku_besar").ToString, 2) = "6%" Or Microsoft.VisualBasic.Left(gvKompensasi.GetFocusedRowCellValue("kd_buku_besar").ToString, 2) = "8%" Then
                    If bkpusat = "0000000" Then
                        e.Valid = False
            ...
            Else
                e.Valid = False
                e.ErrorText = "Rekening masih kosong, pilih rekening terlebih dahulu."
            End If
