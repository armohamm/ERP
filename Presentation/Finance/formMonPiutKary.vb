...
Public Class formMonPiutKary
    ...
    Private Sub loadLookUp()
        ...
        daLookKartu = New SqlDataAdapter(New SqlCommand("SELECT Kode_Pegawai, Nama_Pegawai FROM SIF.dbo.SIF_Pegawai", sqlconn))
    ...
    Private Sub loadPiutang()
        ...
        If txtNoTrans.Text = "" Then
            If chkAll.Checked Then
                If lueKaryawan.EditValue = "00000" Or lueKaryawan.EditValue = vbNullString Then
                    If deTglAkhir.EditValue = vbNullString Then
                        daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_cust) as 'nomer', kd_buku_besar, kd_cust, SUM(jml_tagihan) as 'tagihan', SUM(jml_bayar) as 'bayar', SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_NOTA WHERE tipe_trans = 'JKK-KPT-01' AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY kd_buku_besar, kd_cust", sqlconn))
                    Else
                        daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_cust) as 'nomer', kd_buku_besar, kd_cust, SUM(jml_tagihan) as 'tagihan', SUM(jml_bayar) as 'bayar', SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_NOTA WHERE tipe_trans = 'JKK-KPT-01' AND tgl_posting <= CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY kd_buku_besar, kd_cust", sqlconn))
                    End If
                Else
                    If deTglAkhir.EditValue = vbNullString Then
                        daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_cust) as 'nomer', kd_buku_besar, kd_cust, SUM(jml_tagihan) as 'tagihan', SUM(jml_bayar) as 'bayar', SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_NOTA WHERE tipe_trans = 'JKK-KPT-01' AND kd_cust LIKE '%" & lueKaryawan.EditValue & "%' AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY kd_buku_besar, kd_cust", sqlconn))
                    Else
            ...
            Else
                If lueKaryawan.EditValue = "00000" Or lueKaryawan.EditValue = vbNullString Then
                    If deTglAkhir.EditValue = vbNullString Then
                        daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_cust) as 'nomer', kd_buku_besar, kd_cust, SUM(jml_tagihan) as 'tagihan', SUM(jml_bayar) as 'bayar', SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_NOTA WHERE tipe_trans = 'JKK-KPT-01' AND jml_akhir > 0 AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY kd_buku_besar, kd_cust", sqlconn))
                    Else
                        daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_cust) as 'nomer', kd_buku_besar, kd_cust, SUM(jml_tagihan) as 'tagihan', SUM(jml_bayar) as 'bayar', SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_NOTA WHERE tipe_trans = 'JKK-KPT-01' AND jml_akhir > 0 AND tgl_posting <= CONVERT(DATE,'" & deTglAkhir.EditValue & "',103) AND NOT (no_posting IS NULL OR no_posting = '') GROUP BY kd_buku_besar, kd_cust", sqlconn))
                    End If
                Else
                    If deTglAkhir.EditValue = vbNullString Then
            ...
            End If
        Else
            daPiut = New SqlDataAdapter(New SqlCommand("SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY kd_cust) as 'nomer', kd_buku_besar, kd_cust, SUM(jml_tagihan) as 'tagihan', SUM(jml_bayar) as 'bayar', SUM(jml_akhir) as 'sisa' FROM FIN.dbo.FIN_NOTA WHERE no_inv LIKE '%" & txtNoTrans.EditValue & "%' GROUP BY kd_buku_besar, kd_cust", sqlconn))
        End If
    ...
    Private Sub loadDetil(Optional ByVal kodeBB As String = "", Optional ByVal kodeKar As String = "")
        ...
        If txtNoTrans.Text = "" Then
            ...
            Else
                If deTglAkhir.EditValue = vbNullString Then
                    daDetilCari = New SqlDataAdapter(New SqlCommand("SELECT no_inv, tgl_inv, no_jurnal, tgl_posting, keterangan, jml_tagihan, jml_bayar, jml_akhir, kd_cust FROM FIN.dbo.FIN_NOTA WHERE tipe_trans = 'JKK-KPT-01' AND kd_cust='" & kodeKar & "' AND kd_buku_besar ='" & kodeBB & "' AND jml_akhir > 0 AND NOT (no_posting IS NULL OR no_posting = '')", sqlconn))
                Else
    ...
    Private Sub formMonPiutKary_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        connect()
        prn = getParent(Me)
        Me.Enabled = True
        txtNoTrans.Text = ""
    ...
    Private Sub PrintableComponentLinkLunas_CreateReportHeaderArea(ByVal sender As System.Object, ByVal e As DevExpress.XtraPrinting.CreateAreaEventArgs) Handles PrintableComponentLinkLunas.CreateReportHeaderArea
        Dim reportHeader As String = "REKAP DATA PELUNASAN PIUTANG KARYAWAN"
        Dim reportHeader2 As String = ":"
        Dim reportHeader3 As String = "PT. Dynasti Indomegah"
        Dim reportHeader4 As String = "Tanggal Cetak"
        Dim reportHeader5 As String = getTanggal.ToLongDateString
