...
Public Class aFormLunasPiutangUsahaDOSpring1
    ...
    Public Sub edit_click()
        ...
        If Not frmCari.row Is Nothing Then
            ...
            reload_table(kode)
            If frmCari.row("no_posting") Is Nothing Or frmCari.row("no_posting") = "" Then
    ...
    Private Sub V_giro()
        ...
        Else
            ...
            daGiro = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select idrec, nomor, keterangan,tgl_jth_tempo, jml_trans from FIN.dbo.FIN_GIRO where divisi = '2' and kartu='" & lookcust.EditValue.ToString & "' and jns_giro='" & lookbayar.EditValue.ToString & "' and kd_valuta='" & LEValuta.EditValue.ToString & "' and jns_trans='JUAL' and tipe_trans='JRR-KPT-10' and status='CLEARING' and not idrec in (select distinct isnull(no_giro,'kosong') from FIN.dbo.FIN_NOTA_LUNAS where tipe_trans in ('JKM-KPT-01', 'JKM-KPT-06')) AND NOT (no_jur IS NULL OR no_jur = '')", sqlconn))
        End If
    ...
    Public Sub save_click(Optional ByRef isDelete As Boolean = False)
        ...
        If Not isDelete Then
            ...
            For Each rowx As DataRow In dsNotaLunas.Tables("NotaLunasDPot").Rows
                If rowx.RowState = DataRowState.Added Then
                    ...
                    If lookbayar.EditValue = "01" Or lookbayar.EditValue = "02" Then
                        rowx.Item("tipe_trans") = "JKM-KPT-05"
                    Else
                        rowx.Item("tipe_trans") = "JRR-KPT-16"
                    End If
        ...
        Try
            ...
            If Not isDelete Then
                ...
                'Else
                '    call_procedure("JKM-KPT-16", txtNomor.EditValue, BTRANS)
                'End If
            End If
    ...
    Private Sub loadDataGrid()
        ...
        If Not (lookcust.EditValue = vbNullString Or STATE = "EDIT") Then
            ...
            DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select a.*, isnull(a.jml_cashback,0) + isnull(a.jml_pot_nota,0) as jmldiskon, b.Desc_Data, (select distinct SUM(jml_tagihan) from fin.dbo.FIN_NOTA where jml_akhir > 0 and MONTH(tgl_inv) = MONTH(a.tgl_inv) and kd_cust = a.kd_cust and kd_valuta = a.kd_valuta and tipe_trans in ('JPJ-KPT-01','JPJ-KPT-05') and not (len(no_posting)=0 or no_posting is null or no_posting='')) as jmltagihan from FIN.dbo.FIN_Nota a, sif.dbo.sif_gen_reff_d b where a.jns_inv = b.Id_Data and a.kd_cust = '" & lookcust.EditValue & "' and a.kd_valuta = '" & LEValuta.EditValue & "' and a.tipe_trans in ('JPJ-KPT-01','JPJ-KPT-05') and a.jml_akhir > 0 and not (len(a.no_posting)=0 or a.no_posting is null or a.no_posting='') and b.Id_Ref_FIle = 'JNSINV' order by a.no_inv desc", sqlconn))
    ...
    Public Sub reload_table(ByVal kode As String)
        ...
        If STATE = "ADD" Then
            daNotaLD = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select *, CAST('01/01/2014' AS DATE) as tgl_inv, 0.0 as jmltagihan, 0 as jmldiskon, '' as jenis, 'T' as cek from FIN.dbo.FIN_NOTA_LUNAS_D where no_trans= '" & kode & "'", sqlconn))
            daNotaLD.Fill(dsNotaLunas, "NotaLunasD")
        ElseIf STATE = "EDIT" Then
            daNotaLD = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select *,(SELECT tgl_inv FROM FIN.dbo.FIN_Nota WHERE no_inv = a.prev_no_inv) as tgl_inv, (select distinct SUM(jml_tagihan) from fin.dbo.FIN_NOTA where no_inv = a.prev_no_inv) as jmltagihan, (select y.Desc_Data FROM fin.dbo.FIN_NOTA x, sif.dbo.sif_gen_reff_d y WHERE x.jns_inv = y.Id_Data AND x.no_inv = a.prev_no_inv and y.Id_Ref_FIle = 'JNSINV' ) as jenis, 'Y' as cek from FIN.dbo.FIN_NOTA_LUNAS_D a where no_trans= '" & kode & "'", sqlconn))
            daNotaLD.Fill(dsNotaLunas, "NotaLunasD")
        End If
    ...
    Private Sub Lookgiro_EditValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles Lookgiro.EditValueChanged
        ...
        'End While
        'drPost.Close()
        If Not (Lookgiro.Text = "[Tidak ada data]") Then
    ...
    Private Sub btnCentang_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCentang.Click
        For i As Integer = 0 To GV.RowCount - 1
            If GV.GetRowCellValue(i, "cek") = "T" Then
    ...
    Private Sub lookDO_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lookDO.EditValueChanged
        ...
        Else
            ...
            While drPost.Read()
                txtJmlGiro.EditValue = drPost.Item("jml_akhir")
                txtPembayaran.EditValue = drPost.Item("jml_akhir")
            End While
            drPost.Close()
        End If
    End Sub
