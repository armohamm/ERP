...
Public Class aFormLaporanKartuPiutKar
    ...
    Private Sub aFormLaporanKartuPiutKar_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        connect()
        prn = getParent(Me)
        Me.Enabled = True
        ...
        If isOK = True Then
            ...
            If radKartuPiut.Checked = True Then
                If cmbKartuPiut.EditValue = "Rekap" Then
                    callReport(App_Path() & "\report\repKartuRekapKar.rpt", "", "v_periode=" & lookupPeriode.EditValue & "&v_valuta=" & lookupValuta.EditValue, False)
                ElseIf cmbKartuPiut.EditValue = "Rinci" Then
                    If lookupKartu.EditValue = vbNullString Or lookupKartu.EditValue = "00000" Then
                        callReport(App_Path() & "\report\repKartuRinciKar.rpt", "", "vperiode=" & lookupPeriode.EditValue & "&vvaluta=" & lookupValuta.EditValue, False)
                    Else
                        callReport(App_Path() & "\report\repKartuRinciKar.rpt", "", "vperiode=" & lookupPeriode.EditValue & "&vvaluta=" & lookupValuta.EditValue & "&vkartu=" & lookupKartu.EditValue, False)
                    End If
                End If
            End If
        Else
