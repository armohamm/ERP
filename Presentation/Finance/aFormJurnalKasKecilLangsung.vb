...
Public Class aFormJurnalKasKecilLangsung
        ...
        If isvalid Then
            If Not isDelete Then
                ...
                If STATE = "ADD" Then
                    ...
                    row("tipe_trans") = "JKK-KBB-10"
                    row("kartu") = "00000000"
                    row("kd_buku_pusat") = "00000000"
                    row("kd_buku_besar") = lue_kas.EditValue
    ...
    Private Sub loadList()
        ...
        daJurnal = New SqlDataAdapter(New SqlCommand(" SELECT kd_buku_besar, nm_buku_besar as Nama_Buku_Besar " & _
                                                     ...
                                                     "WHERE grup_header='D' AND div1='Y' and " & _
        ...
        lue_kas.Properties.DataSource = dsJurnal.Tables("BukuBesarKas")
        lue_kas.Properties.DisplayMember = "Nama_Buku_Besar"
        lue_kas.Properties.ValueMember = "kd_buku_besar"

        daBB = New SqlDataAdapter(New SqlCommand("SELECT kd_buku_besar, nm_buku_besar FROM KOPKAR.dbo.m_buku_besar WHERE grup_header='D' AND div1='Y'  and " & _
                                                 "kd_buku_besar not in (SELECT kd_buku_besar " & _
                                                     "FROM KOPKAR.dbo.m_buku_besar " & _
                                                     "WHERE grup_header='D' AND div1='Y' and " & _
                                                     "kd_buku_besar in (Select d.Val_kode1  " & _
                                                     "From KOPKAR.dbo.SIF_Gen_Reff_D d " & _
    ...
    Private Sub loadData(ByVal kode As String)
        ...
        If STATE = "EDIT" Then
            For i = dsJurnal.Tables("JurnalD").Rows.Count - 1 To 0 Step -1
                ...
                If rows("saldo_val_kredit") > 0 Then
                    txt_JumlahBayar.EditValue = rows("saldo_val_kredit")
                    lue_kas.EditValue = rows("kd_buku_besar")
    ...
    Private Sub gvJurnal_CellValueChanged(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles gvJurnal.CellValueChanged
        ...
        If e.Column Is kd_buku_besar Then
            ...
            'drPost = New SqlCommand("select nm_buku_besar, pola_entry from KOPKAR.dbo.m_buku_besar where kd_buku_besar = '" & gvJurnal.GetRow(e.RowHandle).item("kd_buku_besar") & "'", sqlconn).ExecuteReader
    ...
    Private Sub lue_kas_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lue_kas.EditValueChanged
        txt_SaldoKas.Focus()
        reload_kas()
    End Sub
    Sub hitung_selisih()
    ...
    Private Sub reload_kas()
        Dim dr As SqlDataReader = New SqlCommand("SELECT KOPKAR.dbo.hitungsaldoawal('" & lue_kas.EditValue & "', 'IDR','" & Now.Year & "', '" & Now.Month & "', '" & IIf(Now.Day - 1 = 0, Now.Day, Now.Day) & "')", sqlconn).ExecuteReader
        dr.Read()
        If dr.HasRows Then
    ...
    End Sub
    Private Sub txt_JumlahBayar_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txt_SaldoKas.EditValueChanged
        hitung_selisih()
    End Sub
