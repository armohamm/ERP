...
Public Class frmSales
    ...
    Dim row As DataRow
    Dim DA, DA1, daedit As SqlClient.SqlDataAdapter
    ...
    Private Sub reload_table(ByVal kode As String)
        ...
        DA1 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select * from SIF_Sales where Kode_Sales='" & kode & "'", sqlconn))
    ...
    Public Sub edit_click()
        ...
        daedit = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select Kd_Customer as [Nomor],Nama_Customer as [Nama Customer] from SIF_Customer", sqlconn)) '<- query buat ditampilkan di LOV, pakailah alias colom biar enak dibaca
        If Not dsedit.Tables("lookup_Customer") Is Nothing Then dsedit.Tables("lookup_DPM").Clear()
    ...
    End Sub
    Sub View_Pegawai()
        DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select Kode_Pegawai,Nama_pegawai from sif.dbo.SIF_Pegawai", sqlconn))
        DA.Fill(dSO, "View_Peg")
        LEDep.Properties.DataSource = dSO.Tables("View_Peg")
    ...
    Sub View_departemen()
        ...
        LEPegawai.Properties.DataSource = dSO.Tables("View_Dep")
