...
Public Class frmbarangjadi
    ...
    End Sub
    Private Sub frmbarangjadi_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        DA2 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select * from SIF.dbo.SIF_Barang Where kd_jns_persd= 1 ", sqlconn))
        DA2.Fill(dsete, "Barang")
        GridControl1.DataSource = dsete.Tables("Barang")
        GridView1.BestFitColumns()
        Txtjnsprs.Text = "Barang Jadi"
        obj = getParent(Me)
        prn = obj
        Lookup()
    ...
    Public Sub save_click(Optional ByRef isDelete As Boolean = False)
        ...
        Dim sqlDR As SqlDataReader = New SqlCommand("select max( no_urut)+1  from SIF.dbo.SIF_Barang where kd_depart= '" & LookUpEditdivisi.EditValue & "' AND kd_jns_persd = '1'  AND kd_jenis= '" & LookUpEditjnsbrg.EditValue & "' AND kd_merk='" & LookUpEditmerk.EditValue & "' AND kd_tipe= '" & LookUpEdittpbrg.EditValue & "' AND kd_sub_tipe= '" & LookUpEditsubtp.EditValue & "' AND kd_ukuran='" & LookUpEditukrn.EditValue & "'AND kd_kain= '" & LookUpEditkain.EditValue & "' ", sqlconn).ExecuteReader
        sqlDR.Read()
        temp = ""
        hasil = ""
        If TypeOf sqlDR.Item(0) Is DBNull Then
            temp = "001"
            hasil = LookUpEditdivisi.EditValue & "1" & LookUpEditjnsbrg.EditValue & LookUpEditmerk.EditValue & LookUpEdittpbrg.EditValue & LookUpEditsubtp.EditValue & LookUpEditukrn.EditValue & LookUpEditkain.EditValue & temp
        Else
