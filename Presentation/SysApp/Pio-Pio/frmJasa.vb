...
Public Class frmJasa
    ...
    Public Sub save_click(Optional ByRef isDelete As Boolean = False)
        ...
        Dim sqlDR As SqlDataReader = New SqlCommand("select max( no_urut)+1  from SIF.dbo.SIF_Barang where kd_depart= '" & LookUpEditdivisi.EditValue & "' AND kd_jns_persd = '4'  AND kd_jenis= '" & LookUpEditjnsbrg.EditValue & "' AND kd_merk='" & LookUpEditmerk.EditValue & "' AND kd_tipe= '" & LookUpEdittpbrg.EditValue & "' AND kd_sub_tipe= '" & LookUpEditsubtp.EditValue & "' AND kd_ukuran='" & LookUpEditukrn.EditValue & "'AND kd_kain= '" & LookUpEditkain.EditValue & "' ", sqlconn).ExecuteReader
        sqlDR.Read()
        temp = ""
        hasil = ""
        If TypeOf sqlDR.Item(0) Is DBNull Then
            temp = "001"
            hasil = LookUpEditdivisi.EditValue & LookUpEditjnsbrg.EditValue & LookUpEditjnsbrg.EditValue & LookUpEditmerk.EditValue & LookUpEdittpbrg.EditValue & LookUpEditsubtp.EditValue & LookUpEditukrn.EditValue & LookUpEditkain.EditValue & temp
        Else
            If sqlDR.Item(0) >= 1 And sqlDR.Item(0) <= 9 Then
                temp = "00" & sqlDR.Item(0)
                hasil = LookUpEditdivisi.EditValue & "4" & LookUpEditjnsbrg.EditValue & LookUpEditmerk.EditValue & LookUpEdittpbrg.EditValue & LookUpEditsubtp.EditValue & LookUpEditukrn.EditValue & LookUpEditkain.EditValue & temp
