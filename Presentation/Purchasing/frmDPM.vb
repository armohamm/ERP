...
Public Class frmDPM
    ...
    Public Sub edit_click()
        'daedit = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select No_DPM as [Nomor], tgl_DPM as [Tanggal], keterangan [Keterangan], rec_stat Status from PURC.dbo.PURC_DPM where tipe_trans='MATERIAL' and rec_stat in ('ENTRY', 'REJECT' ,'PERBAIKAN') order by No_DPM desc", sqlconn)) '<- query buat ditampilkan di LOV, pakailah alias colom biar enak dibaca
        ...
        If Not frmCari.row Is Nothing Then '<-- jika row ada yg dipilih
            ...
            setStatus()
            Me.Enabled = True
            If frmCari.row("Status") = "PERBAIKAN" Then
    ...
    Private Sub frmDPM_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        RepositoryItemGridLookUpEdit2View.Columns("Kd_Satuan").Caption = "Satuan"
        RepositoryItemGridLookUpEdit2View.Columns("Kode_Barang").Caption = "Kode Barang"
        RepositoryItemGridLookUpEdit2View.Columns("Nama_Barang").Caption = "Nama Barang"
        RepositoryItemGridLookUpEdit2View.Columns("spek_brg").Caption = "Spek Barang"
    ...
    Private Sub GV_InitNewRow(ByVal sender As System.Object, ByVal e As DevExpress.XtraGrid.Views.Grid.InitNewRowEventArgs) Handles GV.InitNewRow
        ...
        da2 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select * from sif.dbo.SIF_Barang where kd_jns_persd <> 4 and rec_stat = 'Y' order by Nama_Barang", sqlconn))
        da2.Fill(dS, "VBarang")
        repbarang.DataSource = dS.Tables("VBARANG")
    End Sub
    ...
    Public Sub caridpmku(ByVal nodpmku As String) ' ============= cretaed by yaya =============
        ...
        If dsku.Tables(0).Rows.Count Then
            ...
            txtStatus.Text = dsku.Tables(0).Rows(0).Item("rec_stat").ToString
    ...
    Private Sub GV_ValidatingEditor(ByVal sender As Object, ByVal e As DevExpress.XtraEditors.Controls.BaseContainerValidateEditorEventArgs) Handles GV.ValidatingEditor
        ...
        '        Dim Kd_Stok As String = e.Value

        '        For i As Integer = 0 To GV.RowCount
        '            If Kd_Stok = vx.GetRowCellValue(i, "Kd_Stok") Then
        '                e.Valid = False
        '                e.ErrorText = "Item sudah ada d list pemesanan"
        '            End If
        '        Next
    ...
    Private Sub txtcari_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtcari.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            If Not dS.Tables("VBarang") Is Nothing Then dS.Tables("VBarang").Clear()
            da2 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select * from sif.dbo.SIF_Barang where kd_jns_persd <> 4 and nama_barang like '%" & txtcari.Text & "%' and rec_stat = 'Y' order by Nama_Barang ", sqlconn))
            da2.Fill(dS, "VBarang")
            If dS.Tables("VBARANG").Rows.Count > 0 Then
                repbarang.DataSource = dS.Tables("VBARANG")
                txtcari.Visible = False
                Label9.Visible = False
            ElseIf dS.Tables("VBARANG").Rows.Count = 0 Then
    ...
    Private Sub GV_ShowingEditor(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles GV.ShowingEditor
        If Not pnlTengahAtas.Enabled Then
            ...
            If Not view.FocusedColumn Is Kd_Stok Or Not view.FocusedColumn Is spek_brg Then
                e.Cancel = True
            End If
        End If
    End Sub
