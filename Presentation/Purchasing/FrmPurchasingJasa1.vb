...
Public Class FrmPurchasingJasa1
        ...
        If Not isDelete Then
            ...
            If STATE = "ADD" Then
                ...
                For Each row In ds4.Tables("PRD").Rows
                    ...
                    row.Item("no_seq") = row.Item("no_seq")
    ...
    Public Sub edit_click()
        ...
        daedit = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select distinct PRD.no_pr as [Nomor],PRD.tgl_pr as Tanggal from PURC.DBO.PURC_PR_D PRD " & _
                                                                       "left join PURC.DBO.PURC_CHOICE_SUPP CSP on PRD.kd_stok=CSP.kd_stok and PRD.no_pr=CSP.no_pr left join PURC.DBO.PURC_DPM dp on PRD.no_dpm=dp.no_dpm " & _
                                                                       "where PRD.jns_dpm='JASA' and dp.Departemen='" & kddep & "' order by PRD.no_pr desc", sqlconn))
        ...
        If Not frmCari.row Is Nothing Then '<-- jika row ada yg dipilih
            ...
            If Not dsedit.Tables("lookup_PR1") Is Nothing Then dsedit.Tables("lookup_PR1").Clear()
            'daedit = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select distinct No_PR as [Nomor], tgl_PR as [Tanggal] from PURC.DBO.PURC_PR_D where jns_dpm='JASA' and rec_stat='ENTRY' or rec_stat='REJECT' ", sqlconn)) '<- query buat ditampilkan di LOV, pakailah alias colom biar enak dibaca
            daedit = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select distinct CSP.rec_stat, PRD.no_pr as [Nomor],PRD.tgl_pr as Tanggal from PURC.DBO.PURC_PR_D PRD " & _
                                                                           "left join PURC.DBO.PURC_CHOICE_SUPP CSP on PRD.kd_stok=CSP.kd_stok and PRD.no_pr=CSP.no_pr left join PURC.DBO.PURC_DPM dp on PRD.no_dpm=dp.no_dpm " & _
                                                                           "where PRD.jns_dpm='JASA' and dp.Departemen='" & kddep & "'", sqlconn))
            daedit.Fill(dsedit, "lookup_PR1")
            For i As Integer = 0 To dsedit.Tables("lookup_PR1").Rows.Count - 1
                Dim row As DataRow
                row = dsedit.Tables("lookup_PR1").Rows(i)
                btnSave = True
                btnadd = False
                btnEdit = False
                btnDelete = True
                ...
                Me.Enabled = True
                btn_supp.Enabled = True
                'If row.Item("rec_stat") = "APPROVE" Then
                '    btnSave = False
                '    btnadd = False
                '    btnEdit = False
                '    btnDelete = False
                ...
                '    Me.Enabled = True
                '    btn_supp.Enabled = True
                'ElseIf row.Item("rec_stat") <> "APPROVE" Then
                '    btnSave = True
                '    btnadd = False
                '    btnEdit = False
                '    btnDelete = True
    ...
    Private Sub btn_supp_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_supp.Click
        FrmPilihSupplierJasa1.LEPR.EditValue = txtNomor.Text
        FrmPilihSupplierJasa1.LEPR.Enabled = False
        'FrmPilihSupplier.SimpleButton1.Visible = True
        'FrmPilihSupplier.btSimpan.Visible = True
        FrmPilihSupplierJasa1.STATE = "ADD"
        FrmPilihSupplierJasa1.ShowDialog()
        btnsupp()
    End Sub
