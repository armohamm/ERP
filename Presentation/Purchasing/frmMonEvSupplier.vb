...
Public Class frmMonEvSupplier
    ...
    Private Sub refreshNomer1()
        Dim i As Int16 = 1
        Dim j As Int16 = 1
        ' If State = "DELETE" Then Exit Sub
        For Each rwd As DataRow In ds.Tables("EVSX1").Rows
            rwd("No_seq") = i
            i += 1
        Next
    End Sub
    ...
    Private Sub reload_table()                   'ByVal periode As String
        ...
        ds.Relations.Clear()
        If Not ds.Tables("EVSX1") Is Nothing Then ds.Tables("EVSX1").Clear()
        ...
        da = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("Select '' AS no_seq,ISNULL(e.no_trans,'') no_trans,e.periode1, e.periode2, e.kd_supplier,s.Nama_Supplier " & _
                                                                   ",sum(e.total) /  COUNT(e.periode) as nilai From PURC.dbo.PURC_EVALUASI_SUPPLIER e " & _
                                                                   "left join SIF.dbo.SIF_Supplier s on s.Kode_Supplier = e.kd_supplier " & _
                                                                   "where e.kd_supplier = '" & txtSupplier.EditValue & "' and e.periode1 BETWEEN '" & lckPeriodeAwal.EditValue & "' and '" & lckPeriodeAkhir.EditValue & "' " & _
                                                                   "GROUP BY ISNULL(e.no_trans,''),e.periode1, e.periode2, e.kd_supplier,s.Nama_Supplier ", sqlconn))
        da.Fill(ds, "EVSX")
        'GcEvSupplier.DataSource = ds.Tables("EVSX")

        da = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("SELECT '' AS no_seq, * FROM PURC.dbo.PURC_EVALUASI_SUPPLIER WHERE kd_supplier='" & txtSupplier.EditValue & "' AND periode between '" & lckPeriodeAwal.EditValue & "' AND '" & lckPeriodeAkhir.EditValue & "' ORDER BY no_po", sqlconn))
        da.Fill(ds, "EVSX1")
        'GcEvSupplier.DataSource = ds.Tables("EVSX")
        GcEvSupplier.DataSource = ds.Tables("EVSX1")
        Dim colD1(0) As DataColumn
        Dim colD2(0) As DataColumn
        Dim rel As DataRelation

        colD1(0) = ds.Tables("EVSX").Columns("no_trans")
        colD2(0) = ds.Tables("EVSX1").Columns("no_trans")
       
        'rel = ds.Relations.Add("Detail", colD1, colD2, False)
        'GcEvSupplier.LevelTree.Nodes.Add("Detail", GridView2)
    ...
    Private Sub frmEvSupplier_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        da.Fill(ds, "EVS")
        'reload_table()
        GcEvSupplier.MainView = GridView2
    End Sub
    ...
    Private Sub PrintableComponentLink1_CreateReportFooterArea(ByVal sender As Object, ByVal e As DevExpress.XtraPrinting.CreateAreaEventArgs) Handles PrintableComponentLink1.CreateReportFooterArea
        ...
        Dim reportFooter9 As String = GridView2.Columns("total").SummaryText
