...
Public Class frmReorderBarang
    ...
    Sub View_PR_D()
        ...
        da7 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select distinct no_po from PURC.DBO.PURC_PO where tipe_trans='JPP-KUT-01'", sqlconn))
    ...
    Public Sub edit_click()
        daedit = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select No_PO as [Nomor],tgl_PO as [Tanggal] from PURC.dbo.PURC_PO where tipe_trans='JPP-KUT-01' and status='ENTRY' or status='REJECT'", sqlconn)) '<- query buat ditampilkan di LOV, pakailah alias colom biar enak dibaca
    ...
    Private Sub reload_table(ByVal kode As String)
        ...
        If ds3.Tables("PO").Rows.Count > 0 Then
            ...
            txtstatus.EditValue = row.Item("status").ToString
