...
Public Class frmMonPODetil_2
    ...
    Dim dSO, ds1, ds2, ds3, ds_cari As New DataSet
    ...
    Sub Load_PO_Filter2()
        ...
        If tglsampai.EditValue = Nothing Then
            If tglDari2.Text = "" And tglsampai2.Text <> "" Then
        ...
        Else
            If tglDari2.Text = "" And tglsampai2.Text <> "" Then
                sql = " and c.tgl_janji_kirim < convert(date,'" & tglsampai2.EditValue & "',103) "
                sql1 = " and c.tgl_po < convert(date,'" & tglsampai.EditValue & "',103) "
            ElseIf tglDari2.Text <> "" And tglsampai2.Text = "" Then
                sql = " and c.tgl_janji_kirim > convert(date,'" & tglDari2.EditValue & "',103) "
                sql1 = " and c.tgl_po > convert(date,'" & tglDari.EditValue & "',103) "
            Else
                sql = " and c.tgl_janji_kirim  between convert(date,'" & tglDari2.EditValue & "',103) and convert(date,'" & tglsampai2.EditValue & "',103) " ' "
                sql1 = " and c.tgl_po  between convert(date,'" & tglDari.EditValue & "',103) and convert(date,'" & tglsampai.EditValue & "',103) " ' "
            End If
        ...
        If tglsampai.EditValue = Nothing Then
            ...
            sqlcmd.CommandText = "SELECT c.keterangan,c.tipe_trans, c.no_po, c.tgl_po, c.kd_supplier, c.Nama_Supplier, c.kd_valuta, c.kd_stok, c.nama_brg, c.kd_satuan, " & _
                                                                   ...
                                                                   "FROM purc.dbo.v_po_in_gudang c where (c.Program_Name <> 'aFormPONonMaterial' and c.Program_Name <> 'frmPOJasa')  " & sql & "  " & _
        ...
        sqlcmd.CommandText = "SELECT c.keterangan,c.tipe_trans, c.no_po, c.tgl_po, c.kd_supplier, c.Nama_Supplier, c.kd_valuta, c.kd_stok, c.nama_brg, c.kd_satuan, " & _
                                                               "c.qty, c.harga, c.total, c.tgl_janji_kirim,c.tgl_kirim,c.qty_in,c.qty_dev,'' as nstatus, c.ket_ppn, c.total_tax " & _
                                                               "FROM purc.dbo.v_po_in_gudang c where (c.Program_Name <> 'aFormPONonMaterial' and c.Program_Name <> 'frmPOJasa')  " & sql1 & " " & sql & " " & _
                                                               " order by c.tgl_janji_kirim asc"
        DA = New SqlClient.SqlDataAdapter(sqlcmd)
        DA.Fill(dSO, "_DpmApprv")
        GcPO_Det.DataSource = dSO.Tables("_DpmApprv")
        ...
        sqlcmd.CommandText = "SELECT c.keterangan,c.tipe_trans, c.no_po, c.tgl_po, c.kd_supplier, c.Nama_Supplier, c.kd_valuta, c.kd_stok, c.nama_brg, c.kd_satuan, " & _
                                                               "c.qty, c.harga, c.total, c.tgl_janji_kirim,c.tgl_kirim,c.qty_in,c.qty_dev,'' as nstatus, c.ket_ppn, c.total_tax " & _
                                                               "FROM purc.dbo.v_po_in_gudang c where (c.Program_Name <> 'aFormPONonMaterial' and c.Program_Name <> 'frmPOJasa')  and not tgl_kirim is null " & sql1 & " " & sql & " " & _
                                                               " and isnull(c.tgl_kirim,'" & Format(getTanggal, "yyyy/MM/dd") & "') <= c.tgl_janji_kirim  " & _
                                                               "order by c.tgl_janji_kirim asc"
        DA = New SqlClient.SqlDataAdapter(sqlcmd)
        DA.Fill(dSO, "_DpmApprv")
    ...
    Sub Load_PO_Filter()
        ...
        DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("SELECT c.keterangan,c.status_po,c.tipe_trans, c.no_po, c.tgl_po,c.ket_batal, c.kd_supplier, c.Nama_Supplier, c.kd_valuta, c.kd_stok, c.nama_brg, c.kd_satuan, " & _
                                                                    "c.qty, c.harga, c.total, c.tgl_janji_kirim, c.tgl_kirim, c.qty_in,c.qty_dev,'' as nstatus, c.ket_ppn, c.total_tax " & _
                                                                    "FROM purc.dbo.v_po_in_gudang c where c.Program_Name not in('aFormPONonMaterial','frmPOJasa') " & sql, sqlconn))
        DA.Fill(dSO, "_DpmApprv")
    ...
    Sub Load_PO_Filtern1()
        tglDari2.EditValue = vbNullString
        tglsampai2.EditValue = vbNullString
        dSO.Relations.Clear()
        If Not dSO.Tables("_DpmApprv_d") Is Nothing Then dSO.Tables("_DpmApprv_d").Clear()
        If Not dSO.Tables("_DpmApprv") Is Nothing Then dSO.Tables("_DpmApprv").Clear()
        ...
        Next
    End Sub
    Private Sub frmMonPODetil_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        prn = getParent(Me)
    ...
    Private Sub cmdRefresh_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdRefresh.Click
        ...
        If Not ds1.Tables("Po_Rekap") Is Nothing Then ds1.Tables("Po_Rekap").Clear()

        DA = New SqlDataAdapter(New SqlCommand("select * from purc.dbo.v_po_rekap where tgl_po BETWEEN CONVERT(DATE, '" & tglDari.EditValue & "', 103) AND CONVERT(DATE, '" & tglsampai.EditValue & "', 103)", sqlconn))

        DA.Fill(ds1, "Po_Rekap")
        GcPO_Det.DataSource = ds1.Tables("Po_Rekap")
        GVApprvDPM.BestFitColumns()
    End Sub

    Sub loadTable()
        If Not dSO.Tables("PO_Rekap") Is Nothing Then dSO.Tables("PO_Rekap").Clear()

        Dim sql As String
        'Dim kolom As String = ""
        If tglDari.Text = "" And tglsampai.Text <> "" Then
            sql = " and gp.tgl_po < convert(date,'" & tglsampai.EditValue & "',103) "
        ElseIf tglDari2.Text <> "" And tglsampai2.Text = "" Then
            sql = " and gp.tgl_po > convert(date,'" & tglDari.EditValue & "',103) "
        Else
            sql = " and gp.tgl_po between convert(date,'" & tglDari.EditValue & "',103) and convert(date,'" & tglsampai.EditValue & "',103) " ' "
        End If

        '        DA = New SqlDataAdapter(New SqlCommand("select pg.kd_supplier,pg.Nama_Supplier,pg.no_po,pg.tgl_po,pg.kd_stok,pg.nama_brg,pg.qty as qty_po,pg.kd_satuan, " & _
        '"pg.harga,pg.tgl_janji_kirim, pg.qty_in, pg.total, pg.ket_ppn,pg.total_tax, pg.keterangan, pg.status_po, pg.tipe_trans, pg.ket_batal, " & _
        '"pg.kd_valuta,pg.tgl_kirim,pg.qty_dev,'' as nstatus,gp.no_trans, gp.tgl_trans, gp.no_ref,sum(gp.qty_in) qty, gp.sj_supplier " & _
        '"from PURC.dbo.v_po_in_gudang pg left join PURC.dbo.v_gudang_in_purc gp on gp.no_ref=pg.no_po " & _
        '"where pg.Program_Name not in('aFormPONonMaterial','frmPOJasa')" & sql & " " & _
        '"group by pg.kd_supplier,pg.Nama_Supplier,pg.no_po,pg.tgl_po,pg.kd_stok,pg.nama_brg,pg.qty,pg.kd_satuan, " & _
        '"pg.harga,pg.tgl_janji_kirim, pg.qty_in, pg.total, pg.ket_ppn,pg.total_tax, pg.keterangan, pg.status_po, pg.tipe_trans, pg.ket_batal, " & _
        '"pg.kd_valuta,pg.tgl_kirim,pg.qty_dev,gp.no_trans, gp.tgl_trans, gp.no_ref, " & _
        '"pg.qty_in, gp.sj_supplier order by pg.tgl_po asc", sqlconn))

        DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("SELECT keterangan,status_po,tipe_trans,no_po,tgl_po,ket_batal,kd_supplier, " & _
        "Nama_Supplier,kd_valuta,kd_stok,nama_brg,kd_satuan,sum(qty) qty,harga,total,tgl_janji_kirim,tgl_kirim,qty_in,qty_dev,'' as nstatus,ket_ppn,total_tax  " & _
        "no_trans, tgl_trans, no_ref, kd_stok, kd_satuan, sj_supplier" & _
        "FROM purc.dbo.v_po_detil where Program_Name not in('aFormPONonMaterial','frmPOJasa') and tgl_trans between convert(date,'" & tglDari.EditValue & "',103) and convert(date,'" & tglsampai.EditValue & "',103)", sqlconn))

        Dim m, o As String
        Dim jml As Integer
        m = dSO.Tables("PO_Rekap").Select("tgl_janji_kirim <= tgl_kirim", "").Count

        jml = 0
        If Val(m) > 0 Then
            For i As Integer = 0 To m

                If dSO.Tables("PO_Rekap").Rows(i).Item("status_po") = "CLOSE" Then
                    jml = jml + 1
        ...
        'lbl_jmlTepat.Text = dSO.Tables("PO_Rekap").Select("tgl_janji_kirim > tgl_kirim", "").Count
        'lbl_jumlahtotal.Text = dSO.Tables("PO_Rekap").Rows.Count
        'lblOutstanding.Text = Val(lbl_jumlahtotal.Text - (Val(lbl_jmlTepat.Text) + Val(lbl_jmlTerlambat.Text)))
    End Sub
    ...
    Sub loadTable2()
        ...
        'Dim kolom As String = ""
        If tglDari.Text = "" And tglsampai.Text <> "" Then
            sql = " and d.tgl_trans < convert(date,'" & tglsampai.EditValue & "',103) "
        ElseIf tglDari2.Text <> "" And tglsampai2.Text = "" Then
            sql = " and d.tgl_trans > convert(date,'" & tglDari.EditValue & "',103) "
        Else
            sql = " and d.tgl_trans between convert(date,'" & tglDari.EditValue & "',103) and convert(date,'" & tglsampai.EditValue & "',103) " ' "
        End If

        DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("SELECT c.keterangan,c.tipe_trans, c.no_po, c.tgl_po, c.kd_supplier, c.Nama_Supplier, " & _
        "c.kd_valuta, c.kd_stok, c.nama_brg, c.kd_satuan,c.qty, c.harga, c.total, c.tgl_janji_kirim, " & _
        "c.tgl_kirim,c.qty_in,c.qty_dev,'' as nstatus, c.ket_ppn, c.total_tax,d.no_trans, d.tgl_trans, d.no_ref, sum(d.qty_in) qty, d.sj_supplier " & _
        "FROM purc.dbo.v_po_in_gudang_non c left join PURC.dbo.v_gudang_in_purc d on c.no_po=d.no_ref " & _
        "WHERE c.Program_Name not in('aFormPONonMaterial','frmPOJasa') " & sql & " " & _
        "group by c.keterangan,c.tipe_trans, c.no_po, c.tgl_po, c.kd_supplier, c.Nama_Supplier, " & _
        "c.kd_valuta, c.kd_stok, c.nama_brg, c.kd_satuan, c.qty, c.harga, c.total, c.tgl_janji_kirim, " & _
        "c.tgl_kirim,c.qty_in,c.qty_dev, c.ket_ppn, c.total_tax,d.no_trans, d.tgl_trans, d.no_ref, d.qty_in, d.sj_supplier", sqlconn))
        DA.Fill(dSO, "PO_Rekap")
        GcPO_Det.DataSource = dSO.Tables("PO_Rekap")

        'lbl_jmlTerlambat.Text = dSO.Tables("PO_Rekap").Select("tgl_janji_kirim <= tgl_kirim", "").Count
        'lbl_jmlTepat.Text = dSO.Tables("PO_Rekap").Select("tgl_janji_kirim > tgl_kirim", "").Count
        'lbl_jumlahtotal.Text = dSO.Tables("PO_Rekap").Rows.Count
        'lblOutstanding.Text = Val(lbl_jumlahtotal.Text - (Val(lbl_jmlTepat.Text) + Val(lbl_jmlTerlambat.Text)))
        ...
        For Each rw As DataRow In dSO.Tables("PO_Rekap").Rows
            rw("nstatus") = cek_status_PO(rw("no_po"))
        Next
    End Sub
    ...
    Private Sub PrintableComponentLink1_CreateReportHeaderArea(ByVal sender As System.Object, ByVal e As DevExpress.XtraPrinting.CreateAreaEventArgs) Handles PrintableComponentLink1.CreateReportHeaderArea
        Dim reportHeader As String = "LAPORAN REKAP PURCHASE ORDER"
    ...
    Private Sub SimpleButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SimpleButton1.Click
        ...
        If Not ds2.Tables("Po_Rekap") Is Nothing Then ds2.Tables("Po_Rekap").Clear()

        DA2 = New SqlDataAdapter(New SqlCommand("select * from purc.dbo.v_po_rekap where tgl_trans BETWEEN CONVERT(DATE, '" & tglDari2.EditValue & "', 103) AND CONVERT(DATE, '" & tglsampai2.EditValue & "', 103)", sqlconn))

        DA2.Fill(ds2, "Po_Rekap")
        GcPO_Det.DataSource = ds2.Tables("Po_Rekap")
        GVApprvDPM.BestFitColumns()
    End Sub
    ...
    Private Sub SimpleButton2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SimpleButton2.Click
        ...
        If Not ds3.Tables("Po_Rekap") Is Nothing Then ds3.Tables("Po_Rekap").Clear()

        DA3 = New SqlDataAdapter(New SqlCommand("select * from purc.dbo.v_po_rekap where sj_supplier = '" & lckNoSJ.Text & "'", sqlconn))

        DA3.Fill(ds3, "Po_Rekap")
        GcPO_Det.DataSource = ds3.Tables("Po_Rekap")
        GVApprvDPM.BestFitColumns()
    End Sub
    ...
    Sub Load_PO_Filter4()
        ...
        DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("SELECT c.keterangan,c.tipe_trans, c.no_po, c.tgl_po, c.kd_supplier, c.Nama_Supplier, c.kd_valuta, c.kd_stok, c.nama_brg, c.kd_satuan, " & _
                                                                    ...
                                                                    "FROM purc.dbo.v_po_in_gudang c where c.Program_Name not in('aFormPONonMaterial','frmPOJasa') " & _
                                                                        " AND c.no_po IN (" & _
                                                                        "Select no_ref from PURC.dbo.v_gudang_in_purc" & _
                                                                        " where sj_supplier like '%" & lckNoSJ.EditValue & "%' )" & _
                                                                        " AND c.kd_stok IN (" & _
                                                                        " Select kd_stok from PURC.dbo.v_gudang_in_purc" & _
    ...
    Sub Load_PO_TEST()
        ...
        "c.qty, c.harga, c.total, c.tgl_janji_kirim, c.tgl_kirim, c.qty_in,c.qty_dev,'' as nstatus, c.ket_ppn, c.total_tax " & _
        "FROM purc.dbo.v_po_in_gudang c where c.Program_Name not in('aFormPONonMaterial','frmPOJasa')" & sql, sqlconn))
        'and c.tgl_po between convert(date,'" & tglDari.EditValue & "',103) and convert(date,'" & tglsampai.EditValue & "',103)", sqlconn))
        ...
        '        DA2 = New SqlDataAdapter(New SqlCommand("select no_trans, tgl_trans, no_ref, kd_stok, kd_satuan, sum(qty_in) qty, sj_supplier " & _
        '"from PURC.dbo.v_gudang_in_purc where tgl_trans between convert(date,'" & tglDari.EditValue & "',103) and convert(date,'" & tglsampai.EditValue & "',103)" & _
        '"group by  no_trans,tgl_trans,no_ref, kd_stok, kd_satuan, sj_supplier", sqlconn))

        DA2 = New SqlDataAdapter(New SqlCommand("select no_trans, tgl_trans, no_ref, kd_stok, kd_satuan, sum(qty_in) qty, sj_supplier " & _
"from PURC.dbo.v_gudang_in_purc where tgl_trans in " & _
"(select tgl_kirim from PURC.dbo.v_po_in_gudang where tgl_po between convert(date,'" & tglDari.EditValue & "',103) and convert(date,'" & tglsampai.EditValue & "',103))" & _
"group by no_trans,tgl_trans,no_ref, kd_stok, kd_satuan, sj_supplier", sqlconn))
