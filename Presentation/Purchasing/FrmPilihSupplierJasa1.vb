...
Public Class FrmPilihSupplierJasa1
    ...
    Public Sub add_click()
        ...
        da7 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select distinct PRD.no_pr from PURC.DBO.PURC_PR_D_BARANG PRD " & _
                                                                    ...
                                                                    "where PR.rec_stat='APPROVE' and PR.jns_dpm='JASA' and CSP.no_csp is null and PR.jns_dpm='JASA'", sqlconn))
        da7.Fill(ds7, "VPR")
    ...
    Private Sub reload_table(ByVal kode As String)
        ...
        Else
            da4 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select *,(select Nama_Supplier from sif.dbo.sif_supplier where kode_supplier=kd_supplier) as Nama_Supplier, (select DISTINCT b.nama_kota FROM sif.dbo.SIF_Supplier a inner join sif.dbo.SIF_Kota b on a.Kota1=b.Kode_Kota WHERE a.kode_supplier=kd_supplier) as Kota1,(select No_Telepon1 from sif.dbo.sif_supplier where kode_supplier=kd_supplier) as No_Telepon1,(select Kontak_Person from sif.dbo.sif_supplier where kode_supplier=kd_supplier) as Kontak_Person from PURC.DBO.PURC_CHOICE_SUPP_D where No_CSP='" & kode & "' order by No_Seq asc", sqlconn))
        End If
        da4.Fill(ds4, "CSD")
        ...
        If ds3.Tables("CS").Rows.Count > 0 Then
            ...
            LEBarangD.EditValue = row.Item("kd_substok").ToString
    ...
    Public Sub edit_click()
        JenisLoad = "Edit"
        daedit = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select No_Csp as [Nomor], tgl_csp as [Tanggal], PURC.DBO.PURC_CHOICE_SUPP.no_pr as PR from PURC.DBO.PURC_CHOICE_SUPP inner join PURC.DBO.PURC_PR_D on PURC.DBO.PURC_CHOICE_SUPP.no_pr = PURC.DBO.PURC_PR_D.no_pr   where PURC.DBO.PURC_PR_D.jns_dpm='JASA'  ", sqlconn)) '<- query buat ditampilkan di LOV, pakailah alias colom biar enak dibaca
    ...
    Private Sub GV_CellValueChanged(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles GV.CellValueChanged
        ...
        If e.Column Is KodeSupplier Then
            ...
            If dr.Read() Then
                ...
                GV.GetRow(e.RowHandle).item("flagbonus") = "T"
            End If

            dr.Close()
        End If

        If e.Column Is Bonus Then
            ...
            'MessageBox.Show(GV.GetRow(e.RowHandle).item("flagbonus"))
            '    MessageBox.Show(RCHBNS.ValueChecked)
            If GV.GetRow(e.RowHandle).item("flagbonus") = "Y" Then
                GV.GetRow(e.RowHandle).item("Harga_Baru") = 0
                GV.GetRow(e.RowHandle).item("Keterangan") = "Bonus"
            ElseIf GV.GetRow(e.RowHandle).item("flagbonus") = "T" Then
                GV.GetRow(e.RowHandle).item("Harga_Baru") = 1
                GV.GetRow(e.RowHandle).item("Keterangan") = ""
            End If
            'End If
        End If
    End Sub
    ...
    Private Sub LEPR_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LEPR.EditValueChanged
        ...
        ElseIf STATE = "EDIT" Then
            da8 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select DISTINCT BRG.Kode_Barang, BRG.Nama_Barang, BRG.kd_Satuan, ISNULL(CSP.no_seq,1) from PURC.DBO.PURC_PR_D_BARANG PRD " & _
                                                                        "left join SIF.dbo.SIF_Barang BRG on PRD.kd_jasa=BRG.Kode_Barang  " & _
                                                                        "left join PURC.dbo.PURC_CHOICE_SUPP CSP on PRD.no_pr=CSP.no_pr and PRD.kd_jasa=CSP.kd_stok and PRD.kd_stok=CSP.kd_substok " & _
    ...
    Private Sub LEBarang_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LEBarang.EditValueChanged
        ...
        End If
        If STATE = "ADD" Then
            da7 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select ISNULL(P.no_urut,'1') no_urut, B.Nama_Barang,P.kd_stok,C.no_csp from purc.dbo.PURC_PR_D_BARANG P " & _
                                                                        "left join SIF.dbo.SIF_Barang B on P.kd_stok=B.Kode_Barang " & _
                                                                        "left join PURC.dbo.PURC_CHOICE_SUPP C on P.kd_jasa=C.kd_stok and P.kd_stok=C.kd_substok and p.no_pr = c.no_pr " & _
                                                                        "where P.no_pr='" & LEPR.EditValue & "' and P.kd_jasa='" & LEBarang.EditValue & "' and C.no_csp is null", sqlconn)) 'where status='PR'
        Else
            da7 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select P.no_urut, B.Nama_Barang,P.kd_stok,C.no_csp from purc.dbo.PURC_PR_D_BARANG P " & _
                                                                       "left join SIF.dbo.SIF_Barang B on P.kd_stok=B.Kode_Barang " & _
                                                                       "left join PURC.dbo.PURC_CHOICE_SUPP C on P.kd_jasa=C.kd_stok and P.kd_stok=C.kd_substok and p.no_pr = c.no_pr " & _
                                                                       "where P.no_pr='" & LEPR.EditValue & "' and P.kd_jasa='" & LEBarang.EditValue & "'", sqlconn)) 'where status='PR'
        End If
    ...
    Private Sub LEBarangD_EditValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles LEBarangD.EditValueChanged
        If JenisLoad <> "Edit" Then
            cek_noPR()
        End If
        nomor = IIf(TypeOf LEBarangD.GetColumnValue("no_urut") Is DBNull, "1", LEBarangD.GetColumnValue("no_urut"))
    End Sub
End Class