...
Public Class FrmPilihSupplier
    ...
    Public Sub add_click()
        ...
        da7 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select 	distinct PRD.no_pr from PURC.DBO.PURC_PR_D PRD " & _
                                                                    "left join PURC.dbo.PURC_CHOICE_SUPP CSP on PRD.no_pr=CSP.no_pr and PRD.kd_stok=CSP.kd_stok " & _
                                                                    "where PRD.rec_stat='APPROVE' and CSP.no_csp is null", sqlconn))
        da7.Fill(ds7, "VPR")
    ...
    Private Sub controlOff(ByVal stat As Boolean)
        txtNomor.Enabled = stat
        dtTanggal.Enabled = stat
        LEPR.Enabled = stat
        LEBarang.Enabled = stat
        txtstatus.Enabled = stat
        GC.Enabled = stat
        LESupplier.Enabled = stat
        txtKet.Enabled = stat
        txtLastPO.Enabled = stat
        txtLastTgl.Enabled = stat
        txtLastSupp.Enabled = stat
        txtLastHarga.Enabled = stat
    End Sub
    ...
    Private Sub FrmPilihSupplier_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        ElseIf txtNomor.Text <> "" Then
            da7 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select distinct no_pr from PURC.DBO.PURC_PR_D ", sqlconn)) 'where status='PR'
            da7.Fill(ds7, "VPR")
            LEPR.Properties.DataSource = ds7.Tables("VPR")
    ...
    Private Sub LEBarang_EditValueChanging(ByVal sender As Object, ByVal e As DevExpress.XtraEditors.Controls.ChangingEventArgs) Handles LEBarang.EditValueChanging
