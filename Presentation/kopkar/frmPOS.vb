...
Public Class frmPOS
    ...
    Public STATE, no_trn As String '<-- untuk mengecek status saat ini (ADD, EDIT, delete)
    ...
    Private rp_tot, byr, kmbali As Double
    ...
    Public Sub edit_click()
        ...
        DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select g.tgl_trans,g.no_trans from INV_GUDANG_OUT g ", sqlconn)) '
    ...
    Public Sub delete_click()
        If MsgBox("Melakukan Hapu Data QC ? ", MsgBoxStyle.Exclamation + MsgBoxStyle.YesNo, "Hapus Data/Pembatalan?") = MsgBoxResult.Yes Then
            STATE = "DELETE"
    ...
    Public Sub simpan(Optional ByRef isDelete As Boolean = False)
        ...
        If Not isDelete Then
            ...
            'End If
            If STATE = "ADD" Then
                no_trn = getNoTrans("POS", getTanggal(BTRANS), BTRANS)
            ...
            ElseIf STATE = "EDIT" Then
                ...
                'row.Item("gudang_tujuan") = Look_Sales.EditValue
                row.Item("Program_Name") = Me.Name
                lb_no_trn.Text = row.Item("no_trans")
            End If
    ...
    Public Sub cetak_click()
        callReport(App_Path() & "\report\BK_Loading.rpt", "", "no_mtsi=" & lb_no_trn.Text, False, True)
    End Sub
    ...
    Private Sub LoadStruktur(ByVal kode As String)
        ...
        If dSO.Tables("_bke").Rows.Count > 0 Then
            ...
            lb_no_trn.Text = rw("no_trans").ToString
    ...
    Private Sub hitung_ulang()
        rp_tot = 0
        For Each rw_hist In dSO1.Tables("_bke_d").Select("qty_out>0")
            'If rw_hist("rec_lunas") = 1 Then
            rp_tot += rw_hist.Item("rp_trans")
            ...
            'End If
        Next
        lb_rptotal.Text = rp_tot.ToString("c", New Globalization.CultureInfo("id-ID", False))
    End Sub
    ...
    Private Sub GvBK_CellValueChanged(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles GvBK.CellValueChanged
        If e.Column Is qty_out Then
            If GvBK.GetRow(e.RowHandle).item("akhir_qty_onstok") <= GvBK.GetRow(e.RowHandle).item("qty_out") Then
                MsgBox("Qty Out melebihi stok gudang", MsgBoxStyle.Critical)
                Exit Sub
            Else
                GvBK.GetRow(e.RowHandle).item("rp_trans") = GvBK.GetRow(e.RowHandle).item("qty_out") * GvBK.GetRow(e.RowHandle).item("harga")
                'GvBK.GetRow(e.RowHandle).item("qty_sisa") = GvBK.GetRow(e.RowHandle).item("akhir_qty_onstok") - GvBK.GetRow(e.RowHandle).item("qty_out")
    ...
    Private Sub dt_tgl_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        If e.KeyCode = Keys.Enter Then
            'TxbBarcode.Focus()
            txtcari.Focus()
        End If
    ...
    Private Sub GvBK_RowUpdated(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.RowObjectEventArgs) Handles GvBK.RowUpdated
        rp_tot = 0
        For Each rw_hist In dSO1.Tables("_bke_d").Select("qty_out>0")
            'If rw_hist("rec_lunas") = 1 Then
            rp_tot += rw_hist.Item("rp_trans")
    ...
    Private Sub TE_Bayar_EditValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles TE_Bayar.EditValueChanged
        byr = TE_Bayar.EditValue
        kmbali = rp_tot - byr
        lb_kembali.Text = kmbali.ToString("c", New Globalization.CultureInfo("id-ID", False))
    End Sub

   
    Private Sub TE_Bayar_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles TE_Bayar.KeyDown
        If e.KeyCode = Keys.Enter Then
            byr = TE_Bayar.EditValue
            kmbali = rp_tot - byr
            lb_kembali.Text = kmbali.ToString("c", New Globalization.CultureInfo("id-ID", False))
