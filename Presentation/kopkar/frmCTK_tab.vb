...
Public Class frmCTK_tab
    ...
    Public Sub cetak_click()
        ...
        ViewReport(App_Path() & "\report\cetak_tabungan.rpt", "", "id=" & txbKdBrg.Text)
        'Else
    ...
    Private Sub txtcari_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtcari.KeyDown
        ...
        If e.KeyCode = Keys.Enter Then
            DA_keypres = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select nasabah_id,Nama from KOPKAR.dbo.anggota where Nama ='" & txtcari.Text & "'", sqlconn))
    ...
    Private Sub frmCTK_tab_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        connect()
        prn = getParent(Me)
        prn.btnAdd_Click(prn.btnAdd, New EventArgs)
        da_vbrg = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select nasabah_id,nama from KOPKAR.dbo.anggota", sqlconn))
        da_vbrg.Fill(ds_vbrg, "_vbrg")
        'LookupBlthn.Properties.ValueMember = "kd_stok"
        'LookupBlthn.Properties.DisplayMember = "Nama_Barang"
        txtcari.AutoCompleteCustomSource.AddRange((From row As Object In ds_vbrg.Tables("_vbrg").Rows.Cast(Of DataRow)() Select CStr(row("Nama"))).ToArray())
    End Sub
End Class