...
Public Class frmMon_rekapOutstanding
    ...
    Private Sub PrintableComponentLink1_CreateReportFooterArea(ByVal sender As Object, ByVal e As DevExpress.XtraPrinting.CreateAreaEventArgs) Handles PrintableComponentLink1.CreateReportFooterArea
        ...
        Dim reportFooter8 As String = "Jumlah Rupiah"
        'Dim reportFooter11 As String = "Total Debet"
        Dim reportFooter9 As String = gv_byr_piu.Columns("nomer").SummaryText
        Dim reportFooter10 As String = gv_byr_piu.Columns("total_angs").SummaryText
    ...
    Public Sub LoadStruktur()
        ...
        Dim sql As String = "select 0 as nomer,jenis,sum(jml_angsuran) as total_angs from kredit where status_lunas='BELUM' AND stat_approve >=5 and jenis not in('PTAT','PBNS','PTHR','PKHS') group by jenis  "
        DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand(sql & sqltgl, sqlconn))
        DA.Fill(dSO, "_krd")

        DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select 0 as nomer,jenis,sum(jml_angsuran) as total_angs from retail where status_lunas='BELUM' AND stat_approve >=5 group by jenis" & sqltgl, sqlconn))
        DA.Fill(dSO, "_krd")
