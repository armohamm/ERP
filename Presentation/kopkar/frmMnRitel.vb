...
Public Class frmMnRitel
    ...
    Public Sub LoadStruktur()
        ...
        DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select *,0 as nomer from retail where STATUS_LUNAS='BELUM' order by jenis", sqlconn)) '1 anggota AND stat_anggota=1  ditambpilkan smua dulu
        DA.Fill(dSO, "_krd")
    ...
    Private Sub frmMDebetExt_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        DALOOKUP = New SqlDataAdapter(New SqlCommand("select kode,Nama from m_jns_upload where tipe_trans='RTL' ", sqlconn))
    ...
    Private Sub GV_export_CellValueChanged(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles GV_export.CellValueChanged
        If e.Column Is sisa_angs Then
            'GV_export.GetRow(e.RowHandle).item("sisa_pokok") = GV_export.GetRow(e.RowHandle).item("sisa_pokok") - GV_export.GetRow(e.RowHandle).item("jml_angsuran")
