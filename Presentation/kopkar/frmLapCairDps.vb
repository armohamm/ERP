...
Public Class frmLapCairDps
    ...
    Public Sub cetak_click()
        ...
        print_gc1(GcINV_Ret, "                                                                LAPORAN PENCAIRAN DEPOSITO    " & vbCrLf & _
                 "     ")
    ...
    Public Sub LoadStruktur()
        ...
        Dim sql As String = "select *,0 as nomer from v_mtabung_trans where KODE_TRANS='02' or KODE_TRANS='07' and " 'where kode 02=pencairan
        DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand(sql & sqltgl, sqlconn))
        Clipboard.SetText(sql & sqltgl)
        DA.Fill(dSO, "_krd")
    ...
    Private Sub frmLapCairDps_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        connect()
        prn = getParent(Me)
        'LoadStruktur()

        DALOOKUP = New SqlDataAdapter(New SqlCommand("select KODE_TRANS,DESKRIPSI_TRANS from kodetranstabungan", sqlconn))
        DALOOKUP.Fill(ds_cari, "_lookup1")
        RLnama.DataSource = ds_cari.Tables("_lookup1")
        RLnama.ValueMember = "KODE_TRANS"
        RLnama.DisplayMember = "DESKRIPSI_TRANS"
