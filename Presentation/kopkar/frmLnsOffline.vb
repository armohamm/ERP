...
Public Class frmLnsOffline
    ...
    Private bunga, pokok_akhir, ang_baru, bts_adm, bts1, bts2, pokok, m_prv, pokok_asli, pot_min, bsr_pjm, bg_br, jml_rec, penalti, jml_rec_bunga, sisa_pk_lama, plafond, angs, denda, t_bunga, total, recov, tpot, pk_br, t_pk_br, r_bg_jln, prv, r_adm, adm_cek, adm, r_pen, r_jml_ut, nilai, prv_r, bg_lama, bg_jln, j_bg_lama, t_pot, pk_lama, pk_akhir, pot, pot_bns, pot_thr, pot_tat, angs_asli, pk_akhir_asli, sisa_pk_br, t_pk_baru As Double
    ...
    Public Sub LoadStruktur(ByVal kode As String)
        ...
        If dSO.Tables("_krd").Rows.Count > 0 Then
            ...
            TE_pokok.EditValue = rwm("JML_POKOK").ToString
        ...
        If Not isDelete Then
            If STATE = "ADD" Then
                ...
                If pot_bns > 0 Then 'BONUS
                    ...
                    'row1.Item("no_kredit") = ntran_lama
    ...
    Private Sub cek_recovery()
        ...
        If sqlcek.HasRows Then
            ...
            ElseIf bln_dbt = 1 Then
                ...
                r_bg_jln = j_bg_lama + (((bsr_pjm * (bunga / 100)) / jm_bl_skrg) * jm_hr_dbt)
            ElseIf bln_dbt > 1 Then
            ...
            nett_asli = bsr_pjm - (jml_rec + prv + r_adm + r_pen + r_bg_jln)
