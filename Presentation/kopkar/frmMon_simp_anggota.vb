...
Public Class frmMon_simp_anggota
    ...
    Public Sub LoadStruktur()
        ...
        ElseIf tglDari.EditValue <> vbNullString And tglsampai.EditValue = vbNullString Then
            sqltgl = " where tgl_masuk > convert(date,'" & tglDari.Text & "',103)"
        ElseIf tglDari.EditValue = vbNullString And tglsampai.EditValue <> vbNullString Then
            sqltgl = "where tgl_masuk < convert(date,'" & tglsampai.Text & "',103)"
        ElseIf tglDari.EditValue <> vbNullString And tglsampai.EditValue <> vbNullString Then
            sqltgl = "where tgl_masuk between convert(date,'" & tglDari.Text & "',103) and convert(date,'" & tglsampai.Text & "',103)"
        ...
        DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("SELECT 0 as nomer,* from tab_wajib  " & sqltgl & " order by tgl_masuk desc ", sqlconn))
    ...
    Private Sub gv_byr_piu_CellValueChanged(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles gv_byr_piu.CellValueChanged
        If e.Column Is saldo_akhir Then
            gv_byr_piu.GetRow(e.RowHandle).item("last_update_by") = namauser
            gv_byr_piu.GetRow(e.RowHandle).item("tgl_bayar") = Today
        End If
    End Sub
    ...
    Private Sub gv_byr_piu_InitNewRow(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Grid.InitNewRowEventArgs) Handles gv_byr_piu.InitNewRow
        gv_byr_piu.GetRow(e.RowHandle).item("no_tab") = "yayak"
        gv_byr_piu.GetRow(e.RowHandle).item("rec_stat") = 1
        gv_byr_piu.GetRow(e.RowHandle).item("debet_pokok") = 1
    End Sub
End Class