...
Public Class frmMon_rekapUpload
    ...
    Private dSO, dSO1, ds_tbh, ds_cari3, dso2, ds_cari, ds_cari1, ds_gdg1, ds_gdg2, dsLookUp As New DataSet
    Private Buildernya As New SqlClient.SqlCommandBuilder
    Private row As DataRow
    Private DA, DA2, DALOOKUP, DALOOKUP1, da_bln, DA_gdg1, DA_gdg2 As SqlClient.SqlDataAdapter
    ...
    Private Sub PrintableComponentLink1_CreateReportFooterArea(ByVal sender As Object, ByVal e As DevExpress.XtraPrinting.CreateAreaEventArgs) Handles PrintableComponentLink1.CreateReportFooterArea
        ...
        Dim reportFooter7 As String = "Jumlah Debet"
        Dim reportFooter8 As String = "Jumlah Rekening"
        'Dim reportFooter11 As String = "Total Debet"
        Dim reportFooter9 As String = gv_byr_piu.Columns("total_angs").SummaryText
        Dim reportFooter10 As String = gv_byr_piu.Columns("jml_rek").SummaryText
    ...
    Public Sub LoadStruktur()
        ...
        '    sqltgl = " where blthn='" & lookupBulanAwal.EditValue & "'"
        ...
        DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select 0 as nomer,'    ' as jenis,sum(iuran) as total_angs,count(kd_upload) as jml_acc,count(kd_upload) as jml_rek,sum(saldo_akhir) as saldo from tab_wajib where rec_stat=1 group by kd_upload " & _
" UNION ALL " & _
" select 0 as nomer,'PKOK' as jenis,sum(jml_pokok) as total_angs,count(kd_upload) as jml_acc,0 as jml_rek,sum(saldo_akhir) as saldo from tab_wajib where rec_stat=1 and debet_pokok=1 group by kd_spokok  " & _
" UNION ALL " & _
" select 0 as nomer,j.prefix as jenis,sum(t.iuran) as total_angs,count(t.JENIS_TABUNGAN) as jml_acc,count(t.JENIS_TABUNGAN) as jml_rek,sum(SALDO_AKHIR) as saldo from tabung t  " & _
" left join kodejenistabungan j on t.JENIS_TABUNGAN=j.KODE_JENIS_TABUNGAN " & _
" where JENIS_TABUNGAN <>'005' group by t.JENIS_TABUNGAN,j.prefix " & _
" UNION ALL " & _
" select 0 as nomer,jenis,sum(jml_angsuran) as total_angs,count(jenis) as jml_acc,count(jenis) as jml_rek,sum(sisa_pokok) as saldo from kredit where status_lunas='BELUM' AND stat_approve >=5 and jenis not in('PTAT','PBNS','PTHR','PKHS') group by jenis  " & _
" UNION ALL " & _
" select 0 as nomer,jenis,sum(jml_angsuran) as total_angs,count(jenis) as jml_acc,count(jenis) as jml_rek,sum(sisa_pokok) as saldo from retail where status_lunas='BELUM' AND stat_approve >=5 AND sudah_upload=1 group by jenis  " & _
" UNION ALL " & _
" select 0 as nomer,jenis,SUM(debet) as debet,count(jenis) as jml_acc,count(jenis) as jml_rek,0 as saldo from m_ssb group by jenis " & _
" UNION ALL " & _
" select 0 as nomer,kd_grup as jenis,sum(total_debet) as total_angs,count(kd_grup) as jml_acc,count(kd_grup) as jml_rek,0 as saldo from m_debet_ext group by kd_grup ", sqlconn))
        DA.Fill(dSO, "_krd")


        'DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select 0 as nomer,kd_grup as jenis,sum(total_debet) as total_angs,sum(ke_koperasi) as ke_koperasi from m_debet_ext group by kd_grup" & sqltgl, sqlconn))
        'DA.Fill(dSO, "_krd") 'ext
    ...
    Private Sub frmMon_ritel_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        DALOOKUP1 = New SqlDataAdapter(New SqlCommand("select kode,Nama from m_jns_upload union all select kd_grup,nama_grup from m_grup_debet_ext", sqlconn))
