...
Public Class frmMon_agt_kluar
    ...
    Private Sub PrintableComponentLink1_CreateReportFooterArea(ByVal sender As Object, ByVal e As DevExpress.XtraPrinting.CreateAreaEventArgs) Handles PrintableComponentLink1.CreateReportFooterArea
        ...
        'Dim reportFooter9 As String = gv_byr_piu.Columns("JML_BUNGA").SummaryText
        'Dim reportFooter10 As String = gv_byr_piu.Columns("JML_POKOK").SummaryText
    ...
    Public Sub LoadStruktur()
        ...
        ElseIf tglDari.EditValue <> vbNullString And tglsampai.EditValue = vbNullString Then
            sqltgl = " AND tgl_keluar > convert(date,'" & tglDari.Text & "',103)"
        ElseIf tglDari.EditValue = vbNullString And tglsampai.EditValue <> vbNullString Then
            sqltgl = " AND tgl_keluar < convert(date,'" & tglsampai.Text & "',103)"
        ElseIf tglDari.EditValue <> vbNullString And tglsampai.EditValue <> vbNullString Then
            sqltgl = " AND tgl_keluar between convert(date,'" & tglDari.Text & "',103) and convert(date,'" & tglsampai.Text & "',103)"
        ElseIf te_NIP.EditValue <> vbNullString Then
            sqltgl = " AND nasabah_id ='" & te_NIP.EditValue & "'"
        ...
        DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select 0 as nomer,* from anggota where Rec_stat=0 " & sqltgl & "   order by tgl_masuk desc ", sqlconn))
