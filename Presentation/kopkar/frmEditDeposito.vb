...
Public Class frmEditDeposito
    ...
    Public Sub Cari()
        ...
        If Lue_bilyet.EditValue = Nothing Then
            Exit Sub
        End If
        DA_gdg1 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("SELECT * FROM TABUNG T LEFT JOIN ANGGOTA A ON A.nasabah_id=T.NASABAH_ID where JENIS_TABUNGAN='005' AND t.NO_TABUNGAN='" & Lue_bilyet.EditValue & "'", sqlconn)) '<- query buat ditampilkan di LOV, pakailah alias colom biar enak dibaca
        ...
        If Not row Is Nothing Then
            ...
            lb1_nip.Text = row("nasabah_id")
            lb1_almt.Text = IIf(row.IsNull("Almt_asal"), "", row.Item("Almt_asal"))  'row("Almt_asal")
            lb_kota.Text = IIf(row.IsNull("nama_kota"), "", row.Item("nama_kota")) 'row("nama_kota")
            id_anggota = row("nasabah_id")
            lb1_hp.Text = IIf(row.IsNull("telp_skrg"), "", row.Item("telp_skrg")) 'row("telp_skrg")
            lb1_telp1.Text = IIf(row.IsNull("telp_asal"), "", row.Item("telp_asal")) 'row("telp_skrg");row("telp_asal")
            'lb1_umur.Text = IIf(row.IsNull("Umur"), "", row.Item("Umur")) 'row("Umur")
            lb1_nama.Text = IIf(row.IsNull("Nama"), "", row.Item("Nama")) 'row("Nama")
            tx_rek.Text = row("no_rek")
            dtp_reg.Text = row("TGL_REGISTRASI")
            tx_nominal.Text = row("SALDO_AWAL")
            tx_nama_bank.Text = IIf(row.IsNull("nama_bank"), "", row.Item("nama_bank")) 'row("nama_bank")
            tx_no_tbg.Text = row("NO_TABUNGAN")
            tx_bunga.Text = row("bunga")
            bg1 = row("bunga")
            bsr_pjm = row("SALDO_AWAL")

            t_bg = bg1 / 100
            bunga = (bsr_pjm * t_bg) / 12
            't_pajak = bunga * (pajak / 100)
            ' bunga_net = bunga - t_pajak
            lb1_gros.Text = bunga.ToString("c", New Globalization.CultureInfo("id-ID", False))

            LoadStruktur(row("NO_TABUNGAN"))
        End If
        'TE_Setoran.Focus()
    End Sub
    ...
    Public Sub LoadStruktur(ByVal kode As String)
        ...
        If dSO.Tables("_krd").Rows.Count > 0 Then
            ...
            'lb1_nip.Text = row("nasabah_id")
            'lb1_almt.Text = row("Almt_asal")
            'lb_kota.Text = row("nama_kota")
            'id_anggota = row("nasabah_id")
            'lb1_hp.Text = row("telp_skrg")
            'lb1_telp1.Text = row("telp_asal")
            'lb1_umur.Text = row("Umur")
            'lb1_nama.Text = row("Nama")
            'tx_rek.Text = row("REKBCA1")
            'dtp_reg.Text = row("TGL_REGISTRASI")
            'tx_nominal.Text = row("SALDO_AWAL")
            'tx_nama_bank.Text = row("nama_bank")
            'tx_no_tbg.Text = row("NO_TABUNGAN")
            'tx_bunga.Text = row("bunga")

            tx_no_tbg.Text = rwm.Item("NO_TABUNGAN").ToString
            'lb1_nama.Text = rwm.Item("NASABAH_ID").ToString
            lb1_nip.Text = rwm.Item("NASABAH_ID").ToString
            'lb_alamat.Text = rwm.Item("").ToString
            'lb_kota.Text = rwm.Item("").ToString
            'lb_telp.Text = rwm.Item("").ToString
            Lue_jns_tb.EditValue = IIf(rwm.IsNull("JENIS_TABUNGAN"), 0, rwm.Item("JENIS_TABUNGAN"))
            'LU_bunga_dps.EditValue = rwm.Item("").ToString
            tx_nominal.Text = IIf(rwm.IsNull("SALDO_AWAL"), 0, rwm.Item("SALDO_AWAL"))
            dtp_reg.Text = rwm.Item("TGL_REGISTRASI").ToString
            'dtp_sd.Text = IIf(rwm.IsNull("TGL_AKHIR"), Today, rwm.Item("TGL_AKHIR").ToString)
            tx_bunga.Text = rwm.Item("bunga").ToString
            ...
            'End If
            lb1_pajak.Text = rwm.Item("rp_pajak_bulanan").ToString
            'lb1_t_bunga.Text = rwm.Item("bunga").ToString
            bg1 = rwm.Item("bunga").ToString
            t_bg = bg1 / 100
            bunga = (bsr_pjm * t_bg) / 12
            't_pajak = bunga * (pajak / 100)
            ' bunga_net = bunga - t_pajak
    ...
    Public Sub save_click()
        If Lue_bilyet.EditValue = vbNullString Then
    ...
    Public Sub simpan()
        ...
        '        row.Item("bunga") = tx_bg_khus.Text
        '        row.Item("status_anggota") = "KHUSUS"
        '    Else
        '        row.Item("status_anggota") = "BIASA"
        '    End If
        ...
        'End If
        row.Item("tipe_trans") = "JKM-KBB-07"
    ...
    Public Function getNextNumber() As String
        ...
        Dim sqlDR As SqlDataReader = New SqlCommand("select MAX( cast(substring(NO_TABUNGAN,3,5) as numeric))+1 from tabung where NO_TABUNGAN like '" & awalan & "%'", sqlconn).ExecuteReader
        sqlDR.Read()
        temp = ""
    ...
    Private Sub frmDaftarDeposito_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        connect()
        prn = getParent(Me)
        ...
        DA_gdg2 = New SqlDataAdapter(New SqlCommand("SELECT t.NO_TABUNGAN,A.Nama FROM TABUNG T LEFT JOIN ANGGOTA A ON A.nasabah_id=T.NASABAH_ID where T.JENIS_TABUNGAN='005'", sqlconn))
        DA_gdg2.Fill(ds_gdg2, "_bilyet")
        Lue_bilyet.Properties.DataSource = ds_gdg2.Tables("_bilyet")
        Lue_bilyet.Properties.ValueMember = "NO_TABUNGAN"
        Lue_bilyet.Properties.DisplayMember = "NO_TABUNGAN"
    ...
    'Private Sub LU_bunga_dps_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LU_bunga_dps.EditValueChanged
    '    Dim i As Integer
    ...
    'Private Sub TE_Setoran_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TE_Setoran.EditValueChanged
    '    dtp_sd.Value = dtp_tgl.Value.AddYears(1)
    'End Sub

    'Private Sub dtp_tgl_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles dtp_tgl.KeyDown
    '    If e.KeyCode = Keys.Enter Then
    '        t_rek.Focus()
    '    End If
    'End Sub

    'Private Sub dtp_tgl_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles dtp_tgl.ValueChanged
    '    dtp_sd.Value = dtp_tgl.Value.AddYears(1)
    'End Sub

    'Private Sub tx_nama_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs)
    '    If e.KeyCode = Keys.Enter Then
    '        Cari()
    '    End If
    'End Sub
    ...
    'Private Sub txb_no_tbg_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txb_no_tbg.KeyDown
    '    If e.KeyCode = Keys.Enter Then
    ...
    'Private Sub t_rek_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles t_rek.KeyDown
    '    If e.KeyCode = Keys.Enter Then
    ...
    Private Sub Lue_bilyet_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Lue_bilyet.KeyDown
        If e.KeyCode = Keys.Enter Then
            Cari()
        End If
    End Sub
