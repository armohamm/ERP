...
'= Project: Dinasty group american pillow @2011  ? what?              =
...
Public Class frmMnTab
    ...
    Public Sub LoadStruktur()
        ...
        DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select *,0 as nomer from tabung where JENIS_TABUNGAN not in ('005') order by TGL_REGISTRASI DESC", sqlconn))
        DA.Fill(dSO, "_krd")
    ...
    Private Sub frmMDebetExt_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        DALOOKUP = New SqlDataAdapter(New SqlCommand("select KODE_JENIS_TABUNGAN,prefix,DESKRIPSI_JENIS_TABUNGAN as nama from kodejenistabungan ", sqlconn))
        DALOOKUP.Fill(ds_cari, "_lookup1")
        RL_grup.DataSource = ds_cari.Tables("_lookup1")
        RL_grup.ValueMember = "KODE_JENIS_TABUNGAN"
    ...
    Private Sub GV_export_InitNewRow(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Grid.InitNewRowEventArgs) Handles GV_export.InitNewRow
        ...
        GV_export.GetRow(e.RowHandle).item("NO_TABUNGAN") = "yayak"
        GV_export.GetRow(e.RowHandle).item("STATUS_AKTIF") = "AKTIF"
    ...
    Private Sub GV_export_CellValueChanged(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles GV_export.CellValueChanged
        If e.Column Is SALDO_AKHIR Then
            GV_export.GetRow(e.RowHandle).item("SALDO_AWAL") = GV_export.GetRow(e.RowHandle).item("SALDO_AKHIR")
            GV_export.GetRow(e.RowHandle).item("Last_Update_By") = namauser
            GV_export.GetRow(e.RowHandle).item("Last_Update") = Today
        End If
        'If e.Column Is sisa_angs Then
        '    'GV_export.GetRow(e.RowHandle).item("Last_Update") = GV_export.GetRow(e.RowHandle).item("sisa_pokok") - GV_export.GetRow(e.RowHandle).item("jml_angsuran")
        ...
        'End If
        If e.Column Is JENIS_TABUNGAN Then
            Dim drows As DataRow() = ds_cari.Tables("_lookup1").Select("KODE_JENIS_TABUNGAN='" & e.Value & "'")
            GV_export.GetRow(e.RowHandle).item("KODE_CAB") = drows(0).Item("prefix")
        End If
    End Sub
