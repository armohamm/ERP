...
Public Class frmDaftarDeposito
    ...
    Private bunga, pajak, t_pajak, bunga_net, tempo, pokok, bsr_pjm, angs, denda, t_bunga, total, recov, asr, bg1, bg2, bg3, bg4, bts1, bts2, bts3, bts4, t_bg As Double
    Private BTRAN As SqlTransaction
    Private ntran_cek, ket, awalan, No_bilyet As String
    ...
    Public Sub add_click()
        ...
        txb_bank.Text = "BCA"
        No_bilyet = notran() 'generate sbg info
        txb_no_tbg.Text = No_bilyet
    ...
    Public Sub save_click()
        If LUE_jnsTab.EditValue = vbNullString Or txb_no_tbg.Text = "" Or TE_Setoran.EditValue = vbNullString Then
            MsgBox("Jenis Tabungan/NO Tabungan/Setoran awal Belum dipilih/di set", MsgBoxStyle.Exclamation)
            Exit Sub
        End If
    ...
    Public Sub simpan()
        ...
        If STATE = "ADD" Then
            ...
            row.Item("kabag_approve") = 0 'kabag_approve=0 belum
            If txb_no_tbg.Text = "" Then
                MsgBox("NO Bilyet TIDAK BOLEH KOSONG")
            End If
            ...
            '    End If
            'Else
            '    row.Item("NO_TABUNGAN") = No_bilyet
            '    txb_no_tbg.Text = No_bilyet
            'End If
    ...
    Public Sub Cari()
        ...
        DA_gdg1 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select a.nasabah_id,isnull(a.telp_skrg,'') as telp_skrg,Nama as [Nama],a.jns_kel as [Jenis_kelamin],a.job_id,isnull(a.Almt_asal,'')as Almt_asal,CONVERT(int,ROUND(DATEDIFF(hour, isnull(convert(date,Tgl_lahir,1),''),GETDATE())/8766.0,0)) AS Umur, * from anggota a where a.nasabah_id='" & LUE_nama.EditValue & "'", sqlconn)) '<- query buat ditampilkan di LOV, pakailah alias colom biar enak dibaca
        ...
        If Not row Is Nothing Then
            ...
            lb_telp.Text = row("telp_skrg")
            t_rek.Text = row("REKBCA1")
            lb_umur.Text = row("Umur")
            lb_nama.Text = row("Nama")
            lb_stat_agt.Text = IIf(row.IsNull("status_anggota"), "BIASA", row("status_anggota").ToString) 'row("REKBCA2") row("status_anggota")
            If lb_stat_agt.Text = "KHUSUS" Then
    ...
    Public Sub hitung_dpst()
        ...
        ElseIf TE_Setoran.EditValue >= bts2 And TE_Setoran.EditValue < bts3 Then
            ...
            lb1_pajak.Text = t_pajak.ToString("c", New Globalization.CultureInfo("id-ID", False))
            lb1_t_bunga.Text = bunga_net.ToString("c", New Globalization.CultureInfo("id-ID", False))
        ElseIf TE_Setoran.EditValue >= bts3 And TE_Setoran.EditValue < bts4 Then
            If cb_bg.Checked = True Then
                t_bg = 12 / 100
            Else
                t_bg = bg3 / 100
            ...
            'MsgBox(bunga)
            'End If
        ElseIf TE_Setoran.EditValue >= bts4 Then
            If cb_bg.Checked = True Then
                t_bg = 12 / 100
            Else
                t_bg = bg4 / 100
    ...
    Private Sub LU_bunga_dps_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LU_bunga_dps.EditValueChanged
        ...
        bts4 = dr(i).Item("batas4_min")
        bg4 = dr(i).Item("bunga4")
