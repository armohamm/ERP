...
Public Class frmPO
    ...
    Public Sub cetak_click()
        ...
        If txtNoPO.Text = "" Then
            ...
            Exit Sub
        End If
        callReport(App_Path() & "\report\rptPOM.rpt", "", "&no_pom=" & txtNoPO.Text, False, False)
    ...
    Public Sub cancel_click()
        ...
        da2 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("SELECT Kode_Barang, Nama_Barang,Kd_Satuan,spek_brg FROM SIF.dbo.SIF_Barang where rec_stat='Y'", sqlconn))
        da2.Fill(ds, "VBarangq")
        repbarang.DataSource = ds.Tables("VBarangq")
    ...
    Private Sub Init_Bayar()
        ...
        RentangBayar.EditValue = "1" 'set seminggu dulu
    End Sub
    Public Sub View_PR2()
        If Not ds.Tables("_PR") Is Nothing Then ds.Tables("_PR").Clear()
        da7 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select no_pr from PURC.dbo.PURC_DPM ", sqlconn))
        da7.Fill(ds, "_PR")
        LEPR.Properties.DataSource = ds.Tables("_PR")
        LEPR.Properties.ValueMember = "no_pr"
        LEPR.Properties.DisplayMember = "no_pr"
    End Sub
    ...
    Public Sub edit_click()
        ...
        If Not frmCari.row Is Nothing Then '<-- jika row ada yg dipilih
            ...
            If frmCari.row.Item("Status").ToString = "APPROVE" Then
                ...
                Me.Enabled = True
                controlOff(True)
                GV_PR.OptionsBehavior.ReadOnly = True
            ...
            ElseIf frmCari.row.Item("Status").ToString <> "APPROVE" Then
                ...
                Me.Enabled = True
                controlOff(False)
                GV_PR.OptionsBehavior.ReadOnly = False
    ...
    Public Sub save_click(Optional ByRef isDelete As Boolean = False)
        If MsgBox("Apakah Status PO Sudah Benar??", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
            ...
            If Not isDelete Then
                ...
                row.Item("prosen_diskon") = 0
                row.Item("jml_diskon") = IIf(txtJmlDiskon1.Text = "", 0, txtJmlDiskon1.Text)
                ...
                'Dim j As Integer = GV_PR.RowCount + 1
    ...
    Private Sub controlOff(ByVal stat As Boolean)
        ...
        GV_PR.OptionsBehavior.ReadOnly = stat
    ...
    Private Sub reload_table(ByVal kode As String)
        ...
        'GV_PR.Columns("rec_stat").Visible = False
    ...
    Private Sub frmPO_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not ds.Tables("VBarangq") Is Nothing Then ds.Tables("VBarangq").Clear()
        da2 = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("SELECT b.Kode_Barang, b.Nama_Barang,h.Harga_Rupiah,h.harga_jual,b.Kd_Satuan,b.spek_brg FROM SIF.dbo.SIF_Barang b inner join SIF.dbo.SIF_Harga h on b.Kode_Barang=h.Kode_Barang where b.rec_stat='Y'", sqlconn))
        da2.Fill(ds, "VBarangq")
        repbarang.DataSource = ds.Tables("VBarangq")
    ...
    Private Sub V_Supplier()
        ...
        LESupllier.EditValue = "S00001" 'dibuat default Jaleca utk depo
    ...
    Private Sub LEPR_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LEPR.EditValueChanged
        If STATE = "ADD" Then
            ...
            Dim dr As SqlDataReader = New SqlCommand(" select *,0 as prosen_diskon,0 as jml_diskon,0 as harga_kini  from PURC.dbo.PURC_DPM p inner join PURC.dbo.PURC_DPM_D d on p.no_pr=d.no_pr inner join SIF.dbo.SIF_Barang b on d.Kd_Stok=b.Kode_Barang where p.no_pr='" & LEPR.EditValue & "'", sqlconn).ExecuteReader
            ...
            While dr.Read
                ...
                rw.Item("total") = isNull(dr.Item("harga_kini"), 0) * isNull(dr.Item("qty"), 0)
        ...
        GV_PR.FocusedColumn = GV_PR.VisibleColumns(5)
    End Sub
    ...
    Private Sub GV_PR_InitNewRow(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Grid.InitNewRowEventArgs) Handles GV_PR.InitNewRow
        GV_PR.GetRow(e.RowHandle).item("no_seq") = GV_PR.RowCount + 1
        GV_PR.GetRow(e.RowHandle).item("harga") = 0
        GV_PR.GetRow(e.RowHandle).item("qty") = 0
        'GV_PR.GetRow(e.RowHandle).item("prosen_diskon") = 0
        'GV_PR.GetRow(e.RowHandle).item("jml_diskon") = 0
        'GV_PR.GetRow(e.RowHandle).item("total") = 0
    End Sub

    Private Sub GV_PR_CellValueChanged(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles GV_PR.CellValueChanged
        ...
        If e.Column Is kd_stok Then
            ...
            'GV_PR.GetRow(e.RowHandle).item("kd_stok") = rw("Kode_Barang")
            GV_PR.GetRow(e.RowHandle).item("kd_satuan") = rw("Kd_Satuan")
            GV_PR.GetRow(e.RowHandle).item("spek_brg") = rw("spek_brg")
            GV_PR.GetRow(e.RowHandle).item("harga") = rw("Harga_Rupiah")
            GV_PR.GetRow(e.RowHandle).item("qty") = 0
            GV_PR.FocusedColumn = GV_PR.VisibleColumns(3)
        End If

        Dim hasilX, hasilY As Double
        If e.Column Is harga Or e.Column Is qty Then
            If GV_PR.GetRow(e.RowHandle).item("qty").ToString = "" Then
                GV_PR.SetRowCellValue(e.RowHandle, "qty", 0)
            End If
            Dim subharga As Decimal = GV_PR.GetRow(e.RowHandle).item("harga").ToString * GV_PR.GetRow(e.RowHandle).item("qty").ToString
            GV_PR.GetRow(e.RowHandle).item("total") = GV_PR.GetRow(e.RowHandle).item("qty").ToString * GV_PR.GetRow(e.RowHandle).item("harga").ToString
            'GV_PR.GetRow(e.RowHandle).item("jml_val_trans") = GV_PR.GetRow(e.RowHandle).item("jml_rp_trans")
            'GV_PR.GetRow(e.RowHandle).item("total") = GV_PR.GetRow(e.RowHandle).item("harga").ToString * GV_PR.GetRow(e.RowHandle).item("qty").ToString
            ...
            '    GV.GetRow(e.RowHandle).item("total") = subharga.ToString - GV.GetRow(e.RowHandle).item("jml_diskon").ToString
            'End If
            'txtSubTotal.EditValue = subharga
        End If
    ...
    Function getTotal() As Double
        ...
        For Each rw As DataRow In ds.Tables("POD").Rows
            If TypeOf rw.Item("qty") Is DBNull Or TypeOf rw.Item("harga") Is DBNull Then
            ...
            ElseIf TypeOf rw.Item("total") Is DBNull Then
                ...
                rw.Item("total") = 0
            End If
            'ttl = GV_PR.Columns("total").SummaryItem.SummaryValue
            ttl += CDbl(rw("total"))
    ...
    'Private Sub gvAdd_CellValueChanged(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs)
    ...
    'Private Sub gvAdd_InitNewRow(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Grid.InitNewRowEventArgs)
    ...
    '        isValid = False
    '        GV_PR.SetColumnError(tglkirim, "Harap Memilih Tanggal KedatanganTelebih Dahulu.", DevExpress.XtraEditors.DXErrorProvider.ErrorType.Critical)
    '    End If
    '    e.Valid = isValid
    'End Sub
    ...
    Private Sub GV_PR_RowUpdated(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.RowObjectEventArgs) Handles GV_PR.RowUpdated
        'End If
        txtSubTotal.EditValue = getTotal()
    End Sub

    Private Sub GV_PR_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles GV_PR.KeyDown
        If e.KeyCode = Keys.Delete Then
        ...
        If (e.KeyCode = Keys.Enter AndAlso e.Modifiers = Keys.Control) Then
            'If MsgBox("CTRL + [Enter]: Data akan di simpan?  !", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
            save_click()
            'End If
        End If
    End Sub

    Private Sub txtNoRef_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtNoRef.KeyDown
        If e.KeyCode = Keys.Enter Then

            GV_PR.FocusedColumn = GV_PR.VisibleColumns(1)
        End If
    End Sub
