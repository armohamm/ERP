...
Public Class frmMon_ByrOffline
    ...
    Public Sub LoadStruktur()
        ...
        If tglDari.EditValue = vbNullString And tglsampai.EditValue = vbNullString Then ' And lookupBulanAwal.EditValue = vbNullString
            sqltgl = ""
        ElseIf tglDari.EditValue <> vbNullString And tglsampai.EditValue = vbNullString Then
            sqltgl = " and tgl_bayar > convert(date,'" & tglDari.Text & "',103)"
        ElseIf tglDari.EditValue = vbNullString And tglsampai.EditValue <> vbNullString Then
            sqltgl = " and tgl_bayar < convert(date,'" & tglsampai.Text & "',103)"
        ElseIf tglDari.EditValue <> vbNullString And tglsampai.EditValue <> vbNullString Then
            sqltgl = " and tgl_bayar between convert(date,'" & tglDari.Text & "',103) and convert(date,'" & tglsampai.Text & "',103)"
        ...
        'Dim sql As String = "select case when STATUS_AKTIF=0 THEN 'PENGAJUAN' WHEN STATUS_AKTIF=1 THEN 'SUDAH CAIR' END AS stat ,*,0 as nomer from retail where status_lunas='BELUM' "
        ...
        'End If
        DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select *,0 as nomer from angsuran where kode_lunas=2  " & sqltgl, sqlconn))
