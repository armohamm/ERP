...
Public Class multiCtk_TBW
    ...
    Sub ReloadTable()
        ...
        da = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select 'T' as cetak_ulang,0 as nomer,* from tab_wajib t inner join anggota a on t.nasabah_id=a.nasabah_id where SUBSTRING(t.no_tab,1,3)='TBW' and t.stat_aktif=1 ", sqlconn))
    ...
    Public Sub cetak_click()
        ...
        For Each rw As DataRow In ds.Tables("_mgdo").Select("cetak_ulang='Y'")
            Try
                Dim sqlcmd As New SqlCommand("update tab_wajib set cetak_ke = isnull(cetak_ke,0) + 1, last_print=getDate() where no_tab='" & rw.Item("no_tab").ToString & "'", sqlconn, BTRANZ)
                sqlcmd.ExecuteNonQuery()

                'If i > 0 Then
                'MsgBox(rw.Item("no_trans").ToString)
                callReport(App_Path() & "\report\tab_wajib.rpt", "", "no_tbg=" & rw.Item("no_tab") & "&nip=" & rw.Item("nasabah_id"), True, True)
                'rw("stat") = "OK"
                'Else
    ...
    '    CREATE VIEW dbo.v_tabungWajib
    'AS
    'select t.id_cabang,substring(t.blthn,1,4)as bln,substring(t.blthn,5,2) as thn,a.Nama,a.almt_skrg,a.REKBCA1 as norek,t.NASABAH_ID,a.tgl_masuk,t.no_tab,t.saldo_awal,t.saldo_akhir from tab_wajib t 
    'left join anggota a on t.nasabah_id=a.nasabah_id
    'where SUBSTRING(t.no_tab,1,3)='TBW'

    'End Sub


    Private Sub multiCtk_TBW_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        connect()
        ReloadTable()
