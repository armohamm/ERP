...
Public Class frmMonPinjaman
    ...
    Public Sub LoadStruktur()
        ...
        If cb_all.CheckState = CheckState.Checked Then
            If Lue_SetPjm.EditValue = vbNullString Or Lue_SetPjm.EditValue = "ALL" Then
                DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select case when STATUS_AKTIF=0 THEN 'PENGAJUAN' WHEN STATUS_AKTIF=1 THEN 'SUDAH CAIR' END AS stat ,0 as nomer,no_trans,jenis,kd_kredit,tgl_trans,nasabah_id,JML_PINJAMAN,LAMA_ANGS,JML_POKOK,JML_BUNGA,(JML_POKOK+JML_BUNGA) as angsuran,jml_angsuran*sisa_angs as pj_bg,sisa_angs,sisa_pokok from kredit where status_lunas='BELUM' AND stat_approve >=5 " & sqltgl, sqlconn))
                'Clipboard.SetText(sql & " where " & sqltgl)
                DA.Fill(dSO, "_krd")
                'OMI amsil RITEL
                DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select case when STATUS_AKTIF=0 THEN 'PENGAJUAN' WHEN STATUS_AKTIF=1 THEN 'SUDAH CAIR' END AS stat ,0 as nomer,no_trans,jenis,kd_kredit,tgl_trans,nasabah_id,JML_PINJAMAN,LAMA_ANGS,JML_POKOK,JML_BUNGA,(JML_POKOK+JML_BUNGA) as angsuran,jml_angsuran*sisa_angs as pj_bg,sisa_angs,sisa_pokok from retail where status_lunas='BELUM' AND stat_approve >=5 " & sqltgl, sqlconn))
                DA.Fill(dSO, "_krd")
            Else
                DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select case when STATUS_AKTIF=0 THEN 'PENGAJUAN' WHEN STATUS_AKTIF=1 THEN 'SUDAH CAIR' END AS stat ,0 as nomer,no_trans,jenis,kd_kredit,tgl_trans,nasabah_id,JML_PINJAMAN,LAMA_ANGS,JML_POKOK,JML_BUNGA,(JML_POKOK+JML_BUNGA) as angsuran,jml_angsuran*sisa_angs as pj_bg,sisa_angs,sisa_pokok from kredit where jenis='" & Lue_SetPjm.EditValue & "' and status_lunas='BELUM' AND stat_approve >=5 " & sqltgl, sqlconn))
                'Clipboard.SetText(sql & " where " & sqltgl)
                DA.Fill(dSO, "_krd")
                'OMI amsil RITEL
        ...
        Else
            DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select case when STATUS_AKTIF=0 THEN 'PENGAJUAN' WHEN STATUS_AKTIF=1 THEN 'SUDAH CAIR' END AS stat ,0 as nomer,no_trans,jenis,kd_kredit,tgl_trans,nasabah_id,JML_PINJAMAN,LAMA_ANGS,JML_POKOK,JML_BUNGA,(JML_POKOK+JML_BUNGA) as angsuran,jml_angsuran*sisa_angs as pj_bg,sisa_angs,sisa_pokok from kredit where status_lunas='BELUM' AND stat_approve >=5 AND NASABAH_ID='" & LUE_Kartu.Text & "'" & sqltgl, sqlconn))
            DA.Fill(dSO, "_krd")
            'OMI amsil RITEL
