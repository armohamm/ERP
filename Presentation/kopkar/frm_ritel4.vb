...
Public Class frm_ritel4
    ...
    'Private tgl_upload As SqlDataReader
    Public tgl_upload As String
    ...
    Dim persen_provisi, persen_bunga, persen_denda, persen_penalty, jml_plafond, jml_admin, batas_admin1, batas_admin2 As Decimal
    ...
    Public Sub LoadStruktur(ByVal kode As String)
        ...
        If dSO.Tables("_krd").Rows.Count > 0 Then
            ...
            End If
            le_jenis_pot.EditValue = IIf(row("kd_kredit") = row("jenis"), "", row("jenis"))
    ...
    Public Sub edit_click()
        'LookupPO.Hide()
        STATE = "EDIT"
        Dim sqlstr As String = "select  r.tgl_trans as [Tanggal_Transaksi], r.no_trans as [No_Transaksi], r.nasabah_id as [NIP], isnull(r.nama, a.Nama) as Nama,isnull(r.JML_PINJAMAN,0) as Nilai, s.nama as [Nama_Supplier], b.nama as [Nama_Barang], r.KETERANGAN as [Keterangan], a.job_nama as Dinas from retail r inner join anggota a on a.nasabah_id=r.nasabah_id inner join m_barang s on s.kd_brg=r.kd_brg inner join m_jns_barang b on b.kd_jns=r.kd_jns_brg where r.Last_Created_By='" & username & "'  order by  r.tgl_trans desc, r.Last_Create_Date desc  "
    ...
    Private Sub bersihkan()
        ...
        txb_norek.Text = ""
        Lue_Brg.EditValue = "00000003" '--- default BUMSS
        lb_sld_awal.Text = ""
        Lue_Jns_brg.EditValue = 3 '--- default pulsa
    ...
    Public Sub simpan(Optional ByRef isDelete As Boolean = False)
        ...
        If Not isDelete Then
            ...
            row.Item("jenis") = IIf(le_jenis_pot.EditValue = "", Lue_SetPjm.EditValue, le_jenis_pot.EditValue)
    ...
    Public Sub hitung_angsuran()
        ...
        Dim tday, jml_hr, hr_ini, hrdbt, jm_hr_lalu, bln_skrg, jm_hr_bln, jm_bl_lalu, jm_bl_skrg, hr_k_dbt, jm_hr_today As Integer
        Dim dbt_next, debet_lalu As String
        Dim tgl_upload As DateTime = tgl_up.Text
        ...
        'sqdr1.Close()
        'jml_hr = jm_bl_skrg - (hr_ini - 1)
        Dim tgl As DateTime = de_tgl.EditValue
        tday = tgl.Day()
    ...
    Public Sub cari_brg()
        ...
        ElseIf Lue_Brg.EditValue = "00000008" Then ' jika Lain2
            Lue_Jns_brg.EditValue = 0 ' --- default lain2
            lb_fee.Text = "Jumlah"
    ...
    Private Sub frm_anggota_kluar_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        '--- Lookup POtongan
        DALOOKUP = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("SELECT '' as kode, 'TIDAK' as nama UNION SELECT kode, nama FROM m_jns_upload where tipe_trans='BNS'", sqlconn))
        DALOOKUP.Fill(ds_gdg1, "LookPot")
        le_jenis_pot.Properties.DataSource = ds_gdg1.Tables("LookPot")
        le_jenis_pot.Properties.ValueMember = "kode"
        le_jenis_pot.Properties.DisplayMember = "nama"
        le_jenis_pot.EditValue = ""

        tgl_upload = "select last_upload from m_konfig"
        cmd = New SqlCommand(tgl_upload, sqlconn)
        rd = cmd.ExecuteReader
        rd.Read()
        If rd.HasRows Then
            tgl_up.Text = rd.Item("last_upload")
        End If
        rd.Close()
    End Sub
    ...
    Private Sub TE_BsrPJM_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles TE_BsrPJM1.KeyDown
        If e.KeyCode = Keys.Enter Then
            tempo = tx1_LamaPJM.Text
            If (Lue_Brg.EditValue <> "00000004" And Lue_Brg.EditValue <> "00000008") And TE_BsrPJM1.EditValue > rp_akr Then
                MsgBox("Nominal tidak boleh melebih saldo Supplier")
                Exit Sub
            End If
            hitung_all()
            TE_Fee.Focus()
    ...
    Private Sub btnTopUp_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnTopUp.Click
        ...
        supp_deposit.ShowDialog()
    End Sub

    Private Sub le_jenis_pot_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles le_jenis_pot.EditValueChanged
        lb_tgl_debet.Visible = False
        de_tgl_debet.Visible = False
        If le_jenis_pot.EditValue = "" Then
            kredit_changed()
            hitung_angsuran()
        Else
            lb_tgl_debet.Visible = True
            de_tgl_debet.Visible = True

            Dim dr As SqlDataReader = New SqlCommand("SELECT * from m_jns_upload where kode='" & le_jenis_pot.EditValue & "'", sqlconn).ExecuteReader
            dr.Read()


            prv = dr.Item("provisi")
            bunga = dr.Item("Bunga_KSP")
            persen_denda = dr.Item("Denda")
            persen_penalty = IIf(dr.Item("penalty") Is DBNull.Value, 0, dr.Item("penalty"))
            jml_plafond = dr.Item("nilai")
            adm = dr.Item("Adm")
            Decimal.TryParse(dr.Item("batas_admin1").ToString(), batas_admin1)
            Decimal.TryParse(dr.Item("batas_admin2").ToString(), batas_admin2)
            ...
            dr.Close()
            Dim tgl As DateTime = de_tgl.EditValue
            Dim tgl_debet, tgl_tmp As DateTime

            If le_jenis_pot.EditValue = "PTAT" Then
                tgl_tmp = xtat
            ElseIf le_jenis_pot.EditValue = "PTHR" Then
                tgl_tmp = xthr
            ElseIf le_jenis_pot.EditValue = "PBNS" Then
                tgl_tmp = xbns
            End If
            tgl_debet = tgl_tmp
            If tgl > tgl_tmp Then
                tgl_debet = New DateTime((tgl_tmp.Year + 1), tgl_tmp.Month, tgl_tmp.Day)
            End If
            de_tgl_debet.EditValue = tgl_debet
            hitung_potongan()
        End If
    End Sub
    Private Sub hitung_potongan()
        Dim jml_terima, fee As Decimal
        Dim tgl As DateTime = de_tgl.EditValue
        Dim tgl_debet, tgl_tmp As DateTime
        ...
        '    tgl_debet = New DateTime((tgl_tmp.Year + 1), tgl_tmp.Month, tgl_tmp.Day)
        'End If
        tgl_debet = de_tgl_debet.EditValue

        Dim cnt As Int16 = 0
        Dim faktor_bulan As Decimal = 0
        Dim jml_pinjaman As Decimal
        Dim tgl_iterate As DateTime
        tgl_iterate = tgl
        Dim yearmonth_iterate As DateTime = New Date(tgl_iterate.Year, tgl_iterate.Month, 1)
        Dim yearmonth_debet As DateTime = New Date(tgl_debet.Year, tgl_debet.Month, 1)
        While yearmonth_iterate <= yearmonth_debet
            Dim daysinmonth As Int16 = Date.DaysInMonth(tgl_iterate.Year, tgl_iterate.Month)
            Dim jml_hari As Int16

            If yearmonth_iterate = yearmonth_debet Then
                If cnt = 0 Then
                    jml_hari = tgl_debet.Day - tgl_iterate.Day + 1
                    faktor_bulan += (jml_hari / daysinmonth)
                Else
                    faktor_bulan += (tgl_debet.Day / daysinmonth)
                End If
            Else
                If cnt = 0 Then
                    jml_hari = daysinmonth - tgl_iterate.Day + 1
                    faktor_bulan += (jml_hari / daysinmonth)
                Else
                    faktor_bulan += 1
                End If
            ...
            tgl_iterate = tgl_iterate.AddMonths(1)
            yearmonth_iterate = New Date(tgl_iterate.Year, tgl_iterate.Month, 1)
            cnt += 1

        End While
        'fee = TE_Fee.EditValue
        'jml_pinjaman = TE_BsrPJM1.EditValue + (bunga * TE_BsrPJM1.EditValue * faktor_bulan / 100)

        te_pokok.EditValue = TE_BsrPJM1.EditValue
        te_bunga.EditValue = (bunga * TE_BsrPJM1.EditValue * faktor_bulan / 100)
        bsr_pjm = te_pokok.EditValue
        pokok_pertama = bsr_pjm
        pokok_akhir = bsr_pjm
        t_bunga = te_bunga.EditValue
    End Sub
    ...
    Private Sub Jenis_Bayar_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Jenis_Bayar.EditValueChanged
        hitung_angsuran()
    End Sub
End Class
