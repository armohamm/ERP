...
Public Class frmKredit2
    ...
    Dim foto() As Byte
    Dim pot_pertahun, pot_total As Double
    Dim bunga_prv As Decimal
    ...
    Public Sub LoadStruktur(ByVal kode As String)
        ...
        If dSO.Tables("_krd").Rows.Count > 0 Then
            ...
            TE_pokok.EditValue = rwm("JML_POKOK")
    ...
    Public Sub simpan(Optional ByRef isDelete As Boolean = False)
        ...
        If Not isDelete Then
            ...
            row.Item("JML_POKOK") = TE_pokok.EditValue
            row.Item("JML_BUNGA") = t_bunga
            row.Item("jml_angsuran") = TE_pokok.EditValue + t_bunga
            ...
            If jns_krd = "PKHS" Or jns_krd = "PBNS" Then
    ...
    Private Sub frmKredit_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        DALOOKUP = New SqlDataAdapter(New SqlCommand("SELECT * FROM m_jns_upload where divisi='KRD' ", sqlconn)) 'where tipe_trans='KRD'
    ...
    Private Sub Lue_SetPjm_EditValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles Lue_SetPjm.EditValueChanged
        ...
        If STATE = "ADD" Then
            ...
            LUE_nama.Enabled = True
            If Lue_SetPjm.EditValue = "PTAT" Or Lue_SetPjm.EditValue = "PTHR" Or Lue_SetPjm.EditValue = "PBNS" Then 'Or Lue_SetPjm.EditValue = "PKHS"
                ...
                jns_krd = "PBNS" 'bonus april tat thr
            ...
            ElseIf Lue_SetPjm.EditValue = "PKHS" Then '4 char
                taunan = True
                jns_krd = "PKHS"
                lb_debet.Visible = True
                de_debet.Visible = True
            ...
            lb_prvs.Text = dr(i).Item("provisi")
            bunga_prv = dr(i).Item("provisi") / 100
    ...
    Private Sub LuE_Jab_EditValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles LuE_Jab.EditValueChanged
        Dim i As Integer
        Dim dr() As DataRow = ds_cari.Tables("LookTran").Select("kode='" & Lue_SetPjm.EditValue & "'")
        If Lue_SetPjm.EditValue = "PTHR" Or Lue_SetPjm.EditValue = "PTAT" Or Lue_SetPjm.EditValue = "PBNS" Then 'Or Lue_SetPjm.EditValue = "PKHS" 
            'taunan = True
            'MsgBox("Taunan = true")
...
"1 as LAMA_ANGS,1 as sisa_angs,rp_tunggakan as JML_PINJAMAN,rp_tunggakan sisa_pokok from tunggakan where nasabah_id= '" & LUE_nama.EditValue & "' and stat_lunas=0", sqlconn).ExecuteReader
    ...
    Private Sub hitung_cicilan()
        Dim tgl_real As DateTime = DateTime.Now
        Dim pinjaman As Long = TE_BsrPJM.EditValue
        Dim tenor As Integer = tx1_LamaPJM.Text
        Dim angsuran_bunga As Decimal = pinjaman * (bunga / 100)
        t_bunga = angsuran_bunga
        pokok_akhir = pinjaman
        Decimal.TryParse(TE_bns.Text, pot_bns)
        Decimal.TryParse(TE_thr.Text, pot_thr)
        Decimal.TryParse(TE_tat.Text, pot_tat)
        If Not ds_byr_kartu.Tables("_kartu") Is Nothing Then ds_byr_kartu.Tables("_kartu").Clear()
        ...
        For i As Integer = 1 To tenor
            ...
            rw_kartu = ds_byr_kartu.Tables("_kartu").NewRow()
            rw_kartu.Item("c_pokok_pinj") = 0
            rw_kartu.Item("c_pokok") = 0
            rw_kartu.Item("c_bunga") = angsuran_bunga
            rw_kartu.Item("c_angs") = 0
            rw_kartu.Item("c_bns") = 0
            rw_kartu.Item("c_thr") = 0
            rw_kartu.Item("c_tat") = 0
            rw_kartu.Item("angs_ke") = i
            rw_kartu.Item("c_tahun") = iterate_monthyear.Year
            rw_kartu.Item("c_bulan") = MonthName(Convert.ToInt16(iterate_monthyear.Month.ToString), True)
            ...
            ds_byr_kartu.Tables("_kartu").Rows.Add(rw_kartu)
        Next
        Dim angsuran_pokok As Long = Math.Ceiling(pokok_akhir / tenor)

        Dim rw_kartu_prev As DataRow
        For i As Integer = 0 To (tenor - 1)
            rw_kartu = ds_byr_kartu.Tables("_kartu")(i)
            rw_kartu_prev = ds_byr_kartu.Tables("_kartu")(i - 1)


            If i = 0 Then
                rw_kartu.Item("c_pokok_pinj") = pinjaman
            Else
                rw_kartu.Item("c_pokok_pinj") = rw_kartu_prev.Item("c_pokok_akhir")
            End If
            rw_kartu.Item("c_pokok") = angsuran_pokok
            rw_kartu.Item("c_angs") = rw_kartu.Item("c_pokok") + rw_kartu.Item("c_bunga")

            Dim saldo_pokok_akhir As Decimal = rw_kartu.Item("c_pokok_pinj") - rw_kartu.Item("c_pokok")
            Dim pot_all As Decimal
            If i = (tenor - 1) Then
                rw_kartu.Item("c_bns") = IIf(rw_kartu.Item("c_bns") = 0, 0, saldo_pokok_akhir)
                rw_kartu.Item("c_thr") = IIf(rw_kartu.Item("c_thr") = 0, 0, saldo_pokok_akhir)
                rw_kartu.Item("c_tat") = IIf(rw_kartu.Item("c_tat") = 0, 0, saldo_pokok_akhir)
                'Else 'If i > 0 Then

            End If
            pot_all = rw_kartu.Item("c_bns") + rw_kartu.Item("c_thr") + rw_kartu.Item("c_tat")
            rw_kartu.Item("c_pokok_akhir") = rw_kartu.Item("c_pokok_pinj") - (rw_kartu.Item("c_pokok") + pot_all)

            'If i = 0 Then
            '    rw_kartu.Item("c_pokok_pinj") = pinjaman
            'Else
            '    rw_kartu.Item("c_pokok_pinj") = rw_kartu_prev.Item("c_pokok_pinj") - rw_kartu_prev.Item("c_pokok") - pot_all

            'End If
        Next

        TE_pokok.EditValue = angsuran_pokok
        lb1_pokok_br.Text = angsuran_pokok.ToString("c", New Globalization.CultureInfo("id-ID", False))
        lb1_bunga_br.Text = angsuran_bunga.ToString("c", New Globalization.CultureInfo("id-ID", False))
        pot_pertahun = (TE_bns.EditValue + TE_thr.EditValue + TE_tat.EditValue)
        ...
        Dim angsuran_total As Double = angsuran_pokok + angsuran_bunga
        lb1_ang_baru.Text = angsuran_total.ToString("c", New Globalization.CultureInfo("id-ID", False))
        lb_selama.Text = Math.Ceiling(tenor / 12)
        prv = Math.Ceiling(prv)
        TE_prv.EditValue = prv
        TE_adm.EditValue = adm
        TE_net.EditValue = pinjaman - (prv + adm)
    End Sub
    Private Sub hitung_by_pokok()
        ...
        Dim iterate_monthyear As DateTime
        pot_total = 0
        Dim angsuran_pokok_baru As Double = TE_pokok.EditValue
        Dim saldo_pokok_awal As Double = TE_BsrPJM.EditValue
        Dim saldo_pokok_akhir As Double = TE_BsrPJM.EditValue
        ...
        While saldo_pokok_akhir > 0
            iterate_tenor += 1
            rw_kartu_prev = ds_byr_kartu.Tables("_kartu")(iterate_tenor - 1)
            iterate_monthyear = tgl_real.AddMonths(iterate_tenor)
            rw_kartu = ds_byr_kartu.Tables("_kartu").NewRow()
            'rw_kartu.Item("bultah") = iterate_monthyear
            'rw_kartu.Item("saldo_pokok_awal") = 0
            If (iterate_tenor > 1) Then
                rw_kartu.Item("c_pokok_pinj") = saldo_pokok_akhir
            Else
                rw_kartu.Item("c_pokok_pinj") = saldo_pokok_awal
            End If
            'rw_kartu.Item("c_pokok_pinj") = IIf(iterate_tenor > 1, rw_kartu_prev.Item("saldo_pokok_akhir"), saldo_pokok_awal)
            '$row['saldo_pokok_awal'] = isset($kartu[$iterate_tenor-1]['saldo_pokok_akhir']) ? $kartu[$iterate_tenor-1]['saldo_pokok_akhir'] : $pinjaman;;
            'rw_kartu.Item("saldo_pokok_akhir") = 0
            'rw_kartu.Item("angsuran_pokok") = 0
            rw_kartu.Item("c_pokok") = angsuran_pokok_baru
            'rw_kartu.Item("angsuran_bunga") = angsuran_bunga
            rw_kartu.Item("c_bunga") = angsuran_bunga
            rw_kartu.Item("c_angs") = angsuran_pokok_baru + angsuran_bunga
            rw_kartu.Item("c_bns") = 0
            rw_kartu.Item("c_thr") = 0
            rw_kartu.Item("c_tat") = 0
            rw_kartu.Item("angs_ke") = iterate_tenor
            rw_kartu.Item("c_tahun") = iterate_monthyear.Year
            rw_kartu.Item("c_bulan") = MonthName(Convert.ToInt16(iterate_monthyear.Month.ToString), True)
            saldo_pokok_akhir -= angsuran_pokok_baru
            If saldo_pokok_akhir > 0 Then
                ...
                End If
            End If
            rw_kartu.Item("c_pokok_akhir") = saldo_pokok_akhir

            ds_byr_kartu.Tables("_kartu").Rows.Add(rw_kartu)
        End While

        tx1_LamaPJM.Text = iterate_tenor
        lb1_pokok_br.Text = angsuran_pokok_baru.ToString("c", New Globalization.CultureInfo("id-ID", False))
        lb1_ang_baru.Text = (angsuran_pokok_baru + angsuran_bunga).ToString("c", New Globalization.CultureInfo("id-ID", False))
        lb_selama.Text = Math.Ceiling(iterate_tenor / 12)
        lb_tpot.Text = pot_pertahun.ToString("c", New Globalization.CultureInfo("id-ID", False))
        lb_total2.Text = pot_total.ToString("c", New Globalization.CultureInfo("id-ID", False))
        Console.WriteLine(iterate_tenor)
    End Sub
    Private Sub hitung_jml_terima()
        bsr_pjm = (TE_net.EditValue - (bunga_prv * bts1) + TE_adm.EditValue) / (1 - bunga_prv)
        TE_BsrPJM.EditValue = Math.Ceiling(bsr_pjm)
        prv_all()
        hitung_all()
    End Sub
End Class
