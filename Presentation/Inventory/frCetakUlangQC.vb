...
Public Class frCetakUlangQC
    ...
    Public Sub cetak_click()
        ...
        For Each rw As DataRow In ds.Tables("_MonQC").Select("cetak_ulang='Y'")
            Try
                Dim i As Int16 = New SqlCommand("update INV.dbo.INV_QC_M set cetak_ulang='T', cetak_ke = cetak_ke + 1 where no_trans='" & rw.Item("no_trans").ToString & "'", sqlconn).ExecuteNonQuery
                If i > 0 Then
                    'MsgBox(rw.Item("no_trans").ToString)
                    callReport(App_Path() & "\report\QC.rpt", "", "no_qc=" & rw.Item("no_trans"), True, False)
                    'rw("stat") = "OK"
                Else
