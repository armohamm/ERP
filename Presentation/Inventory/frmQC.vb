...
Public Class frmQC
    ...
    Private STATE, stathold, p_np, kd_sup As String '<-- untuk mengecek status saat ini (ADD, EDIT, delete)
    Private dso, dSO1, dso2, ds_cari, ds_cari1, ds_cari2, ds_gdg1, ds_gdg2, ds_sup, ds_palsu As New DataSet
    Private Buildernya As New SqlClient.SqlCommandBuilder
    Private rw As DataRow
    Public old_sj As String
    Dim sisa_asli As Decimal
    Private DA, DA2, DA_gdg1, DA_gdg2, DALOOKUP, DALOOKUP1, DALOOKUP2, da_sup, DA_palsu As SqlClient.SqlDataAdapter
    ...
    Public Sub save_click(Optional ByRef isDel As Boolean = False)
        ...
        ElseIf STATE = "EDIT" And dSO1.Tables("_qc").Rows.Count = 1 And dSO1.Tables("_qc").Select("[hold] = 0").Count > 0 And dSO1.Tables("_qc").Select("qty_order=qty_sisa").Count = 1 Then
            MsgBox("Data tidak dapat disimpan!!! Qty Good=0 dan Qty Hold=0," & vbCrLf & "Silahkan Gunakan fasilitas (x)HAPUS", MsgBoxStyle.Critical)
            Exit Sub
        End If
        'End if
    ...
    Public Sub simpan(Optional ByRef isDelete As Boolean = False)
        ...
        If Not isDelete Then
            ...
            rw.Item("jml_rp_trans") = GvBME_d.Columns(10).SummaryItem.SummaryValue
            ...
            For Each row As DataRow In dSO1.Tables("_qc").Rows
                If row.RowState = DataRowState.Added Then
                    ...
                    row.Item("no_ref2") = txbSJSupplier.Text
        ...
        Try
            ...
            If Not STATE = "DELETE" Then
                ...
                For Each row1 As DataRow In dSO1.Tables("_qc").Rows
                    ...
                    Dim sqlcmd As New SqlCommand("update PURC.dbo.PURC_PO_D set qty_sisa=@nilai where no_po='" & LookupPO.EditValue & "' and kd_stok='" & row1("kd_stok") & "' and keterangan=@ket", sqlconn, BTRANS)
                    sqlcmd.Parameters.AddWithValue("@nilai", row1.Item("qty_sisa"))
                    sqlcmd.Parameters.AddWithValue("@ket", row1.Item("keterangan"))
    ...
    Private Sub frmQC_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load


        prn = getParent(Me)
        GvBME_d.Columns("harga").Visible = False
        GvBME_d.Columns("rp_trans").Visible = False
        GvBME_d.Columns("status_hold").Visible = False
        ...
        LookupSup.Properties.DataSource = ds_sup.Tables("_sup")
        LookupSup.Properties.ValueMember = "Kode_Supplier"
        LookupSup.Properties.DisplayMember = "Nama_Supplier"
    ...
    Private Sub load_po()
        If STATE = "ADD" Then
            ...
            'DALOOKUP = New SqlDataAdapter(New SqlCommand("SELECT distinct POD.no_po as no_ref FROM PURC.dbo.PURC_PO_D POD, PURC.dbo.PURC_PO PO where POD.no_po=PO.no_PO and POD.qty_sisa <> 0 and PO.rec_stat = 'APPROVE'", sqlconn))
            DALOOKUP = New SqlDataAdapter(New SqlCommand("select distinct (no_po) from(select po.rec_stat, po.no_po,po.flag_ppn,pod.kd_stok,pod.keterangan,pod.kd_satuan,pod.harga,(select qty_order from INV.dbo.v_saldo_qc_pass where no_po= po.no_po and kd_stok=pod.kd_stok and no_seq=pod.no_seq)as qty_ordered, pod.qty,pod.qty_sisa, brg.kd_ukuran,brg.lokasi,brg.rek_persediaan,brg.rek_biaya, 0 as qty_qc_pass from PURC.DBO.PURC_PO_D pod inner join PURC.DBO.PURC_PO po on pod.no_po=po.no_po inner join SIF.dbo.SIF_Barang brg on pod.kd_stok=brg.Kode_Barang where po.rec_stat='APPROVE' and SUBSTRING(po.no_po,12,1)='M' and po.status_po<>'CLOSE')a where a.qty_ordered IS NOT NULL", sqlconn))
            DALOOKUP.Fill(ds_cari, "_lookup1")
            LookupPO.Properties.DataSource = ds_cari.Tables("_lookup1")
        ...
        ElseIf STATE = "EDIT" Then
            If Not ds_cari.Tables("_lookup1") Is Nothing Then ds_cari.Tables("_lookup1").Clear()
            DALOOKUP = New SqlDataAdapter(New SqlCommand("select distinct (no_ref)as no_po from INV.dbo.INV_QC_M where SUBSTRING(no_ref,12,1)='M'", sqlconn))
            DALOOKUP.Fill(ds_cari, "_lookup1")
            LookupPO.Properties.DataSource = ds_cari.Tables("_lookup1")
            LookupPO.Properties.ValueMember = "no_po"
            LookupPO.Properties.DisplayMember = "no_po"
    ...
    Private Sub formate_yayak()
        ...
        GvBME_d.Columns("qty_qc_unpass").DisplayFormat.FormatType = DevExpress.Utils.FormatType.Numeric
        GvBME_d.Columns("qty_qc_unpass").DisplayFormat.FormatString = "n3"
        GvBME_d.Columns("hold").DisplayFormat.FormatType = DevExpress.Utils.FormatType.Numeric
        GvBME_d.Columns("hold").DisplayFormat.FormatString = "n3"
        GvBME_d.Columns("qty_sisa").DisplayFormat.FormatType = DevExpress.Utils.FormatType.Numeric
        GvBME_d.Columns("qty_sisa").DisplayFormat.FormatString = "n3"
    End Sub
    ...
    Private Sub LoadStruktur(ByVal kodepo As String)
        ...
        If dso.Tables("_qcm").Rows.Count > 0 Then
            ...
            LookupSup.EditValue = rwm("kd_supplier").ToString
            'MsgBox(rw("no_ref").ToString)
            old_sj = rwm("sj_supplier").ToString
        End If
    ...
    Private Sub GvBKE_d_CellValueChanged(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles GvBME_d.CellValueChanged
        Dim hold_po As Integer
        'If e.Column Is qty_datang Then
        '    ' ''GvBME_d.GetRow(e.RowHandle).item("hold") = GvBME_d.GetRowCellValue((e.RowHandle), "qty_datang") - GvBME_d.GetRowCellValue((e.RowHandle), "qty_order")
        '    'If GvBME_d.GetRow(e.RowHandle).item("qty_datang") < GvBME_d.GetRow(e.RowHandle).item("qty_order") Then 'berarti ada sisa/parsial maka ga bole ada hold
        '    '    If GvBME_d.GetRow(e.RowHandle).item("qty_datang") > GvBME_d.GetRow(e.RowHandle).item("qty_sisa") Then
        '    '        MsgBox("Pengiriman Parsial tak boleh ada hold " & vbCrLf & " barang datang melebihi sisa, lakukan PO")
        '    '        Exit Sub
        '    '    End If
        '    'Else 'jika ordr=sisa (bkn parsial) maka bisa entry hold
        '    If GvBME_d.GetRow(e.RowHandle).item("qty_datang") > GvBME_d.GetRow(e.RowHandle).item("qty_order") And GvBME_d.GetRow(e.RowHandle).item("qty_sisa") = GvBME_d.GetRow(e.RowHandle).item("qty_order") Then
        '        stathold = "hold"
        '        GvBME_d.GetRow(e.RowHandle).item("hold") = GvBME_d.GetRowCellValue((e.RowHandle), "qty_datang") - GvBME_d.GetRowCellValue((e.RowHandle), "qty_order")
        '        GvBME_d.GetRow(e.RowHandle).item("qty_sisa") = 0
        '    Else
        '        'MsgBox("no hold")
        '        If GvBME_d.GetRow(e.RowHandle).item("qty_datang") > GvBME_d.GetRow(e.RowHandle).item("qty_sisa") Then 'berarti ada sisa/parsial maka ga bole ada hold
        '            MsgBox("Pengiriman Parsial tak boleh melebihi sisa Order" & vbCrLf & "Jika barang datang melebihi sisa, lakukan PO")
        '            GvBME_d.GetRow(e.RowHandle).item("qty_datang") = 0
        '            Exit Sub
        '        Else
        '            GvBME_d.GetRow(e.RowHandle).item("qty_sisa") = GvBME_d.GetRowCellValue((e.RowHandle), "qty_sisa") - GvBME_d.GetRowCellValue((e.RowHandle), "qty_datang")
        '        End If
        ...
        'GvBME_d.GetRow(e.RowHandle).item("rp_trans") = GvBME_d.GetRow(e.RowHandle).item("harga").ToString * GvBME_d.GetRow(e.RowHandle).item("qty_qc_pass").ToString
        'If stathold = "hold" Then
        '    'MsgBox("ini hold")
        '    If GvBME_d.GetRow(e.RowHandle).item("qty_qc_pass") > GvBME_d.GetRow(e.RowHandle).item("qty_order") Then
        '        GvBME_d.GetRow(e.RowHandle).item("qty_qc_pass") = GvBME_d.GetRow(e.RowHandle).item("qty_order")
        '        GvBME_d.GetRow(e.RowHandle).item("qty_sisa") = 0
        '        GvBME_d.GetRow(e.RowHandle).item("qty_qc_unpass") = 0

        '    Else
        '        GvBME_d.GetRow(e.RowHandle).item("qty_qc_unpass") = GvBME_d.GetRowCellValue((e.RowHandle), "qty_order") - GvBME_d.GetRowCellValue((e.RowHandle), "qty_qc_pass")
        '        GvBME_d.GetRow(e.RowHandle).item("qty_sisa") = GvBME_d.GetRow(e.RowHandle).item("qty_qc_unpass")
        '    End If
        'Else
        '    'MsgBox("ini bukan hold")
        '    GvBME_d.GetRow(e.RowHandle).item("qty_qc_unpass") = GvBME_d.GetRowCellValue((e.RowHandle), "qty_datang") - GvBME_d.GetRowCellValue((e.RowHandle), "qty_qc_pass")
        'End If
        'GvBME_d.GetRow(e.RowHandle).item("qty_qc_unpass") = GvBME_d.GetRow(e.RowHandle).item("qty_datang").ToString - GvBME_d.GetRow(e.RowHandle).item("qty_qc_pass").ToString
        'GvBME_d.GetRow(e.RowHandle).item("rp_trans") = GvBME_d.GetRow(e.RowHandle).item("harga").ToString * GvBME_d.GetRow(e.RowHandle).item("qty_qc_pass").ToString
        'If GvBME_d.GetRow(e.RowHandle).item("qty_qc_unpass") < 0 Then
        '    MsgBox("Anda meng-entry LOLOS QC melebihi jumlah Barang Datang", MsgBoxStyle.Critical)
        'End If

        If e.Column Is qty_qc_pass Then

            If GvBME_d.GetRow(e.RowHandle).item("qty_qc_pass") > GvBME_d.GetRow(e.RowHandle).item("qty_order") And GvBME_d.GetRow(e.RowHandle).item("qty_order") = GvBME_d.GetRow(e.RowHandle).item("qty_sisa") Then
                ...
                'ElseIf GvBME_d.GetRow(e.RowHandle).item("qty_qc_pass") > GvBME_d.GetRow(e.RowHandle).item("qty_sisa") And GvBME_d.GetRow(e.RowHandle).item("qty_order") <> GvBME_d.GetRow(e.RowHandle).item("qty_sisa") Then
                '    MsgBox("Barang Lolos QC tak boleh melebihi sisa")
                '    GvBME_d.GetRow(e.RowHandle).item("qty_qc_pass") = 0
                '    GvBME_d.GetRow(e.RowHandle).item("qty_sisa") = sisa_asli
                '    Exit Sub
            End If


            'If GvBME_d.GetRow(e.RowHandle).item("qty_sisa") <> GvBME_d.GetRow(e.RowHandle).item("qty_order") Then
            GvBME_d.GetRow(e.RowHandle).item("qty_sisa") = GvBME_d.GetRow(e.RowHandle).item("qty_order") - GvBME_d.GetRow(e.RowHandle).item("qty_qc_pass")
        ...
        If e.Column Is hold Then
            If GvBME_d.GetRow(e.RowHandle).item("qty_sisa") = 0 Then
                GvBME_d.GetRow(e.RowHandle).item("status_hold") = "Over Order"
            ElseIf GvBME_d.GetRow(e.RowHandle).item("qty_sisa") > 0 Then
                GvBME_d.GetRow(e.RowHandle).item("status_hold") = "Quality Issue"
            End If
            GvBME_d.GetRow(e.RowHandle).item("hold_history") = GvBME_d.GetRow(e.RowHandle).item("hold")
            'If GvBME_d.GetRow(e.RowHandle).item("qty_sisa") <> 0 Then
            '    If GvBME_d.GetRow(e.RowHandle).item("hold") + GvBME_d.GetRow(e.RowHandle).item("qty_qc_pass") > GvBME_d.GetRow(e.RowHandle).item("qty_sisa") Then
            '        hold_po = GvBME_d.GetRow(e.RowHandle).item("hold") - GvBME_d.GetRow(e.RowHandle).item("qty_sisa")
            ...
            '        GvBME_d.GetRow(e.RowHandle).item("hold") = GvBME_d.GetRow(e.RowHandle).item("qty_sisa")
            '        GvBME_d.GetRow(e.RowHandle).item("qty_sisa") = 0
            '        'Else
    ...
    Private Sub LookupPO_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LookupPO.EditValueChanged
        If STATE = "ADD" Then
            If Not dSO1.Tables("_qc") Is Nothing Then dSO1.Tables("_qc").Clear()
            Dim sqdr As SqlDataReader = New SqlCommand("select a.* from(select po.kd_supplier,po.no_po,po.flag_ppn,pod.no_seq,pod.kd_stok,pod.keterangan,pod.kd_satuan,pod.harga,isnull((select qty_order from INV.dbo.v_saldo_qc_pass where no_po= po.no_po and kd_stok=pod.kd_stok and no_seq=pod.no_seq),0)as qty_ordered, pod.qty,pod.qty_sisa, brg.kd_ukuran,brg.lokasi,brg.rek_persediaan,brg.rek_biaya, 0 as qty_qc_pass from PURC.DBO.PURC_PO_D pod inner join PURC.DBO.PURC_PO po on pod.no_po=po.no_po inner join SIF.dbo.SIF_Barang brg on pod.kd_stok=brg.Kode_Barang where pod.no_po='" & LookupPO.EditValue & "')a where a.qty_ordered is not null", sqlconn).ExecuteReader
            Dim i As Int16 = 1
            dSO1.Tables("_qc").Clear()
            ...
            While sqdr.Read
                ...
                kd_sup = sqdr.Item("kd_supplier")
                p_np = sqdr.Item("flag_ppn")
                dSO1.Tables("_qc").Rows.Add(rw1)

                LookupSup.EditValue = kd_sup
                sisa_asli = sqdr.Item("qty_sisa")
            End While

            sqdr.Close()
            GvBME_d.FocusedColumn = GvBME_d.VisibleColumns(5)
        End If
    End Sub
    ...
    Private Sub LookupPO_QueryPopUp(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles LookupPO.QueryPopUp
        ...
        ds_cari.Clear()
        DALOOKUP = New SqlDataAdapter(New SqlCommand("select distinct (no_po) from(select po.rec_stat, po.no_po,po.flag_ppn,pod.kd_stok,pod.keterangan,pod.kd_satuan,pod.harga,(select qty_order from INV.dbo.v_saldo_qc_pass where no_po= po.no_po and kd_stok=pod.kd_stok and no_seq=pod.no_seq)as qty_ordered, pod.qty,pod.qty_sisa, brg.kd_ukuran,brg.lokasi,brg.rek_persediaan,brg.rek_biaya, 0 as qty_qc_pass from PURC.DBO.PURC_PO_D pod inner join PURC.DBO.PURC_PO po on pod.no_po=po.no_po inner join SIF.dbo.SIF_Barang brg on pod.kd_stok=brg.Kode_Barang where po.rec_stat='APPROVE' and SUBSTRING(po.no_po,12,1)='M' )a where a.qty_ordered is not null", sqlconn))
        DALOOKUP.Fill(ds_cari, "_lookup1")
        LookupPO.Properties.DataSource = ds_cari.Tables("_lookup1")
        LookupPO.Properties.ValueMember = "no_po"
        LookupPO.Properties.DisplayMember = "no_po"
    ...
    Private Sub txbSJSupplier_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txbSJSupplier.GotFocus
        If LookupPO.EditValue.ToString = "" Then
            LookupPO.Focus()
        End If
    End Sub

    Private Sub txbSJSupplier_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles txbSJSupplier.Validated
        DXEP.SetError(sender, "")

    End Sub

    'Public Sub delete_click()
    '    If MsgBox("Hapus Data Transaksi? ", MsgBoxStyle.Exclamation + MsgBoxStyle.YesNo, "Hapus Data") = MsgBoxResult.Yes Then
    '        Dim BTRANS As SqlTransaction

    '        Dim sqlcmd1 As New SqlCommand("delete INV.dbo.INV_QC where no_trans='" & LookupPO.Text & "'", sqlconn, BTRANS)
    '        sqlcmd1.ExecuteNonQuery()

    '        Dim sqlcmd2 As New SqlCommand("delete INV.dbo.INV_QC_M where no_trans='" & LookupPO.Text & "'", sqlconn, BTRANS)
    '        sqlcmd1.ExecuteNonQuery()
    ...
    Private Sub txbSJSupplier_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txbSJSupplier.Validating
        If LookupPO.Text.ToString <> "" Then
            If txbSJSupplier.Text = "" Then
                ...
                e.Cancel = True
            ElseIf STATE = "ADD" Or (STATE = "EDIT" And old_sj <> txbSJSupplier.Text) Then
                With New SqlCommand("select count(1) from inv.dbo.INV_QC_M where no_ref='" & LookupPO.EditValue & "' and sj_supplier='" & txbSJSupplier.Text & "'", sqlconn).ExecuteReader
                    .Read()
                    Try
                        If .Item(0) > 0 Then
                            DXEP.SetError(sender, "Nomor Surat Jalan Sudah Pernah Dipakai")
                            .Close()
                            e.Cancel = True
                        Else
                            .Close()
                    ...
                    Catch ex As Exception
                        ...
                        MsgBox("Error Occured : " & ex.Message, MsgBoxStyle.Critical + MsgBoxStyle.OkOnly, "Error")
                        txbSJSupplier.Text = ""
                        LookupPO.EditValue = vbNullString
                        e.Cancel = False
                    End Try
