...
Public Class aFormMonStok
    ...
    Private Sub aFormMonPR_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        awal_nilai.Visible = True
        nilai_in.Visible = True
        nilai_out.Visible = True
        nilai_rata.Visible = True
        akhir_nilai.Visible = True
        If kddep = KD_DIV_PPIC_INV Then
            awal_nilai.Visible = False
            nilai_in.Visible = False
            nilai_out.Visible = False
            nilai_rata.Visible = False
            akhir_nilai.Visible = False
        End If
    End Sub
    ...
    Private Sub loadLookUp()
        ...
        daLookUp = New SqlDataAdapter(New SqlCommand("SELECT Kode_Barang, Nama_Barang FROM SIF.dbo.SIF_Barang UNION SELECT '0000000000000000000000', '[Semua Barang]' order by nama_barang", sqlconn))
    ...
    Private Sub loadData(ByVal periode As String)
        ...
        If lookBarang.EditValue = "0000000000000000000000" Then
            daStokSaldo = New SqlDataAdapter(New SqlCommand("SELECT a.*, b.kd_jns_persd jenis, c.Nama_Departemen nama_divisi, d.Nama_Jenis jns_brg " & _
                                                            "FROM INV.dbo.INV_STOK_SALDO a, SIF.dbo.SIF_Barang b, sif.dbo.SIF_Departemen c, sif.dbo.SIF_Jenis_Barang d " & _
                                                            "WHERE a.kd_stok = b.Kode_Barang AND b.Kd_Depart = c.Kd_Departemen AND b.kd_jns_persd = d.Kd_Jns_Persd " & _
                                                            "AND b.kd_jenis = d.Kd_Jns_Brg AND a.bultah = '" & periode & "' order by  b.nama_barang", sqlconn))
        Else
            daStokSaldo = New SqlDataAdapter(New SqlCommand("SELECT a.*, b.kd_jns_persd jenis, c.Nama_Departemen nama_divisi, d.Nama_Jenis jns_brg " & _
                                                            "FROM INV.dbo.INV_STOK_SALDO a, SIF.dbo.SIF_Barang b, sif.dbo.SIF_Departemen c, sif.dbo.SIF_Jenis_Barang d " & _
                                                            "WHERE a.kd_stok = b.Kode_Barang AND b.Kd_Depart = c.Kd_Departemen AND b.kd_jns_persd = d.Kd_Jns_Persd " & _
                                                            "AND b.kd_jenis = d.Kd_Jns_Brg AND a.bultah = '" & periode & "' AND a.kd_stok = '" & lookBarang.EditValue & "' order by b.nama_barang", sqlconn))
        End If
