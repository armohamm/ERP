...
Public Class frmBM_Retur
    ...
    Public Sub edit_click()
        'enabel()
        DA = New SqlClient.SqlDataAdapter(New SqlClient.SqlCommand("select * from INV.DBO.INV_QC_M where substring((no_trans),12,5)='QC-RT'", sqlconn)) '
    ...
    Private Sub LoadStruktur(ByVal kode As String)
        ...
        If dSO.Tables("_qcm").Rows.Count > 0 Then
            ...
            TxbPenerima.Text = rw("penyerah").ToString
    ...
    Private Sub frmBM_Retur_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        connect()
        ...
        DALOOKUP1 = New SqlDataAdapter(New SqlCommand("select gdo.no_trans from INV.dbo.INV_GUDANG_OUT gdo inner join INV.dbo.INV_GUDANG_OUT_D gdod on gdo.no_trans=gdod.no_trans  where substring((gdo.no_trans),12,3)='RTR' and (gdod.qty_sisa_retur is null or gdod.qty_sisa_retur > 0)", sqlconn))
    ...
    Private Sub LookupNoRetur_EditValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles LookupNoRetur.EditValueChanged
        ...
        If STATE = "ADD" Then
            ...
            'MsgBox(LookupNoRetur.Text)
            Dim sqldr As SqlDataReader = New SqlCommand("select gdo.no_trans,brg.Nama_Barang,gdo.tipe_trans,gdo.penerima,gdod.kd_stok,gdod.qty_sisa_retur,gdod.kd_satuan,brg.kd_ukuran,uk.lebar,uk.Tinggi,uk.Panjang,brg.lokasi,brg.rek_persediaan,brg.rek_biaya,po.flag_ppn,rtrd.harga from inv.dbo.INV_GUDANG_OUT gdo inner join INV.dbo.INV_GUDANG_OUT_D gdod on gdo.no_trans=gdod.no_trans inner join SIF.dbo.SIF_Barang brg on gdod.kd_stok=brg.Kode_Barang inner join SIF.dbo.SIF_Ukuran uk on brg.kd_ukuran=uk.Kode_Ukuran inner join PURC.dbo.PURC_RETUR_BELI rtr on rtr.no_retur=gdo.no_ref inner join PURC.dbo.PURC_PO po on po.no_po=rtr.no_po inner join PURC.dbo.PURC_RETUR_BELI_D rtrd on rtr.no_retur=rtrd.no_retur where gdo.no_trans='" & LookupNoRetur.EditValue & "'", sqlconn).ExecuteReader
            Dim i As Int16 = 1
